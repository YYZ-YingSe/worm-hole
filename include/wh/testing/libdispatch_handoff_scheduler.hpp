...
class libdispatch_handoff_context {
public:
  using fallback_scheduler_type =
      experimental::execution::libdispatch_scheduler;
  ...
  public:
    ...
    struct domain;
    struct sender_env {
      libdispatch_handoff_context *context_{nullptr};
    ...
    public:
      ...
      auto start() & noexcept -> void {
        auto fb_scheduler = context_->fallback_scheduler();
        fallback_state_.emplace(
            stdexec::connect(stdexec::schedule(fb_scheduler),
  ...
  private:
    friend class libdispatch_handoff_context;

    explicit scheduler(libdispatch_handoff_context *context) noexcept
        : context_(context) {}

    libdispatch_handoff_context *context_{nullptr};
  };

  libdispatch_handoff_context()
      : queue_{DISPATCH_QUEUE_PRIORITY_DEFAULT} {}

  explicit libdispatch_handoff_context(int priority)
      : queue_{priority} {}
  ...
  [[nodiscard]] auto fallback_scheduler() noexcept -> fallback_scheduler_type {
    return queue_.get_scheduler();
  }

  class tls_scope {
  public:
    explicit tls_scope(libdispatch_handoff_context *context) noexcept
  ...
  private:
    libdispatch_handoff_context *previous_{nullptr};
  };
...
private:
  experimental::execution::libdispatch_queue queue_;
  inline static thread_local libdispatch_handoff_context
