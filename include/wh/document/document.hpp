...
template <typename impl_t>
concept sender_document_handler_move =
    requires(const impl_t &impl, document_request &&request) {
...
template <typename impl_t>
concept document_impl = sync_document_handler_with_context<impl_t> ||
                        async_document_handler<impl_t>;
...
[[nodiscard]] inline auto describe_impl(const impl_t &impl)
  ...
  if constexpr (requires {
    ...
    return wh::core::component_descriptor{"Document",
...
public:
  ...
  explicit document(impl_t &&impl) noexcept(
...
template <typename impl_t>
document(impl_t &&) -> document<std::remove_cvref_t<impl_t>>;
