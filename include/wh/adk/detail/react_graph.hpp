...
inline auto write_output_value(wh::agent::react_state &state, const std::string_view output_key,
  ...
  if (output_mode == wh::agent::react_output_mode::value) {
    state.output_values.insert_or_assign(std::string{output_key}, wh::core::any{message});
    return;
  }
...
[[nodiscard]] inline auto make_finalize_options(std::string output_key,
                                                const wh::agent::react_output_mode output_mode)
    -> wh::compose::graph_add_node_options {
  wh::compose::graph_add_node_options options{};
  options.state.bind_post(
      [output_key = std::move(output_key), output_mode](
        ...
        payload = wh::core::any(make_agent_output(react_state, *message));
        return {};
      });
  return options;
}

[[nodiscard]] inline auto make_emit_direct_options(std::string output_key,
                                                   const wh::agent::react_output_mode output_mode)
    -> wh::compose::graph_add_node_options {
  wh::compose::graph_add_node_options options{};
  options.state.bind_post(
  ...
  [[nodiscard]] auto lower() const -> wh::core::result<wh::compose::graph> {
    ...
    auto apply_tools = wh::compose::make_lambda_node(
        ...
        [toolset](
            wh::compose::graph_value &input, wh::core::run_context &,
            const wh::compose::graph_call_scope &) -> wh::core::result<wh::compose::graph_value> {
          auto *results = wh::core::any_cast<std::vector<wh::compose::tool_result>>(&input);
          if (results == nullptr) {
          ...
          for (const auto &result : *results) {
            ...
            if (!decision.direct_return && toolset.is_return_direct_tool(result.tool_name)) {
