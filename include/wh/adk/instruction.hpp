// Defines ADK instruction composition primitives with deterministic append and
// replace precedence.
#pragma once
...
public:
  ...
  /// highest-priority replace (if any) becomes the base, then append fragments
  /// at the same or higher priority are concatenated in stable order.
  [[nodiscard]] auto render(const std::string_view separator = "\n") const
    ...
    std::stable_sort(
        ...
        [](const instruction_entry *left, const instruction_entry *right) -> bool {
