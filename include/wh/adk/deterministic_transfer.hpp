...
/// Terminal reason used when deciding whether transfer messages may be appended.
...
[[nodiscard]] inline auto make_context_text(
    const wh::schema::message &message) -> std::string {
...
[[nodiscard]] inline auto is_transfer_assistant_message(
    const wh::schema::message &message) -> bool {
  ...
  for (const auto &part : message.parts) {
    ...
    if (tool != nullptr &&
        tool->name == deterministic_transfer_tool_name) {
      return true;
    }
  }
  return false;
...
[[nodiscard]] inline auto transfer_assistant_call_id(
    const wh::schema::message &message) -> std::optional<std::string_view> {
...
[[nodiscard]] inline auto is_transfer_tool_message(
    const wh::schema::message &message) -> bool {
...
[[nodiscard]] inline auto make_transfer_assistant_message(
    const std::string_view target_agent_name, const std::string_view tool_call_id)
    -> wh::schema::message {
  wh::schema::message message{};
...
[[nodiscard]] inline auto make_transfer_tool_message(
    const std::string_view target_agent_name, const std::string_view tool_call_id)
    -> wh::schema::message {
  wh::schema::message message{};
  ...
  message.parts.emplace_back(
      wh::schema::text_part{std::string{"transfer:"} +
...
[[nodiscard]] inline auto begin_deterministic_transfer(
  ...
  if (state.visited_agents.contains(resolved)) {
    return wh::core::result<std::string>::failure(
        wh::core::errc::already_exists);
  }
  state.visited_agents.insert(resolved);
  state.pending_target = resolved;
  return resolved;
}
...
inline auto record_parent_visible_event(deterministic_transfer_state &state,
  ...
  if (const auto *control = std::get_if<control_action>(&event.payload);
      control != nullptr &&
      control->kind == control_action_kind::interrupt) {
    return {};
  }
...
[[nodiscard]] inline auto rewrite_transfer_history(
    const std::span<const wh::schema::message> history,
  ...
  for (std::size_t index = 0U; index < history.size(); ++index) {
    ...
    const bool trim_assistant =
        ...
        detail::is_transfer_assistant_message(message);
    const bool trim_tool =
        trim.trim_tool_transfer_pair && detail::is_transfer_tool_message(message);
    ...
    const bool foreign_tool =
        message.role == wh::schema::message_role::tool &&
        !message.name.empty() && message.name != current_agent_name;
    if (foreign_assistant || foreign_tool) {
      rewritten.push_back(
          detail::make_context_message(detail::make_context_text(message)));
      continue;
    }
...
inline auto append_transfer_messages_once(
    std::vector<wh::schema::message> &history,
    deterministic_transfer_state &state, const std::string_view target_agent_name,
    const std::string_view tool_call_id,
    const transfer_completion_kind completion) -> wh::core::result<void> {
