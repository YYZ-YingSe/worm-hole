...
// projection of compose/core interrupt semantics.
#pragma once
...
/// Human/audit metadata attached to one ADK interrupt decision.
struct interrupt_audit {
  /// Stable audit identifier for trace/replay correlation.
  std::string audit_id{};
...
struct interrupt_info {
  ...
  /// Projected ADK run path exposing only agent/tool segments.
...
/// Projects one runtime address into the ADK-visible agent/tool path dialect.
...
/// Applies one ordered ADK patch batch. Later items overwrite earlier payloads
/// for the same interrupt id through resume-state upsert semantics.
