...
struct resume_target {
  ...
  /// Optional explicit location for this target; when absent the runner tries
  /// to resolve it from already-bound resume or interrupt state.
...
concept async_runner_handler_move =
    requires(const impl_t &impl, wh::compose::graph_invoke_request &&request,
             wh::core::run_context &context) {
      requires wh::compose::result_typed_sender<
          decltype(impl.run(std::move(request), context)),
          agent_run_result>;
    };
...
[[nodiscard]] inline auto has_checkpoint_service(
  ...
  return services != nullptr &&
         (services->checkpoint.store != nullptr ||
          services->checkpoint.backend != nullptr);
}

[[nodiscard]] inline auto
lower_run_request(const run_request &request) -> wh::compose::graph_invoke_request {
...
[[nodiscard]] inline auto
lower_run_request(run_request &&request) -> wh::compose::graph_invoke_request {
  wh::compose::graph_invoke_request lowered{};
...
[[nodiscard]] inline auto resolve_resume_location_one(
    const wh::core::run_context &context, const std::string_view interrupt_id)
...
[[nodiscard]] inline auto resolve_resume_location(
    const wh::core::run_context &context, const resume_target &target)
    -> wh::core::result<wh::core::address> {
...
inline auto append_resume_targets(
    const wh::core::run_context &context, const std::vector<resume_target> &targets,
    const bool reinterrupt_unmatched,
    wh::compose::graph_invoke_controls &controls) -> wh::core::result<void> {
...
dispatch_run(const impl_t &impl, wh::compose::graph_invoke_request request,
  ...
  if constexpr (async_runner_handler_move<impl_t>) {
    ...
    return wh::core::detail::normalize_result_sender<agent_run_result>(
        impl.run(static_cast<const wh::compose::graph_invoke_request &>(request),
                 context));
  }
}
...
[[nodiscard]] inline auto
lower_and_dispatch(const impl_t &impl, request_t &&request,
                   wh::core::run_context &context) {
  using stored_request_t = std::remove_cvref_t<request_t>;
  return wh::core::detail::defer_result_sender<agent_run_result>(
      [&impl, request = stored_request_t{std::forward<request_t>(request)},
       &context]() mutable {
        return dispatch_run(impl, lower_run_request(std::move(request)), context);
      });
}
...
lower_resume_and_dispatch(const impl_t &impl, resume_request request,
  ...
  using dispatch_sender_t = decltype(dispatch_run(
      std::declval<const impl_t &>(),
  ...
  return wh::core::detail::defer_result_sender<agent_run_result>(
      [&impl, request = std::move(request), &context]() mutable
          -> resume_sender_t {
        ...
        auto appended = append_resume_targets(context, request.targets,
                                             request.reinterrupt_unmatched,
                                             lowered.controls);
...
public:
  ...
  [[nodiscard]] auto run(request_t &&request,
    ...
    return detail::lower_and_dispatch(impl_, std::forward<request_t>(request),
                                      context);
  }
  ...
  /// Starts one resume path after lowering checkpoint and explicit target-resume
  /// controls into one compose invoke request.
