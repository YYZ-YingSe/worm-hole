...
[[nodiscard]] inline auto append_run_path_prefix(const run_path &prefix, const run_path &suffix)
    -> run_path {
...
[[nodiscard]] inline auto prefix_agent_event(agent_event event, const run_path &prefix)
    -> agent_event {
...
[[nodiscard]] inline auto collect_agent_messages(agent_message_stream_reader &&reader)
...
[[nodiscard]] inline auto collect_agent_events(agent_event_stream_reader &&reader)
    -> wh::core::result<std::vector<agent_event>> {
  return wh::schema::stream::collect_stream_reader(std::move(reader));
}

/// Returns the last owned message carried by one event sequence, when any.
[[nodiscard]] inline auto find_final_message(const std::vector<agent_event> &events)
...
[[nodiscard]] inline auto make_run_request(query_request request) -> run_request {
...
concept async_runner_handler_const = requires(const impl_t &impl,
                                              const wh::compose::graph_invoke_request &request,
                                              wh::core::run_context &context) {
  requires wh::compose::result_typed_sender<decltype(impl.run(request, context)), agent_run_result>;
};
...
concept async_runner_handler_move =
    requires(const impl_t &impl, wh::compose::graph_invoke_request &&request,
             wh::core::run_context &context) {
      requires wh::compose::result_typed_sender<decltype(impl.run(std::move(request), context)),
                                                agent_run_result>;
    };

template <typename impl_t>
concept runner_impl = async_runner_handler_const<impl_t> || async_runner_handler_move<impl_t>;

template <typename impl_t>
[[nodiscard]] inline auto dispatch_run(const impl_t &impl,
                                       wh::compose::graph_invoke_request request,
                                       wh::core::run_context &context);

[[nodiscard]] inline auto
has_checkpoint_service(const wh::compose::graph_runtime_services *services) noexcept -> bool {
  return services != nullptr &&
         (services->checkpoint.store != nullptr || services->checkpoint.backend != nullptr);
}
...
[[nodiscard]] inline auto lower_run_request(const run_request &request)
  ...
  lowered.input = wh::compose::graph_input::value(wh::core::any{request.messages});
...
template <typename impl_t>
using runner_failure_sender_t = decltype(wh::core::detail::failure_result_sender<agent_run_result>(
...
template <typename impl_t>
using runner_sender_t = wh::core::detail::variant_sender<runner_failure_sender_t<impl_t>,
                                                         runner_dispatch_sender_t<impl_t>>;

template <typename impl_t>
[[nodiscard]] inline auto make_runner_failure_sender(const wh::core::error_code error)
    -> runner_sender_t<impl_t> {
  return runner_sender_t<impl_t>{wh::core::detail::failure_result_sender<agent_run_result>(error)};
}

[[nodiscard]] inline auto lower_run_request(run_request &&request)
    -> wh::compose::graph_invoke_request {
  wh::compose::graph_invoke_request lowered{};
  lowered.input = wh::compose::graph_input::value(wh::core::any{std::move(request.messages)});
  lowered.controls = std::move(request.options.compose_controls);
  lowered.services = request.options.compose_services;
  return lowered;
}

[[nodiscard]] inline auto resolve_resume_location_one(const wh::core::run_context &context,
  ...
  if (context.interrupt_info.has_value() && context.interrupt_info->interrupt_id == interrupt_id) {
    return context.interrupt_info->location;
  }

  return wh::core::result<wh::core::address>::failure(wh::core::errc::not_found);
}

[[nodiscard]] inline auto resolve_resume_location(const wh::core::run_context &context,
...
inline auto append_resume_targets(const wh::core::run_context &context,
  ...
  controls.resume.batch_items.reserve(controls.resume.batch_items.size() + targets.size());
  controls.resume.contexts.reserve(controls.resume.contexts.size() + targets.size());
...
[[nodiscard]] inline auto dispatch_run(const impl_t &impl,
  ...
  if constexpr (async_runner_handler_move<impl_t>) {
    ...
    return wh::core::detail::normalize_result_sender<agent_run_result>(
        impl.run(static_cast<const wh::compose::graph_invoke_request &>(request), context));
  }
}

template <typename impl_t, typename request_t>
  requires std::same_as<std::remove_cvref_t<request_t>, wh::adk::run_request>
[[nodiscard]] inline auto lower_run_and_dispatch(const impl_t &impl, request_t &&request,
                                                 wh::core::run_context &context) {
...
[[nodiscard]] inline auto lower_query_and_dispatch(const impl_t &impl, request_t &&request,
                                                   wh::core::run_context &context) {
  auto owned_request = wh::core::into_owned(std::forward<request_t>(request));
  if (owned_request.has_error()) {
    return make_runner_failure_sender<impl_t>(owned_request.error());
  }
  return lower_run_and_dispatch(impl, make_run_request(std::move(owned_request).value()), context);
}

template <typename impl_t, typename request_t>
  requires std::same_as<std::remove_cvref_t<request_t>, wh::adk::resume_request>
[[nodiscard]] inline auto lower_resume_and_dispatch(const impl_t &impl, request_t &&request,
                                                    wh::core::run_context &context) {
  auto owned_request = wh::core::into_owned(std::forward<request_t>(request));
  if (owned_request.has_error()) {
    return make_runner_failure_sender<impl_t>(owned_request.error());
...
public:
  ...
  explicit runner(impl_t impl) noexcept(std::is_nothrow_move_constructible_v<impl_t>)
      : impl_(std::move(impl)) {}
  ...
  [[nodiscard]] auto implementation() const noexcept -> const impl_t & { return impl_; }
  ...
  [[nodiscard]] auto run(request_t &&request, wh::core::run_context &context) const {
    return detail::lower_run_and_dispatch(impl_, std::forward<request_t>(request), context);
  }
  ...
  [[nodiscard]] auto query(request_t &&request, wh::core::run_context &context) const {
    return detail::lower_query_and_dispatch(impl_, std::forward<request_t>(request), context);
  }
  ...
  [[nodiscard]] auto resume(request_t &&request, wh::core::run_context &context) const {
    return detail::lower_resume_and_dispatch(impl_, std::forward<request_t>(request), context);
  }
...
template <> struct any_owned_traits<wh::adk::run_options> {
  [[nodiscard]] static auto into_owned(const wh::adk::run_options &value)
    ...
    if (compose_controls.has_error()) {
      return wh::core::result<wh::adk::run_options>::failure(compose_controls.error());
    }
  ...
  [[nodiscard]] static auto into_owned(const wh::adk::resume_request &value)
    ...
    for (const auto &target : value.targets) {
      ...
      if (owned_target.has_error()) {
        return wh::core::result<wh::adk::resume_request>::failure(owned_target.error());
      }
