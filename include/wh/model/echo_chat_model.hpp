...
[[nodiscard]] inline auto first_text_part(const wh::schema::message &message) -> std::string {
  const auto iter =
      std::ranges::find_if(message.parts, [](const wh::schema::message_part &part) noexcept {
...
[[nodiscard]] inline auto make_echo_response(const chat_request &request)
  ...
  if (request.messages.empty()) {
    return wh::core::result<chat_response>::failure(wh::core::errc::invalid_argument);
  }
  ...
  response_message.parts.emplace_back(wh::schema::text_part{first_text_part(last)});
...
public:
  ...
  explicit echo_chat_model_impl(const chat_model_options &options) : options_(options) {}
  explicit echo_chat_model_impl(chat_model_options &&options) : options_(std::move(options)) {}

  [[nodiscard]] auto descriptor() const -> wh::core::component_descriptor {
    return wh::core::component_descriptor{"EchoChatModel", wh::core::component_kind::model};
  }

  [[nodiscard]] auto invoke(const chat_request &request) const -> wh::core::result<chat_response> {
    return make_echo_response(request);
  }

  [[nodiscard]] auto invoke(chat_request &&request) const -> wh::core::result<chat_response> {
  ...
  [[nodiscard]] auto stream(const chat_request &request) const
    ...
    if (response.has_error()) {
      return wh::core::result<chat_message_stream_reader>::failure(response.error());
    }

    auto [writer, reader] = wh::schema::stream::make_pipe_stream<wh::schema::message>(8U);
    auto write_status = writer.try_write(std::move(response).value().message);
    if (write_status.has_error()) {
      return wh::core::result<chat_message_stream_reader>::failure(write_status.error());
    }
    auto close_status = writer.close();
    if (close_status.has_error()) {
      return wh::core::result<chat_message_stream_reader>::failure(close_status.error());
    }
  ...
  auto finish_stream_event(const chat_request &, chat_model_callback_event &event) const -> void {
  ...
  [[nodiscard]] auto options() const noexcept -> const chat_model_options & { return options_; }
