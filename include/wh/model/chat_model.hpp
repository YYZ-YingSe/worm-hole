...
make_run_info(const wh::core::component_descriptor &descriptor)
    -> wh::callbacks::run_info;

[[nodiscard]] inline auto
make_callback_event(const wh::core::component_descriptor &descriptor,
...
[[nodiscard]] inline auto describe_impl(const impl_t &impl)
  ...
  if constexpr (requires {
    ...
    return wh::core::component_descriptor{"ChatModel",
...
concept sender_invoke_handler_move =
    requires(const impl_t &impl, chat_request &&request) {
...
template <typename impl_t>
concept model_impl =
    (async_invoke_handler<impl_t> || sync_invoke_handler<impl_t>) &&
    (async_stream_handler<impl_t> || sync_stream_handler<impl_t>);

template <typename impl_t>
[[nodiscard]] inline auto run_sync_invoke_impl(const impl_t &impl,
...
template <wh::core::resume_mode Resume, typename impl_t, typename request_t,
  ...
  requires async_invoke_handler<impl_t>
[[nodiscard]] inline auto
invoke_sender(const impl_t &impl,
              const wh::core::component_descriptor &descriptor,
              request_t &&request, callback_sink sink, scheduler_t scheduler) {
  using callback_state_t =
      std::pair<wh::callbacks::run_info, chat_model_callback_event>;
  return wh::core::detail::component_async_entry<Resume>(
      ...
      [&descriptor](const chat_request &state_request) {
        return callback_state_t{
            wh::callbacks::apply_component_run_info(make_run_info(descriptor),
      ...
      [](const callback_sink &start_sink, const callback_state_t &state) {
        emit_callback(start_sink, wh::callbacks::stage::start, state.second,
                      state.first);
      },
      [](const callback_sink &success_sink, callback_state_t &state,
         chat_invoke_result &status) {
      ...
      [](const callback_sink &error_sink, callback_state_t &state,
         chat_invoke_result &) {
        auto &[run_info, event] = state;
...
template <wh::core::resume_mode Resume, typename impl_t, typename request_t,
  ...
  requires async_stream_handler<impl_t>
[[nodiscard]] inline auto
stream_sender(const impl_t &impl,
              const wh::core::component_descriptor &descriptor,
              request_t &&request, callback_sink sink, scheduler_t scheduler) {
  sink = wh::callbacks::filter_callback_sink(std::move(sink), request.options);
  auto state = std::optional<
      std::pair<wh::callbacks::run_info, chat_model_callback_event>>{};
  if (sink.has_value()) {
    state.emplace(wh::callbacks::apply_component_run_info(
  ...
  return wh::core::detail::defer_result_sender<chat_message_stream_result>(
      [&impl, request = chat_request{std::forward<request_t>(request)},
       ...
       scheduler = std::move(scheduler)]() mutable {
        if (state.has_value()) {
          emit_callback(sink, wh::callbacks::stage::start, state->second,
                        state->first);
        }
        return wh::core::detail::inspect_result_sender(
            ...
            [&impl, request = std::move(request), sink = std::move(sink),
             state =
              ...
              auto &[run_info, event] = *state;
              if (status.has_error()) {
                emit_callback(sink, wh::callbacks::stage::error, event,
                              run_info);
                return;
              }
...
make_callback_event(const wh::core::component_descriptor &descriptor,
  ...
  event.model_id = options.model_id.empty() ? descriptor.type_name
                                            : std::string{options.model_id};
...
public:
  ...
  explicit chat_model(impl_t &&impl) noexcept(
  ...
  {
    return invoke_sync_impl(request,
  ...
  [[nodiscard]] auto invoke(chat_request &&request,
    ...
    requires detail::sync_invoke_handler<impl_t>
  {
    return invoke_sync_impl(std::move(request),
                            detail::borrow_callback_sink(callback_context));
  }
...
private:
  ...
  [[nodiscard]] auto invoke_sync_impl(request_t &&request,
    ...
    const auto run_info = wh::callbacks::apply_component_run_info(
        detail::make_run_info(descriptor), request.options);
    ...
    auto output =
        detail::run_sync_invoke_impl(impl_, std::forward<request_t>(request));
    if (output.has_error()) {
  ...
  [[nodiscard]] auto stream_sync_impl(request_t &&request,
    ...
    auto output =
        detail::run_sync_stream_impl(impl_, std::forward<request_t>(request));
    if (output.has_error()) {
      detail::emit_callback(sink, wh::callbacks::stage::error, event, run_info);
      return output;
    }
  ...
  [[nodiscard]] auto invoke_async_impl(request_t &&request,
    ...
    return wh::core::detail::defer_resume_sender<Resume>(
        [this, request = chat_request{std::forward<request_t>(request)},
         sink = std::move(sink), descriptor](auto scheduler) mutable {
          return detail::invoke_sender<Resume>(
              impl_, descriptor, std::move(request), std::move(sink),
              std::move(scheduler));
        });
  }
  ...
  [[nodiscard]] auto stream_async_impl(request_t &&request,
    ...
    return wh::core::detail::defer_resume_sender<Resume>(
        [this, request = chat_request{std::forward<request_t>(request)},
         sink = std::move(sink), descriptor](auto scheduler) mutable {
          return detail::stream_sender<Resume>(
              impl_, descriptor, std::move(request), std::move(sink),
              std::move(scheduler));
        });
  }
...
template <typename impl_t>
chat_model(impl_t &&) -> chat_model<std::remove_cvref_t<impl_t>>;
...
concept chat_model_like =
    ...
    requires(const model_t &model, chat_request &&movable_request,
      ...
      {
        model.invoke(std::move(movable_request), context)
      } -> std::same_as<chat_invoke_result>;
      {
...
invoke_with_fallback(const std::span<const model_t> models,
  ...
  auto report = invoke_with_fallback_report_only(models, request,
...
invoke_with_fallback(const std::span<const model_t> models,
                     ...
                     const bool provider_native_supported = false)
    -> wh::core::result<invoke_fallback_report> {
  auto report = invoke_with_fallback_report_only(models, std::move(request),
                                                 provider_native_supported);
  if (report.final_error.has_value()) {
    return wh::core::result<invoke_fallback_report>::failure(
        *report.final_error);
