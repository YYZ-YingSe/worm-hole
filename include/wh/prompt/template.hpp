...
public:
  ...
  template_value(const integer_t value) noexcept
      : storage_(static_cast<std::int64_t>(value)) {}

  template <std::floating_point floating_t>
  template_value(const floating_t value) noexcept
...
[[nodiscard]] inline auto resolve_delimiters(const template_syntax syntax)
    -> template_delimiters {
  switch (syntax) {
...
[[nodiscard]] inline auto template_value_to_json(const template_value &value)
    -> ordered_json;

[[nodiscard]] inline auto template_object_to_json(const template_object &object)
    -> ordered_json {
...
[[nodiscard]] inline auto template_array_to_json(const template_array &array)
    -> ordered_json {
      ...
      []<typename value_t>(const value_t &inner) -> ordered_json {
        ...
        if constexpr (std::same_as<inner_t, std::nullptr_t>) {
          return nullptr;
                             ...
                             std::same_as<inner_t, std::int64_t> ||
                             std::same_as<inner_t, double> ||
                             std::same_as<inner_t, std::string>) {
          return inner;
...
[[nodiscard]] inline auto find_object_value(const template_object &object,
  ...
  if (iter == object.end()) {
    return wh::core::result<const template_value *>::failure(
...
[[nodiscard]] inline auto descend_path(const template_value &current,
...
[[nodiscard]] inline auto jinja_render(const std::string_view text,
                                       const template_context &context)
    -> wh::core::result<std::string> {
  std::shared_ptr<minja::TemplateNode> root{};
  try {
    root = minja::Parser::parse(std::string{text},
                                minja::Options{false, false, false});
  ...
  try {
    ...
    return wh::core::result<std::string>::failure(
        map_minja_runtime_error(error.what()));
  }
}
...
[[nodiscard]] inline auto render_text_template(
    const std::string_view text, const template_context &context,
