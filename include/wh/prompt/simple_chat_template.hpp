...
public:
  ...
  explicit simple_chat_template_impl(const std::vector<prompt_message_template> &templates)
      : templates_(templates) {}
  explicit simple_chat_template_impl(std::vector<prompt_message_template> &&templates)
      : templates_(std::move(templates)) {}

  [[nodiscard]] auto render(const prompt_render_request &request,
                            prompt_callback_event &event) const -> prompt_result {
    return render_common(request, event);
  }

  [[nodiscard]] auto render(prompt_render_request &&request, prompt_callback_event &event) const
      -> prompt_result {
    return render_common(std::move(request), event);
  }

  auto add_template(const prompt_message_template &value) -> simple_chat_template_impl & {
    templates_.push_back(value);
    return *this;
  }

  auto add_template(prompt_message_template &&value) -> simple_chat_template_impl & {
    templates_.push_back(std::move(value));
    return *this;
  }

  [[nodiscard]] auto templates() const noexcept -> const std::vector<prompt_message_template> & {
    return templates_;
  }

private:
  template <typename request_t>
  [[nodiscard]] auto render_common(request_t &&request, prompt_callback_event &event) const
      -> prompt_result {
    const auto extract_missing_variable = [](const std::string &text) -> std::string {
    ...
    for (const auto &entry : templates_) {
      auto rendered = wh::prompt::render_text_template(entry.text, request.context, options.syntax);
      if (rendered.has_error()) {
        if (rendered.error() == wh::core::errc::not_found && !options.strict_missing_variables) {
...
class simple_chat_template final : public chat_template<detail::simple_chat_template_impl> {
  using base_t = chat_template<detail::simple_chat_template_impl>;

public:
  simple_chat_template() : base_t(detail::simple_chat_template_impl{}) {}
  explicit simple_chat_template(const std::vector<prompt_message_template> &templates)
  ...
  auto add_template(const prompt_message_template &value) -> simple_chat_template & {
