...
make_callback_state(const prompt_render_request &request) -> callback_state {
...
concept sync_prompt_handler = requires(const impl_t &impl,
...
template <typename impl_t>
concept sender_prompt_handler_move =
    requires(const impl_t &impl, prompt_render_request &&request) {
...
template <typename impl_t>
concept chat_template_impl =
    async_prompt_handler<impl_t> || sync_prompt_handler<impl_t>;

template <typename impl_t>
[[nodiscard]] inline auto
run_sync_prompt_impl(const impl_t &impl, const prompt_render_request &request,
                     prompt_callback_event &event) -> prompt_result {
  if constexpr (requires {
                  {
                    impl.render(request, event)
                  } -> std::same_as<prompt_result>;
                }) {
...
[[nodiscard]] inline auto run_sync_prompt_impl(const impl_t &impl,
                                               prompt_render_request &&request,
                                               prompt_callback_event &event)
    -> prompt_result {
  if constexpr (requires {
                  {
                    impl.render(std::move(request), event)
                  } -> std::same_as<prompt_result>;
                }) {
    return impl.render(std::move(request), event);
  } else if constexpr (requires {
                         {
                           impl.render(std::move(request))
                         } -> std::same_as<prompt_result>;
                       }) {
...
[[nodiscard]] inline auto make_render_sender(const impl_t &impl,
                                             request_t &&request) {
...
[[nodiscard]] inline auto render_sender(const impl_t &impl, request_t &&request,
  ...
  return wh::core::detail::component_async_entry<Resume>(
      ...
      [&impl](auto &&forwarded_request) {
        return make_render_sender(
            impl, std::forward<decltype(forwarded_request)>(forwarded_request));
      },
      [](const prompt_render_request &state_request) {
      ...
      [](const callback_sink &success_sink, callback_state &state,
      ...
      [](const callback_sink &error_sink, callback_state &state,
         prompt_result &) {
...
[[nodiscard]] inline auto render_sender(const impl_t &impl, request_t &&request,
                                        callback_sink sink) {
  return render_sender<wh::core::resume_mode::unchanged>(
      impl, std::forward<request_t>(request), std::move(sink),
...
public:
  ...
  explicit chat_template(impl_t &&impl) noexcept(
  ...
  [[nodiscard]] auto descriptor() const -> wh::core::component_descriptor {
    return wh::core::component_descriptor{"Prompt",
  ...
  {
    return render_sync_impl(request,
  ...
  [[nodiscard]] auto render(prompt_render_request &&request,
    ...
    requires detail::sync_prompt_handler<impl_t>
  {
    return render_sync_impl(std::move(request),
                            detail::borrow_callback_sink(callback_context));
  }
  ...
  {
    return render_async_impl(request,
  ...
  [[nodiscard]] auto async_render(prompt_render_request &&request,
    ...
    requires detail::async_prompt_handler<impl_t>
  {
    return render_async_impl(std::move(request),
                             detail::make_callback_sink(callback_context));
  }
...
private:
  ...
  [[nodiscard]] auto render_sync_impl(request_t &&request,
    ...
    auto output = detail::run_sync_prompt_impl(
        impl_, std::forward<request_t>(request), state.event);
    if (output.has_error()) {
  ...
  [[nodiscard]] auto render_async_impl(request_t &&request,
                                       detail::callback_sink sink) const
      -> auto {
    return wh::core::detail::defer_resume_sender<Resume>(
        [this,
         request = prompt_render_request{std::forward<request_t>(request)},
         sink = std::move(sink)](auto scheduler) mutable {
          return detail::render_sender<Resume>(
...
template <typename impl_t>
chat_template(impl_t &&) -> chat_template<std::remove_cvref_t<impl_t>>;
...
concept chat_template_like =
    ...
    requires(const template_t &tmpl, const prompt_render_request &request,
             prompt_render_request &&movable_request,
