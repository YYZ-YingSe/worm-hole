...
concept memory_pool_like = requires(const pool_t &pool, const memory_acquire_request &request,
  ...
  { pool.acquire(std::move(movable_request)) } -> std::same_as<memory_acquire_result>;
  { pool.release(block) } -> std::same_as<memory_release_result>;
  { pool.release(std::move(movable_block)) } -> std::same_as<memory_release_result>;
};
