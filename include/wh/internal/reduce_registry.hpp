...
using dynamic_reduce_function =
    wh::core::function<wh::core::result<dynamic_reduce_value>(dynamic_reduce_values) const>;
...
public:
  ...
  using typed_reduce_fn_t =
      wh::core::function<wh::core::result<value_t>(std::span<const value_t>) const>;
  ...
  auto register_reducer(function_t &&function_value) -> wh::core::result<void> {
    auto function = typed_reduce_fn_t<value_t>{std::forward<function_t>(function_value)};
    if (frozen_) {
    ...
    auto [typed_iter, typed_inserted] = typed_table_.emplace(type, std::move(function));
    if (!typed_inserted) {
      return wh::core::result<void>::failure(wh::core::errc::already_exists);
    }
        ...
        dynamic_table_
            .emplace(
                ...
                [bridge = std::move(bridge)](
                    const dynamic_reduce_values values) -> wh::core::result<dynamic_reduce_value> {
                  ...
                  auto reduced =
                      bridge(std::span<const value_t>{typed_values.data(), typed_values.size()});
                  if (reduced.has_error()) {
                    return wh::core::result<dynamic_reduce_value>::failure(reduced.error());
                  }
  ...
  template <typename value_t, typename function_t>
  auto register_reducer_from_ptrs(function_t &&function_value) -> wh::core::result<void> {
    auto function = typed_ptr_reduce_fn_t<value_t>{std::forward<function_t>(function_value)};
    if (frozen_) {
      return wh::core::result<void>::failure(wh::core::errc::contract_violation);
    }
    if (!static_cast<bool>(function)) {
    ...
    auto typed_function =
        [function](const std::span<const value_t> values) -> wh::core::result<value_t> {
      wh::core::small_vector<const value_t *, 8U> typed_values{};
      typed_values.reserve(static_cast<typename decltype(typed_values)::size_type>(values.size()));
      for (const auto &value : values) {
        typed_values.push_back(std::addressof(value));
      }
      return function(std::span<const value_t *>{typed_values.data(), typed_values.size()});
    };
    ...
    const auto dynamic_inserted =
        dynamic_table_
            .emplace(type,
                     [function = std::move(function)](const dynamic_reduce_values values)
  ...
  [[nodiscard]] auto reduce(const wh::core::any_type_key type,
      ...
      -> wh::core::result<dynamic_reduce_value> {
    if (values.empty()) {
      return wh::core::result<dynamic_reduce_value>::failure(wh::core::errc::invalid_argument);
    }
    ...
    return wh::core::result<dynamic_reduce_value>::failure(wh::core::errc::not_supported);
  }

  /// Looks up one typed reducer for `value_t`.
  template <typename value_t>
  [[nodiscard]] auto find_typed() const noexcept -> const typed_reduce_fn_t<value_t> * {
  ...
  [[nodiscard]] auto size() const noexcept -> std::size_t { return dynamic_table_.size(); }

private:
  std::unordered_map<wh::core::any_type_key, dynamic_reduce_function, wh::core::any_type_key_hash>
      dynamic_table_{};
