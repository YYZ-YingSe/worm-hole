...
public:
  ...
  auto register_global_callbacks(config_t &&config, callbacks_t &&callbacks)
    ...
    for_each_stage([&](const wh::core::callback_stage stage) -> void {
      ...
      append_registration(global_registrations_[stage_index(stage)],
  ...
  auto register_local_callbacks(config_t &&config, callbacks_t &&callbacks)
    ...
    for_each_stage([&](const wh::core::callback_stage stage) -> void {
      ...
      registration.config = stable_config;
      registration.callback = make_stage_callback(*stage_callback);
      local_registrations_[stage_index(stage)].push_back(
          std::move(registration));
    });
  }
  ...
  auto dispatch(const wh::core::callback_stage stage,
    ...
    auto execute_registrations = [&](const registration_list &registrations,
      ...
      if (reverse_order) {
        for (auto iter = registrations.rbegin(); iter != registrations.rend();
             ++iter) {
  ...
  auto dispatch_single(const wh::core::callback_stage stage,
                       ...
                       const wh::core::stage_payload_callback &callback) const
      -> void {
  ...
  auto dispatch_single(const wh::core::callback_stage stage,
                       wh::core::callback_event_payload &&payload,
                       const wh::core::callback_run_info &run_info,
                       wh::core::stage_payload_callback &&callback) const
      -> void {
  ...
  template <wh::core::StagePayloadCallbackLike callback_t>
    requires(!std::same_as<wh::core::remove_cvref_t<callback_t>,
...
private:
  ...
  static auto for_each_stage(callback_t &&callback) -> void {
  ...
  stage_index(const wh::core::callback_stage stage) noexcept -> std::size_t {
    return static_cast<std::size_t>(stage);
  }

  [[nodiscard]] static auto
  make_stage_callback(const wh::core::stage_view_callback &callback)
...
[[nodiscard]] inline auto merge_callback_config(config_t &&...configs)
  ...
  auto merge_one = [&](auto &&config) -> void {
    auto stable_config =
        wh::core::callback_config{std::forward<decltype(config)>(config)};
