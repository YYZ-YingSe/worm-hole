...
public:
  ...
  auto register_merge(std::function<wh::core::result<value_t>(
    ...
    auto [typed_iter, typed_inserted] =
        typed_table_.emplace(type, typed_merge_fn{std::move(function)});
    if (!typed_inserted) {
      return wh::core::result<void>::failure(wh::core::errc::already_exists);
    }
    const auto *typed_function =
        std::any_cast<const typed_merge_fn>(&typed_iter->second);
  ...
  auto register_merge_from_ptrs(
    ...
    using typed_merge_fn = typed_merge_fn_t<value_t>;
    auto [typed_iter, typed_inserted] =
        typed_table_.emplace(type, typed_merge_fn{std::move(typed_function)});
    if (!typed_inserted) {
      return wh::core::result<void>::failure(wh::core::errc::already_exists);
    }
...
private:
  ...
  [[nodiscard]] auto find_typed_merge() const noexcept
    ...
    return std::any_cast<const typed_merge_fn_t<value_t>>(&iter->second);
  }
