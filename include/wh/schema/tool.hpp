...
[[nodiscard]] inline auto tool_mode_name(const tool_call_mode mode)
    -> std::string_view {
  switch (mode) {
  case tool_call_mode::disable:
    return "disable";
  case tool_call_mode::allow:
    return "allow";
  case tool_call_mode::force:
    return "force";
  }
  return "allow";
}
...
inline auto validate_tool_choice(const tool_choice &choice)
    -> wh::core::result<void> {
  if (choice.mode == tool_call_mode::force && choice.name.empty()) {
    return wh::core::result<void>::failure(wh::core::errc::invalid_argument);
  }
  if (choice.mode == tool_call_mode::disable && !choice.name.empty()) {
    return wh::core::result<void>::failure(wh::core::errc::invalid_argument);
  }
  return {};
}
...
[[nodiscard]] inline auto
build_tool_json_schema(const tool_schema_definition &definition)
    -> wh::core::result<wh::core::json_document> {
  if (definition.name.empty()) {
    return wh::core::result<wh::core::json_document>::failure(
        wh::core::errc::invalid_argument);
  }
  auto choice_status = validate_tool_choice(definition.choice);
  if (choice_status.has_error()) {
    return wh::core::result<wh::core::json_document>::failure(
        choice_status.error());
  }
  ...
  wh::core::json_value choice;
  choice.SetObject();
  detail::add_json_string_member(choice, "mode",
                                 detail::tool_mode_name(definition.choice.mode),
                                 output.GetAllocator());
  if (!definition.choice.name.empty()) {
    detail::add_json_string_member(choice, "name", definition.choice.name,
                                   output.GetAllocator());
  }
  detail::add_json_value_member(output, "tool_choice", std::move(choice),
                                output.GetAllocator());
  return output;
}
