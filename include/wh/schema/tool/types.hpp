...
[[nodiscard]] inline auto parameter_type_name(const tool_parameter_type type) -> std::string_view {
  switch (type) {
...
inline auto add_json_string_member(wh::core::json_value &target, const std::string_view key,
  ...
  value_node.SetString(value.data(), static_cast<wh::core::json_size_type>(value.size()),
                       allocator);
  target.AddMember(key_node.Move(), value_node.Move(), allocator);
}

/// Adds value member to a RapidJSON object.
inline auto add_json_value_member(wh::core::json_value &target, const std::string_view key,
                                  wh::core::json_value value, wh::core::json_allocator &allocator)
    -> void {
  wh::core::json_value key_node;
  key_node.SetString(key.data(), static_cast<wh::core::json_size_type>(key.size()), allocator);
...
[[nodiscard]] inline auto build_parameter_schema(const tool_parameter_schema &parameter,
                                                 wh::core::json_allocator &allocator)
    -> wh::core::result<wh::core::json_value> {
  wh::core::json_value output;
  output.SetObject();
  add_json_string_member(output, "type", parameter_type_name(parameter.type), allocator);
  if (!parameter.description.empty()) {
    add_json_string_member(output, "description", parameter.description, allocator);
  }
  ...
  if (!parameter.enum_values.empty()) {
    ...
    for (const auto &item : parameter.enum_values) {
      ...
      value.SetString(item.data(), static_cast<wh::core::json_size_type>(item.size()), allocator);
  ...
  if (parameter.type == tool_parameter_type::object) {
    ...
    std::ranges::sort(sorted_properties,
                      [](const tool_parameter_schema *left, const tool_parameter_schema *right) {
    ...
    for (const auto *property : sorted_properties) {
      ...
      if (property_schema.has_error()) {
        return wh::core::result<wh::core::json_value>::failure(property_schema.error());
      }

      wh::core::json_value key;
      key.SetString(property->name.data(),
                    static_cast<wh::core::json_size_type>(property->name.size()), allocator);
      properties.AddMember(key.Move(), std::move(property_schema).value(), allocator);
      if (property->required) {
        wh::core::json_value required_key;
        required_key.SetString(property->name.data(),
    ...
    add_json_value_member(output, "properties", std::move(properties), allocator);
  ...
  if (parameter.type == tool_parameter_type::array) {
    if (!parameter.item_types.empty()) {
      if (parameter.item_types.size() == 1U) {
        auto item_schema = build_parameter_schema(parameter.item_types.front(), allocator);
        if (item_schema.has_error()) {
          return wh::core::result<wh::core::json_value>::failure(item_schema.error());
        }
        add_json_value_member(output, "items", std::move(item_schema).value(), allocator);
      } else {
...
build_parameters_json_schema(const std::span<const tool_parameter_schema> parameters)
    -> wh::core::result<wh::core::json_document> {
  wh::core::json_document output;
  output.SetObject();
  detail::add_json_string_member(output, "type", "object", output.GetAllocator());
  ...
  for (const auto *parameter : sorted_parameters) {
    auto schema = detail::build_parameter_schema(*parameter, output.GetAllocator());
    if (schema.has_error()) {
    ...
    key.SetString(parameter->name.data(),
                  ...
                  output.GetAllocator());
    properties.AddMember(key.Move(), std::move(schema).value(), output.GetAllocator());
    if (parameter->required) {
      wh::core::json_value required_name;
      required_name.SetString(parameter->name.data(),
                              static_cast<wh::core::json_size_type>(parameter->name.size()),
                              output.GetAllocator());
  ...
  detail::add_json_value_member(output, "properties", std::move(properties), output.GetAllocator());
  detail::add_json_value_member(output, "required", std::move(required), output.GetAllocator());
  return output;
}

/// Builds the default function-style tool JSON schema export.
[[nodiscard]] inline auto build_default_tool_json_schema(const tool_schema_definition &definition)
    -> wh::core::result<wh::core::json_document> {
  ...
  wh::core::json_document output;
  output.SetObject();
  detail::add_json_string_member(output, "type", "function", output.GetAllocator());

  wh::core::json_value function;
  function.SetObject();
  detail::add_json_string_member(function, "name", definition.name, output.GetAllocator());
  detail::add_json_string_member(function, "description", definition.description,
                                 output.GetAllocator());
  ...
  if (!definition.raw_parameters_json_schema.empty()) {
    ...
    if (raw.has_error()) {
      return wh::core::result<wh::core::json_document>::failure(wh::core::errc::parse_error);
    }
  ...
  detail::add_json_value_member(function, "parameters", std::move(parameters),
                                output.GetAllocator());
  detail::add_json_value_member(output, "function", std::move(function), output.GetAllocator());
  return output;
}
