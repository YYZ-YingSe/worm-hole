...
template <typename reader_t> struct copy_stream_policy {
  using result_type = std::remove_cvref_t<decltype(std::declval<reader_t &>().read())>;
  using source_try_type = std::remove_cvref_t<decltype(std::declval<reader_t &>().try_read())>;
  ...
  [[nodiscard]] static auto pending() noexcept -> try_result_type { return stream_pending; }
...
class copy_stream_reader final
    : public stream_base<copy_stream_reader<reader_t>, typename reader_t::value_type> {
private:
  using core_reader_t = ::wh::core::cursor_reader<reader_t, detail::copy_stream_policy<reader_t>>;
...
public:
  ...
  template <typename source_reader_t>
    requires(!std::same_as<std::remove_cvref_t<source_reader_t>, copy_stream_reader>) &&
  ...
  explicit copy_stream_reader(source_reader_t &&reader) {
    auto readers = core_reader_t::make_readers(reader_t{std::forward<source_reader_t>(reader)}, 1U);
  ...
  template <typename source_reader_t>
    requires(!std::same_as<std::remove_cvref_t<source_reader_t>, copy_stream_reader>) &&
            ...
            std::copy_constructible<
                std::remove_cvref_t<decltype(std::declval<reader_t &>().read())>>
  [[nodiscard]] static auto make_copies(source_reader_t &&reader, const std::size_t count)
      -> std::vector<copy_stream_reader> {
    auto readers =
        core_reader_t::make_readers(reader_t{std::forward<source_reader_t>(reader)}, count);
  ...
  [[nodiscard]] auto try_read_impl() -> stream_try_result<chunk_type> { return reader_.try_read(); }

  [[nodiscard]] auto read_impl() -> stream_result<chunk_type> { return reader_.read(); }
  ...
  [[nodiscard]] auto is_closed_impl() const noexcept -> bool { return reader_.is_closed(); }
...
template <stream_reader reader_t>
copy_stream_reader(reader_t &&) -> copy_stream_reader<::wh::core::remove_cvref_t<reader_t>>;

template <stream_reader reader_t>
[[nodiscard]] inline auto make_copy_stream_readers(reader_t &&reader, const std::size_t count)
    -> std::vector<copy_stream_reader<::wh::core::remove_cvref_t<reader_t>>>
  requires std::copy_constructible<std::remove_cvref_t<decltype(std::declval<reader_t &>().read())>>
{
