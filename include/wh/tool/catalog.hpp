...
public:
  ...
  auto set_source(const tool_catalog_source &source)
      -> void {
    source_ = source;
    cached_.clear();
    cache_valid_ = false;
  ...
  [[nodiscard]] auto load(
      ...
      const -> wh::core::result<std::span<const wh::schema::tool_schema_definition>> {
    if (!static_cast<bool>(source_.fetch_catalog)) {
      return wh::core::result<
          std::span<const wh::schema::tool_schema_definition>>::failure(
