...
[[nodiscard]] inline auto append_path_index(const std::string &path,
...
validate_value_against_schema(const wh::core::json_value &value,
                              const wh::schema::tool_parameter_schema &schema,
                              const std::string &path, std::string &error_path,
                              const bool capture_error = true) -> bool {
  ...
  if (schema.type == wh::schema::tool_parameter_type::string &&
      !schema.enum_values.empty()) {
    const std::string_view current{
        value.GetString(), static_cast<std::size_t>(value.GetStringLength())};
    if (std::ranges::find(schema.enum_values, current) ==
  ...
  if (!schema.one_of.empty()) {
    ...
    for (const auto &candidate : schema.one_of) {
      ...
      if (validate_value_against_schema(value, candidate, path, candidate_error,
                                        false)) {
        matched = true;
        break;
      }
  ...
  if (schema.type == wh::schema::tool_parameter_type::object) {
    for (const auto &property : schema.properties) {
      ...
      const auto member = value.FindMember(rapidjson::StringRef(
          property.name.data(),
          static_cast<rapidjson::SizeType>(property.name.size())));
      ...
      if (!validate_value_against_schema(member->value, property, child_path,
                                         error_path, capture_error)) {
        return false;
      }
    }
  }

  if (schema.type == wh::schema::tool_parameter_type::array &&
      !schema.item_types.empty()) {
    for (std::size_t index = 0U; index < value.Size(); ++index) {
      ...
      if (schema.item_types.size() == 1U) {
        if (!validate_value_against_schema(item, schema.item_types.front(),
      ...
      for (const auto &candidate : schema.item_types) {
        std::string candidate_error{};
        if (validate_value_against_schema(item, candidate, item_path,
...
[[nodiscard]] inline auto validate_tool_input_schema(
  ...
  for (const auto &parameter : parameters) {
    ...
    const auto member = parsed.value().FindMember(rapidjson::StringRef(
        parameter.name.data(),
        static_cast<rapidjson::SizeType>(parameter.name.size())));
    ...
    if (!validate_value_against_schema(member->value, parameter, param_path,
                                       error_path)) {
...
[[nodiscard]] inline auto merge_options(const tool_options &base,
...
inline auto mark_error(callback_state &state, const wh::core::error_code error,
  ...
  state.event.interrupted = error == wh::core::errc::canceled ||
...
[[nodiscard]] inline auto
resolve_options_view(const tool_common_options &options) noexcept
...
concept sync_invoke_handler =
    requires(const impl_t &impl, const tool_request &request) {
...
template <typename impl_t>
concept sender_invoke_handler_const =
    requires(const impl_t &impl, const tool_request &request) {
      { impl.invoke_sender(request) } -> stdexec::sender;
    };

template <typename impl_t>
concept sender_invoke_handler_move =
    requires(const impl_t &impl, tool_request &&request) {
      { impl.invoke_sender(std::move(request)) } -> stdexec::sender;
    };

template <typename impl_t>
concept sender_stream_handler_const =
    requires(const impl_t &impl, const tool_request &request) {
      { impl.stream_sender(request) } -> stdexec::sender;
    };

template <typename impl_t>
concept sender_stream_handler_move =
    requires(const impl_t &impl, tool_request &&request) {
...
template <typename impl_t>
concept invoke_capable =
    async_invoke_capable<impl_t> || sync_invoke_handler<impl_t>;

template <typename impl_t>
concept stream_capable =
    async_stream_capable<impl_t> || sync_stream_handler<impl_t>;

template <typename impl_t>
concept tool_impl = invoke_capable<impl_t> || stream_capable<impl_t>;

template <typename output_t>
[[nodiscard]] inline auto normalize_invoke_output(output_t &&output)
    -> tool_invoke_result {
...
template <typename impl_t>
[[nodiscard]] inline auto run_sync_invoke(const impl_t &impl,
                                          const tool_request &request)
    -> tool_invoke_result {
...
[[nodiscard]] inline auto make_invoke_sender(const impl_t &impl,
                                             request_t &&request) {
...
[[nodiscard]] inline auto make_stream_sender(const impl_t &impl,
                                             request_t &&request) {
  using request_value_t = std::remove_cvref_t<request_t>;
  static_assert(std::same_as<request_value_t, tool_request>,
                "tool sender factory requires tool_request input");
...
template <> struct tool_result_traits<tool_invoke_result> {
  static auto record_success(callback_state &state, tool_invoke_result &status)
      -> void {
    state.event.output_text = status.value();
  }

  [[nodiscard]] static auto make_skip_result() -> tool_invoke_result {
    return std::string{};
  }
};
...
[[nodiscard]] inline auto
prepare_tool_run(tool_request request, callback_sink sink,
  ...
  state.max_attempts =
      state.resolved_options.failure_policy == tool_failure_policy::retry
  ...
  if (validated.has_error()) {
    ...
    return wh::core::result<tool_run_state<result_t>>::failure(
        validated.error());
  }
  return state;
}
...
template <typename result_t>
[[nodiscard]] inline auto
consume_tool_attempt_result(tool_run_state<result_t> &state, result_t status)
    -> bool {
  if (status.has_value()) {
  ...
  mark_error(state.callback, state.last_error,
             resolve_options_view(state.resolved_options));
  emit_callback(state.sink, wh::callbacks::stage::error, state.callback);
...
template <typename result_t>
[[nodiscard]] inline auto finish_tool_run(tool_run_state<result_t> state)
    -> result_t {
  ...
  if (state.resolved_options.failure_policy == tool_failure_policy::skip) {
    ...
    if (skipped.has_error()) {
      state.callback.event.error_context = std::string{
          resolve_options_view(state.resolved_options).timeout_label};
      emit_callback(state.sink, wh::callbacks::stage::error, state.callback);
      return skipped;
    }
    emit_callback(state.sink, wh::callbacks::stage::end, state.callback);
...
[[nodiscard]] inline auto run_sync_tool_loop(tool_run_state<result_t> state,
                                             run_attempt_t &&run_attempt)
    -> result_t {
  while (state.next_attempt < state.max_attempts) {
    ++state.next_attempt;
    if (consume_tool_attempt_result(
            state, std::invoke(std::forward<run_attempt_t>(run_attempt),
                               state.request))) {
      break;
    }
  }
...
class tool_attempt_loop_sender {
  template <typename receiver_t> class operation {
    ...
    using child_sender_t =
        std::remove_cvref_t<std::invoke_result_t<make_attempt_t &, state_t &>>;
    using child_op_t =
        stdexec::connect_result_t<child_sender_t, child_receiver>;

  public:
    template <typename stored_state_t, typename stored_make_attempt_t>
      requires std::constructible_from<wh::core::result<state_t>,
                                       stored_state_t &&> &&
                   std::constructible_from<make_attempt_t,
                                           stored_make_attempt_t &&>
...
public:
  ...
  STDEXEC_EXPLICIT_THIS_BEGIN(auto connect)(this self_t &&self,
    ...
    return operation<receiver_t>{std::forward<self_t>(self).state_,
                                 std::forward<self_t>(self).make_attempt_,
                                 std::move(receiver)};
  }
...
template <typename result_t, typename state_t, typename make_attempt_t>
[[nodiscard]] inline auto
make_tool_attempt_loop_sender(wh::core::result<state_t> state,
                              make_attempt_t &&make_attempt)
    -> tool_attempt_loop_sender<result_t, state_t,
                                std::remove_cvref_t<make_attempt_t>> {
  using sender_t =
      tool_attempt_loop_sender<result_t, state_t,
                               std::remove_cvref_t<make_attempt_t>>;
...
template <detail::tool_impl impl_t,
          wh::core::resume_mode Resume = wh::core::resume_mode::unchanged>
class tool {
public:
  explicit tool(const wh::schema::tool_schema_definition &schema,
                const impl_t &impl,
  ...
  [[nodiscard]] auto descriptor() const -> wh::core::component_descriptor {
    return wh::core::component_descriptor{
        wh::tool::utils::to_camel_case(schema_.name),
  ...
  [[nodiscard]] auto invoke(request_t &&request,
...
private:
  ...
  [[nodiscard]] auto invoke_sync_impl(request_t &&request,
          ...
          return detail::make_tool_attempt_loop_sender<tool_invoke_result>(
              std::move(prepared), [this, scheduler = std::move(scheduler)](
                ...
                return wh::core::detail::resume_if<Resume>(
                    detail::make_invoke_sender(impl_, loop_state.request),
                    scheduler);
              });
        });
  }
