...
using schema_modifier = wh::core::callback_function<void(
    wh::schema::tool_parameter_schema &, std::string_view) const>;
...
inline auto apply_modifier_recursive(wh::schema::tool_parameter_schema &schema,
  ...
  if (modifier) {
    modifier(schema,
             path.empty() ? std::string_view{"_root"} : std::string_view{path});
  }
...
[[nodiscard]] inline auto infer_schema(name_t &&name,
...
infer_schema(name_t &&name, description_t &&description,
