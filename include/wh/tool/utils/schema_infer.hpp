...
using schema_modifier =
    wh::core::callback_function<void(wh::schema::tool_parameter_schema &, std::string_view) const>;
...
inline auto apply_modifier_recursive(wh::schema::tool_parameter_schema &schema,
                                     const std::string &path, const schema_modifier &modifier)
    -> void {
  if (modifier) {
    modifier(schema, path.empty() ? std::string_view{"_root"} : std::string_view{path});
  }
...
template <typename name_t, typename description_t, typename parameters_t>
  requires std::constructible_from<std::string, name_t &&> &&
           std::constructible_from<std::string, description_t &&> &&
           std::constructible_from<std::vector<wh::schema::tool_parameter_schema>, parameters_t &&>
...
[[nodiscard]] inline auto infer_schema(name_t &&name, description_t &&description,
  ...
  schema.description = std::forward<description_t>(description);
  schema.parameters =
      std::vector<wh::schema::tool_parameter_schema>{std::forward<parameters_t>(parameters)};
  return schema;
}
...
[[nodiscard]] inline auto infer_schema(name_t &&name, description_t &&description,
                                       parameters_t &&parameters, const schema_modifier &modifier)
    -> wh::schema::tool_schema_definition {
  auto stored_parameters =
      std::vector<wh::schema::tool_parameter_schema>{std::forward<parameters_t>(parameters)};
  for (auto &parameter : stored_parameters) {
    detail::apply_modifier_recursive(parameter, parameter.name, modifier);
  }
  return infer_schema(std::forward<name_t>(name), std::forward<description_t>(description),
