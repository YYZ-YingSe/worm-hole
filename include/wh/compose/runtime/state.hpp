...
template <typename state_t> [[nodiscard]] inline auto state_type_name() -> std::string_view {
  return wh::core::any_info_v<state_t>.name;
}

template <typename state_t> [[nodiscard]] inline auto state_not_found_detail() -> std::string {
  return std::string{"state not found, expected="} + std::string{state_type_name<state_t>()};
}

template <typename state_t>
[[nodiscard]] inline auto state_type_mismatch_detail(const wh::core::any &value) -> std::string {
  return std::string{"state type mismatch, expected="} + std::string{state_type_name<state_t>()} +
...
public:
  ...
  [[nodiscard]] auto parent() const noexcept -> graph_process_state * { return parent_; }
  ...
  auto emplace(args_t &&...args) -> wh::core::result<std::reference_wrapper<state_t>> {
  ...
  [[nodiscard]] auto get() -> wh::core::result<std::reference_wrapper<state_t>> {
    {
      ...
      if (iter != values_.end()) {
        ...
        if (typed == nullptr) {
          last_error_detail_ = detail::state_type_mismatch_detail<state_t>(iter->second);
  ...
  template <typename state_t>
  [[nodiscard]] auto get() const -> wh::core::result<std::reference_wrapper<const state_t>> {
    {
      auto lock = std::scoped_lock{mutex_};
      auto iter = values_.find(wh::core::any_type_key_v<state_t>);
      if (iter != values_.end()) {
...
using graph_state_pre_handler = wh::core::callback_function<wh::core::result<void>(
    const graph_state_cause &, graph_process_state &, graph_value &, wh::core::run_context &)
                                                                const>;

/// Node-level state post-handler (value path).
using graph_state_post_handler = wh::core::callback_function<wh::core::result<void>(
    const graph_state_cause &, graph_process_state &, graph_value &, wh::core::run_context &)
                                                                 const>;
...
using graph_state_handler_registry =
    std::unordered_map<std::string, graph_node_state_handlers, wh::core::transparent_string_hash,
...
public:
  ...
  [[nodiscard]] auto by_key(const std::string_view key) const
    ...
    if (!node_id.has_value()) {
      return wh::core::result<graph_node_state>::failure(wh::core::errc::not_found);
    }
  ...
  auto update(const std::uint32_t node_id, const graph_node_lifecycle_state lifecycle,
              const std::size_t attempts = 0U,
              const std::optional<wh::core::error_code> last_error = std::nullopt)
      -> wh::core::result<void> {
    if (node_id >= states_.size()) {
  ...
  [[nodiscard]] auto states() const -> std::vector<graph_node_state> {
    ...
    for (std::uint32_t node_id = 0U; node_id < static_cast<std::uint32_t>(states_.size());
         ++node_id) {
  ...
  [[nodiscard]] auto collect_keys(const graph_node_lifecycle_state lifecycle) const
  ...
  [[nodiscard]] auto collect_completed_keys() const -> std::vector<std::string> {
...
private:
  ...
  [[nodiscard]] auto find_node_id(const std::string_view key) const noexcept
    ...
    for (std::uint32_t node_id = 0U; node_id < static_cast<std::uint32_t>(keys_.size());
         ++node_id) {
  ...
  [[nodiscard]] auto snapshot(const std::uint32_t node_id) const -> graph_node_state {
