...
inline auto passthrough_node::compile() const & -> wh::core::result<compiled_node> {
  auto copied = *this;
...
inline auto passthrough_node::compile() && -> wh::core::result<compiled_node> {
  return make_compiled_sync_node(
      ...
      descriptor_.input_contract, descriptor_.output_contract, std::move(descriptor_.key),
      [](graph_value &input, wh::core::run_context &,
         const node_runtime &) -> wh::core::result<graph_value> { return std::move(input); },
      std::move(options_));
}
...
[[nodiscard]] inline auto make_passthrough_node(key_t &&key) -> passthrough_node {
  auto descriptor = node_descriptor{
      ...
      .input_gate_info =
          Contract == node_contract::value ? input_gate::open() : input_gate::reader(),
      .output_gate_info =
          Contract == node_contract::value ? output_gate::passthrough() : output_gate::reader(),
  };
