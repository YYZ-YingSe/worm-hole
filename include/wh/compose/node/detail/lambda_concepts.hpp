...
concept value_lambda =
    requires(lambda_t lambda, graph_value &input,
...
template <typename lambda_t>
concept value_sender =
    requires(lambda_t lambda, graph_value &input,
             wh::core::run_context &context,
             const graph_call_scope &call_options) {
      requires result_typed_sender<
          decltype(lambda(input, context, call_options)),
...
template <typename lambda_t>
concept map_lambda =
    requires(lambda_t lambda, graph_value_map &input,
             wh::core::run_context &context,
             const graph_call_scope &call_options) {
      { lambda(input, context, call_options) }
...
concept value_stream_lambda =
    requires(lambda_t lambda, graph_value &input,
             wh::core::run_context &context,
             const graph_call_scope &call_options) {
      requires detail::canonical_stream_status<
          decltype(lambda(input, context, call_options))>;
    };
...
concept value_stream_sender =
    requires(lambda_t lambda, graph_value &input,
             wh::core::run_context &context,
             const graph_call_scope &call_options) {
      requires detail::canonical_stream_sender<
          decltype(lambda(input, context, call_options))>;
    };
...
concept stream_value_sender =
    requires(lambda_t lambda, graph_stream_reader input,
             ...
             const graph_call_scope &call_options) {
      requires result_typed_sender<
          decltype(lambda(std::move(input), context, call_options)),
          wh::core::result<graph_value>>;
    };
...
template <node_exec_mode Exec, node_contract From, node_contract To,
          typename lambda_t>
concept lambda_contract =
    lambda_contract_supported_v<Exec, From, To, lambda_t>;
