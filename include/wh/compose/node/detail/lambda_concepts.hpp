...
concept value_lambda = requires(lambda_t lambda, graph_value &input, wh::core::run_context &context,
                                const graph_call_scope &call_options) {
  { lambda(input, context, call_options) } -> std::same_as<wh::core::result<graph_value>>;
};

/// Lambda concept for `value -> value` sender-style nodes.
template <typename lambda_t>
concept value_sender = requires(lambda_t lambda, graph_value &input, wh::core::run_context &context,
                                const graph_call_scope &call_options) {
...
template <typename lambda_t>
concept map_lambda =
    requires(lambda_t lambda, graph_value_map &input, wh::core::run_context &context,
             const graph_call_scope &call_options) {
      { lambda(input, context, call_options) } -> std::same_as<wh::core::result<graph_value_map>>;
    };
...
template <typename lambda_t>
concept value_stream_lambda =
    requires(lambda_t lambda, graph_value &input, wh::core::run_context &context,
             const graph_call_scope &call_options) {
      requires detail::canonical_stream_status<decltype(lambda(input, context, call_options))>;
    };

/// Lambda concept for `value -> stream` sender-style nodes.
template <typename lambda_t>
concept value_stream_sender =
    requires(lambda_t lambda, graph_value &input, wh::core::run_context &context,
             const graph_call_scope &call_options) {
      requires detail::canonical_stream_sender<decltype(lambda(input, context, call_options))>;
    };

/// Lambda concept for `stream -> value` function-style nodes.
template <typename lambda_t>
concept stream_value_lambda =
    requires(lambda_t lambda, graph_stream_reader input, wh::core::run_context &context,
             const graph_call_scope &call_options) {
      {
...
concept stream_value_sender =
    requires(lambda_t lambda, graph_stream_reader input, wh::core::run_context &context,
             const graph_call_scope &call_options) {
      requires result_typed_sender<decltype(lambda(std::move(input), context, call_options)),
                                   wh::core::result<graph_value>>;
    };
...
concept stream_stream_lambda =
    requires(lambda_t lambda, graph_stream_reader input, wh::core::run_context &context,
             const graph_call_scope &call_options) {
      requires detail::canonical_stream_status<decltype(lambda(std::move(input), context,
                                                               call_options))>;
    };
...
concept stream_stream_sender =
    requires(lambda_t lambda, graph_stream_reader input, wh::core::run_context &context,
             const graph_call_scope &call_options) {
      requires detail::canonical_stream_sender<decltype(lambda(std::move(input), context,
                                                               call_options))>;
    };
