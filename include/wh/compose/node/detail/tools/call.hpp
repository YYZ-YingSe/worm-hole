...
using call_completion_sender = wh::core::detail::result_sender<wh::core::result<call_completion>>;
...
[[nodiscard]] inline auto erase_tools_invoke(sender_t &&sender) -> tools_invoke_sender {
  return tools_invoke_sender{std::forward<sender_t>(sender)};
}

[[nodiscard]] inline auto erase_tools_invoke(tools_invoke_sender sender) -> tools_invoke_sender {
  return sender;
}
...
[[nodiscard]] inline auto erase_tools_stream(sender_t &&sender) -> tools_stream_sender {
  return tools_stream_sender{std::forward<sender_t>(sender)};
}

[[nodiscard]] inline auto erase_tools_stream(tools_stream_sender sender) -> tools_stream_sender {
  return sender;
}
...
template <typename sender_t, typename result_t>
[[nodiscard]] inline auto failure_sender(const wh::core::error_code error) -> sender_t {
...
[[nodiscard]] inline auto erase_call_completion(sender_t &&sender) -> call_completion_sender {
...
[[nodiscard]] inline auto erase_stream_completion(sender_t &&sender) -> stream_completion_sender {
...
[[nodiscard]] inline auto make_stream_completion(
    const std::size_t index, tool_call call, graph_stream_reader stream, graph_value rerun_extra,
    std::optional<wh::core::run_context> after_context = std::nullopt) -> stream_completion {
...
[[nodiscard]] inline auto call_value(const tools_state &state, const tool_call &call,
                                     wh::tool::call_scope scope) -> wh::core::result<graph_value> {
  const auto *tool = find_tool(state, call.tool_name);
  if (tool == nullptr || !static_cast<bool>(tool->invoke)) {
    return wh::core::result<graph_value>::failure(tool == nullptr ? wh::core::errc::not_found
                                                                  : wh::core::errc::not_supported);
  }
  return tool->invoke(call, std::move(scope));
}

[[nodiscard]] inline auto call_stream(const tools_state &state, const tool_call &call,
                                      wh::tool::call_scope scope)
    -> wh::core::result<graph_stream_reader> {
  const auto *tool = find_tool(state, call.tool_name);
  if (tool == nullptr || !static_cast<bool>(tool->stream)) {
    return wh::core::result<graph_stream_reader>::failure(
        tool == nullptr ? wh::core::errc::not_found : wh::core::errc::not_supported);
  }
  return tool->stream(call, std::move(scope));
}

[[nodiscard]] inline auto start_value(const tools_state &state, tool_call call,
                                      wh::tool::call_scope scope) -> tools_invoke_sender {
  const auto *tool = find_tool(state, call.tool_name);
...
[[nodiscard]] inline auto start_stream(const tools_state &state, tool_call call,
                                       wh::tool::call_scope scope) -> tools_stream_sender {
  const auto *tool = find_tool(state, call.tool_name);
  if (tool == nullptr || !static_cast<bool>(tool->async_stream)) {
    return failure_sender<tools_stream_sender, wh::core::result<graph_stream_reader>>(
        tool == nullptr ? wh::core::errc::not_found : wh::core::errc::not_supported);
  }
  return erase_tools_stream(tool->async_stream(std::move(call), std::move(scope)));
}

[[nodiscard]] inline auto run_before(const tools_options &options, tool_call &call,
...
[[nodiscard]] inline auto run_call(const tools_state &state, const std::size_t index,
...
[[nodiscard]] inline auto run_stream_call(const tools_state &state, const std::size_t index,
                                          wh::core::run_context &context)
  ...
  if (!rerun.ids.empty() && !rerun.ids.contains(call.call_id)) {
    ...
    if (!has_tool_afters(state.afters)) {
      return make_stream_completion(index, std::move(call), std::move(replayed).value(),
  ...
  if (!has_tool_afters(state.afters)) {
    return make_stream_completion(index, std::move(call), std::move(streamed).value(),
                                  wh::core::any(std::string{plan.call->arguments}));
  }
...
[[nodiscard]] inline auto start_call(const tools_state &state, const std::size_t index,
  ...
  if (!rerun.ids.empty() && !rerun.ids.contains(call.call_id)) {
    const auto cached = rerun.outputs.find(call.call_id);
    if (cached == rerun.outputs.end()) {
      return failure_sender<call_completion_sender, wh::core::result<call_completion>>(
    ...
    return erase_call_completion(
        ...
        stdexec::then(
            [call = std::move(call), index,
             rerun_extra = wh::core::any(std::string{plan.call->arguments})](
                wh::core::result<graph_value> status) mutable -> wh::core::result<call_completion> {
              if (status.has_error()) {
  ...
  auto call_context_ptr = std::make_unique<wh::core::run_context>(std::move(call_context));
  return erase_call_completion(
      std::move(invoke) |
      stdexec::then(
...
[[nodiscard]] inline auto start_stream_call(const tools_state &state, const std::size_t index,
                                            wh::core::run_context &base_context)
    -> stream_completion_sender {
  const auto &plan = state.plans[index];
  auto call = make_call(plan);
  ...
  if (!rerun.ids.empty() && !rerun.ids.contains(call.call_id)) {
    const auto cached = rerun.outputs.find(call.call_id);
    if (cached == rerun.outputs.end()) {
      return failure_sender<stream_completion_sender, wh::core::result<stream_completion>>(
          wh::core::errc::not_found);
    }
    auto replayed = replay_stream(cached->second);
    if (replayed.has_error()) {
    ...
    if (!has_tool_afters(state.afters)) {
      return ready_sender<stream_completion_sender>(wh::core::result<stream_completion>{
          make_stream_completion(index, std::move(call), std::move(replayed).value(),
                                 wh::core::any(std::string{plan.call->arguments}))});
    }
    auto cloned_context = clone_call_context(base_context);
    if (cloned_context.has_error()) {
  ...
  if (!has_tool_afters(state.afters)) {
    return erase_stream_completion(
        ...
        stdexec::then([call = std::move(call), index,
          ...
          return make_stream_completion(index, std::move(call), std::move(status).value(),
  ...
  return erase_stream_completion(
      std::move(stream) |
      stdexec::then([call = std::move(call), index, call_context = std::move(call_context_ptr),
                     rerun_extra = wh::core::any(std::string{plan.call->arguments})](
                        wh::core::result<graph_stream_reader> status) mutable
                        -> wh::core::result<stream_completion> {
        if (status.has_error()) {
        ...
        return make_stream_completion(index, std::move(call), std::move(status).value(),
                                      std::move(rerun_extra), std::move(*call_context));
      }));
}
