...
[[nodiscard]] inline auto run_tools_sync(const graph_value &input, wh::core::run_context &context,
                                         const node_runtime &runtime, const tool_registry &tools,
                                         const tools_options &options)
    -> wh::core::result<graph_value> {
  return with_node_context(
      context, runtime, [&](wh::core::run_context &node_context) -> wh::core::result<graph_value> {
        auto resolved = make_sync_tools_state(input, tools, options, node_context, runtime);
...
template <node_contract To>
[[nodiscard]] inline auto run_tools_async(const graph_value &input, wh::core::run_context &context,
                                          const node_runtime &runtime, const tool_registry &tools,
                                          const tools_options &options) -> graph_sender {
  auto resolved = make_async_tools_state(input, tools, options, context, runtime);
  if (resolved.has_error()) {
    return ready_graph_sender(wh::core::result<graph_value>::failure(resolved.error()));
  }
  auto state = std::move(resolved).value();
  const auto max_in_flight =
      state.sequential
          ? 1U
          : resolve_parallel_call_budget(state.execute_indices.size(), runtime.parallel_gate());
  ...
  if constexpr (To == node_contract::value) {
    ...
    auto sender =
        wh::core::detail::make_concurrent_sender_vector<wh::core::result<call_completion>>(
            std::move(senders), max_in_flight) |
    ...
    auto sender =
        wh::core::detail::make_concurrent_sender_vector<wh::core::result<stream_completion>>(
            std::move(senders), max_in_flight) |
        stdexec::then([state = std::move(state)](
                          std::vector<wh::core::result<stream_completion>> results) mutable
                          -> wh::core::result<graph_value> {
