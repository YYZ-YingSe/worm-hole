...
[[nodiscard]] inline auto run_tools_sync(const graph_value &input,
  ...
  return with_node_context(
      context, runtime,
      [&](wh::core::run_context &node_context) -> wh::core::result<graph_value> {
        auto resolved =
...
template <node_contract To>
[[nodiscard]] inline auto run_tools_async(const graph_value &input,
                                          wh::core::run_context &context,
                                          const node_runtime &runtime,
                                          const tool_registry &tools,
                                          const tools_options &options)
  ...
  if (resolved.has_error()) {
    return ready_graph_sender(wh::core::result<graph_value>::failure(
        resolved.error()));
  }
  auto state = std::move(resolved).value();
  ...
  if constexpr (To == node_contract::value) {
    ...
    auto sender = wh::core::detail::make_concurrent_sender_vector<
                      wh::core::result<call_completion>>(std::move(senders),
                                                         max_in_flight) |
                  stdexec::then([state = std::move(state)](
                                    std::vector<wh::core::result<call_completion>>
                                        results) mutable
                                    -> wh::core::result<graph_value> {
