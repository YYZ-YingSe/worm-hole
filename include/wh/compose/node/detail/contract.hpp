...
[[nodiscard]] consteval auto canonical_stream_sender_signature_ok() -> bool {
  ...
  (([&] {
    if constexpr (set_value_signature<signatures_t>::value) {
      ...
      value_ok =
          value_ok &&
          canonical_stream_status<typename set_value_signature<
              signatures_t>::result_type>;
    }
  }()),
   ...);
...
template <typename signatures_t> struct canonical_stream_sender_signature
    : std::false_type {};
