     ...
     if constexpr (set_value_signature<signatures_t>::value) {
       ...
       value_ok = value_ok &&
                  canonical_stream_status<
                      typename set_value_signature<signatures_t>::result_type>;
     }
   }()),
   ...);
...
template <typename signatures_t>
struct canonical_stream_sender_signature : std::false_type {};

template <typename... signatures_t>
struct canonical_stream_sender_signature<
    stdexec::completion_signatures<signatures_t...>>
    : std::bool_constant<
          canonical_stream_sender_signature_ok<signatures_t...>()> {};

template <typename sender_t>
concept canonical_stream_sender =
    stdexec::sender<std::remove_cvref_t<sender_t>> &&
    canonical_stream_sender_signature<stdexec::completion_signatures_of_t<
        std::remove_cvref_t<sender_t>, stdexec::env<>>>::value;

template <node_contract From, typename request_t>
concept typed_request =
    (From == node_contract::value &&
     !std::same_as<std::remove_cvref_t<request_t>, graph_stream_reader>) ||
    (From == node_contract::stream &&
     std::same_as<std::remove_cvref_t<request_t>, graph_stream_reader>);

template <typename response_t>
concept exact_value_payload =
    std::copy_constructible<std::remove_cvref_t<response_t>> &&
...
concept typed_response =
    (To == node_contract::value &&
     exact_value_payload<std::remove_cvref_t<response_t>>) ||
