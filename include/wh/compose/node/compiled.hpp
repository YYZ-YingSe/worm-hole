...
compiled_node_is_sync(const compiled_node &node) noexcept -> bool {
  return std::holds_alternative<compiled_sync_program>(node.program);
}

[[nodiscard]] inline auto
compiled_node_is_async(const compiled_node &node) noexcept -> bool {
...
[[nodiscard]] inline auto
run_compiled_async_node(const compiled_node &node, graph_value &input,
                        wh::core::run_context &context,
                        const node_runtime &runtime) -> graph_sender {
  if (const auto *program =
          std::get_if<compiled_async_program>(&node.program)) {
    return (*program)(input, context, runtime);
  }
...
[[nodiscard]] inline auto make_compiled_sync_node(
  ...
  return compiled_node{
      ...
      .program = compiled_sync_program{detail::bind_node_sync_factory(
          std::forward<run_t>(run))},
  };
}
...
[[nodiscard]] inline auto make_compiled_async_node(
  ...
  return compiled_node{
      ...
      .program = compiled_async_program{detail::bind_node_async_factory(
          std::forward<run_t>(run))},
  };
}
