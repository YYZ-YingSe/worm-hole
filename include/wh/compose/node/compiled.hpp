...
struct compiled_sync_program {
  ...
  [[nodiscard]] auto operator()(graph_value &input,
...
using compiled_node_program =
    std::variant<compiled_sync_program, compiled_async_program>;
...
[[nodiscard]] inline auto compiled_node_is_sync(
    const compiled_node &node) noexcept -> bool {
  return std::holds_alternative<compiled_sync_program>(node.program);
}

[[nodiscard]] inline auto compiled_node_is_async(
    const compiled_node &node) noexcept -> bool {
  return std::holds_alternative<compiled_async_program>(node.program);
}

[[nodiscard]] inline auto run_compiled_sync_node(const compiled_node &node,
                                                 graph_value &input,
                                                 wh::core::run_context &context,
...
[[nodiscard]] inline auto run_compiled_async_node(const compiled_node &node,
                                                  graph_value &input,
                                                  wh::core::run_context &context,
                                                  const node_runtime &runtime)
    -> graph_sender {
...
template <typename key_t, typename run_t,
...
make_compiled_sync_node(const node_kind kind,
                        const node_exec_origin exec_origin,
                        const node_contract input_contract,
                        const node_contract output_contract, key_t &&key,
                        run_t &&run, options_t &&options = {}) -> compiled_node {
  return compiled_node{
      .meta =
          compiled_node_meta{
              ...
              .options =
                  graph_add_node_options{std::forward<options_t>(options)},
          },
      .program = compiled_sync_program{
          detail::bind_node_sync_factory(std::forward<run_t>(run))},
  };
}
...
template <typename key_t, typename run_t,
  ...
  requires std::constructible_from<std::string, key_t &&> &&
           std::constructible_from<graph_add_node_options, options_t &&> &&
           (node_async_run<std::remove_cvref_t<run_t>> ||
            graph_result_sender<run_t>)
[[nodiscard]] inline auto make_compiled_async_node(
    const node_kind kind, const node_exec_origin exec_origin,
  ...
  return compiled_node{
      ...
      .program = compiled_async_program{
          detail::bind_node_async_factory(std::forward<run_t>(run))},
  };
}
