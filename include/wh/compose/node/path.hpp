...
[[nodiscard]] inline auto make_node_path(std::initializer_list<std::string_view> segments)
    -> node_path {
  return wh::core::make_address(segments);
}

/// Builds one node path from one pre-materialized segment span.
[[nodiscard]] inline auto make_node_path(const std::span<const std::string_view> segments)
    -> node_path {
  return wh::core::make_address(segments);
}
