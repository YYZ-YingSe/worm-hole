...
decorate_lambda_options(graph_add_node_options options, const node_contract)
    -> graph_add_node_options {
  auto type = std::string{
      wh::internal::diagnostic_type_alias<std::remove_cvref_t<lambda_t>>()};
...
[[nodiscard]] inline auto read_lambda_input(graph_value &input) {
...
[[nodiscard]] inline auto map_lambda_sender(sender_t &&sender,
                                            mapper_t &&mapper) {
...
[[nodiscard]] inline auto make_lambda_descriptor(std::string key,
...
template <node_exec_mode Exec, node_contract From, node_contract To,
          typename key_t, typename run_t>
[[nodiscard]] inline auto
make_lambda_compiled_node(key_t &&key, run_t &&run,
                          graph_add_node_options options) -> compiled_node {
  if constexpr (Exec == node_exec_mode::sync) {
    return make_compiled_sync_node(
        node_kind::lambda, default_exec_origin(node_kind::lambda), From, To,
        std::forward<key_t>(key), std::forward<run_t>(run), std::move(options));
  } else {
...
template <node_exec_mode Exec, node_contract From, node_contract To,
          typename lambda_t, typename run_factory_t>
[[nodiscard]] inline auto
make_lambda_payload_from_runner(lambda_t &&lambda, run_factory_t &&make_run)
    -> lambda_payload {
  using stored_lambda_t = std::remove_cvref_t<lambda_t>;
...
template <node_exec_mode Exec, node_contract From, node_contract To,
          typename lambda_t>
[[nodiscard]] inline auto make_lambda_payload(lambda_t &&lambda)
    -> lambda_payload {
  using stored_lambda_t = std::remove_cvref_t<lambda_t>;
  ...
  if constexpr (From == node_contract::value && To == node_contract::value) {
                  ...
                  [&lambda](graph_value &owned_input,
                    ...
                    return bridge_graph_sender(map_lambda_sender(
                        lambda(typed_input.value().get(), callback_context,
                               call_options),
                        [](graph_value_map map) {
                          return wh::core::any(std::move(map));
                        }));
                  });
            };
          });
            ...
            return [lambda = std::move(lambda)](
              ...
              return invoke_lambda_result(
                  ...
                  [&](wh::core::run_context &callback_context,
                      const graph_call_scope &call_options)
                      -> wh::core::result<graph_value> {
                    return lambda(std::move(stream_input).value(),
            ...
            return [lambda = std::move(lambda)](
              ...
              return invoke_lambda_result(
                  ...
                  [&](wh::core::run_context &callback_context,
                      ...
                      -> wh::core::result<graph_value> {
                    return wrap_lambda_stream_payload(
                        lambda(std::move(stream_input).value(),
                               callback_context, call_options));
                  });
            };
          });
            ...
            return [lambda = std::move(lambda)](
              ...
              return bind_reader_sender(
                  ...
                  [&lambda](graph_stream_reader owned_input,
                            ...
                            const graph_call_scope &call_options) {
                    return map_lambda_sender(
                        lambda(std::move(owned_input), callback_context,
                               call_options),
                        [](auto reader) -> wh::core::result<graph_value> {
                          auto canonical =
                              to_graph_stream_reader(std::move(reader));
    ...
    static_assert(lambda_always_false_v<stored_lambda_t>,
                  "unsupported lambda contract pair");
  }
}
...
[[nodiscard]] inline auto make_lambda_node(key_t &&key, lambda_t &&lambda,
                                           options_t &&options = {})
    -> lambda_node {
