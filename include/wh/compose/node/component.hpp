...
[[nodiscard]] inline auto decorate_component_options(key_t &&key,
  ...
  return decorate_named_node_options(std::forward<key_t>(key),
...
template <typename request_t>
struct component_request_state {
...
[[nodiscard]] inline auto request_matches_runtime(
    const request_t &request, const node_runtime &runtime) -> bool {
...
bind_component_result(const graph_value &input, const node_runtime &runtime,
  ...
  auto &component_context =
      callback_context.has_value() ? *callback_context : empty_context;
  return std::move(request).value().apply([&](auto &&resolved_request) {
    return to_graph_output(std::invoke(
        std::forward<invoke_t>(invoke),
        std::forward<decltype(resolved_request)>(resolved_request),
        component_context));
  });
}
...
template <typename request_t, typename invoke_t>
[[nodiscard]] inline auto bind_component_stream_result(
    const graph_value &input, const node_runtime &runtime,
    wh::core::run_context &context, invoke_t &&invoke)
    -> wh::core::result<graph_value> {
  auto request = resolve_component_request<request_t>(input, runtime);
  if (request.has_error()) {
  ...
  auto &component_context =
      callback_context.has_value() ? *callback_context : empty_context;
  return std::move(request).value().apply([&](auto &&resolved_request) {
    auto output = std::invoke(std::forward<invoke_t>(invoke),
                              std::forward<decltype(resolved_request)>(
                                  resolved_request),
                              component_context);
    if (output.has_error()) {
...
concept async_invoke_result_sender =
    requires(const component_t &value,
      ...
      requires result_typed_sender<
          decltype(value.async_invoke(std::declval<request_t &&>(),
                                      callback_context)),
          result_t>;
    };
...
concept async_stream_result_sender =
    requires(const component_t &value,
             wh::core::run_context &callback_context) {
      requires result_typed_sender<
          decltype(value.async_stream(std::declval<request_t &&>(),
                                      callback_context)),
          result_t>;
    };
...
concept async_render_result_sender =
    requires(const component_t &value,
             wh::core::run_context &callback_context) {
      requires result_typed_sender<
          decltype(value.async_render(std::declval<request_t &&>(),
                                      callback_context)),
          result_t>;
    };
...
concept async_embed_result_sender =
    requires(const component_t &value,
             wh::core::run_context &callback_context) {
      requires result_typed_sender<
          decltype(value.async_embed(std::declval<request_t &&>(),
                                     callback_context)),
          result_t>;
    };
...
concept async_retrieve_result_sender =
    requires(const component_t &value,
             wh::core::run_context &callback_context) {
      requires result_typed_sender<
          decltype(value.async_retrieve(std::declval<request_t &&>(),
                                        callback_context)),
          result_t>;
    };
...
concept async_write_result_sender =
    requires(const component_t &value,
             wh::core::run_context &callback_context) {
      requires result_typed_sender<
          decltype(value.async_write(std::declval<request_t &&>(),
                                     callback_context)),
          result_t>;
    };
...
concept async_process_result_sender =
    requires(const component_t &value,
             wh::core::run_context &callback_context) {
      requires result_typed_sender<
          decltype(value.async_process(std::declval<request_t &&>(),
                                       callback_context)),
          result_t>;
    };
...
struct component_contract<component_kind::tool, node_contract::value,
                          node_contract::stream, component_t> {
  using request_type = explicit_component_request_t<component_kind::tool,
                                                    node_contract::stream>;
...
template <typename component_t>
struct component_async_contract<component_kind::retriever,
                                node_contract::value, node_contract::value,
                                component_t> {
  using request_type = explicit_component_request_t<component_kind::retriever,
                                                    node_contract::value>;
...
template <typename component_t>
struct component_async_contract<component_kind::document,
                                node_contract::value, node_contract::value,
                                component_t> {
  using request_type = explicit_component_request_t<component_kind::document,
                                                    node_contract::value>;
...
inline constexpr bool explicit_component_async_method_supported_v = [] {
  if constexpr (Kind == component_kind::model &&
          ...
          return with_node_context(
              ...
              [&](wh::core::run_context &component_context)
                ...
                if constexpr (requires(const component_t &value,
                                       wh::core::run_context &callback_context) {
                                {
                                  value.invoke(
                                      std::forward<decltype(resolved_request)>(
                                          resolved_request),
                                      callback_context)
                                } -> std::same_as<wh::core::result<response_t>>;
                              }) {
                  ...
                  static_assert(
                      always_false_v<component_t>,
                      "custom component requires invoke(request, run_context) "
                      "returning wh::core::result<response_t> when To == value");
                }
              });
        });
  }
          ...
          return with_node_context(
              ...
              [&](wh::core::run_context &component_context)
                ...
                if constexpr (requires(const component_t &value,
                  ...
                  static_assert(
                      always_false_v<component_t>,
                      "custom component requires stream(request, run_context) "
                      "returning wh::core::result<response_t> when To == stream");
                }
              });
        });
  }
...
[[nodiscard]] inline auto bind_explicit_custom_component(
  ...
  return custom_component_contract<To, component_t, request_t,
                                   response_t>::run(component, input, context,
                                                    runtime);
}

template <typename request_t, typename response_t, typename sender_factory_t>
[[nodiscard]] inline auto
bind_custom_component_sender(graph_value &input,
                             const node_runtime &runtime,
                             wh::core::run_context &context,
                             sender_factory_t &&make_sender) -> graph_sender {
  auto request = resolve_component_request<request_t>(input, runtime);
...
template <typename request_t, typename response_t, typename sender_factory_t>
[[nodiscard]] inline auto
bind_custom_component_stream_sender(graph_value &input,
                                    const node_runtime &runtime,
                                    wh::core::run_context &context,
...
template <component_kind Kind, node_contract From, node_contract To,
          typename component_t>
[[nodiscard]] inline auto
bind_explicit_component_async(const component_t &component,
                              graph_value &input,
                              wh::core::run_context &context,
...
struct custom_component_async_contract<node_contract::value, component_t,
  ...
  static auto run(const component_t &component, graph_value &input,
      ...
      -> graph_sender {
    if constexpr (async_invoke_result_sender<
                      component_t, request_t, wh::core::result<response_t>>) {
...
concept custom_component_sync_binding =
    requires(const component_t &component, graph_value &input,
             wh::core::run_context &context) {
      { custom_component_contract<To, component_t, request_t, response_t>::run(
            component, input, context, std::declval<const node_runtime &>()) }
      -> std::same_as<wh::core::result<graph_value>>;
    };
...
concept custom_component_async_binding =
    requires(const component_t &component, graph_value &input,
             wh::core::run_context &context) {
      { custom_component_async_contract<To, component_t, request_t,
                                        response_t>::run(
            component, input, context, std::declval<const node_runtime &>()) }
      -> std::same_as<graph_sender>;
    };
...
concept custom_component_binding =
    typed_request<From, request_t> &&
    typed_response<To, response_t> &&
    ((Exec == node_exec_mode::sync &&
...
inline auto component_node::compile() const & -> compiled_node {
  auto copied = *this;
  return std::move(copied).compile();
}

inline auto component_node::compile() && -> compiled_node {
...
[[nodiscard]] inline auto make_component_node(key_t &&key,
  ...
  return component_node{
      ...
      component_payload{
          ...
          .lower =
              [component =
                   stored_component_t{std::forward<component_t>(component)}](
                  std::string key,
                  graph_add_node_options options) mutable -> compiled_node {
