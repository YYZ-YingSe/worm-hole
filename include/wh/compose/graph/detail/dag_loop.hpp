...
inline auto detail::invoke_runtime::dag_runtime::take_ready_action()
    -> ready_action {
  ...
  if (!session.owner_->is_node_designated(node_id, invoke.bound_call_scope)) {
    ...
    session.state_table_.update(node_id, graph_node_lifecycle_state::skipped,
                                0U, std::nullopt);
    ...
    session.emit_debug(graph_debug_stream_event::decision_kind::skipped,
                       node_id, invoke.step_count);
  ...
  if (invoke.step_count > invoke.step_budget) {
    ...
    session.owner_->publish_graph_run_error(
        invoke.outputs, session.runtime_node_path(node_id), node_key,
