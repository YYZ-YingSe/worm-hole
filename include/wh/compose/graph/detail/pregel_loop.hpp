...
inline auto detail::invoke_runtime::pregel_runtime::take_ready_action(const std::uint32_t node_id,
                                                                      const std::size_t step)
    -> pregel_action {
  auto &session = session_;
  auto &invoke = session.invoke_state();
  session.emit_debug(graph_debug_stream_event::decision_kind::dequeue, node_id, step);
  ...
  if (session.owner_->classify_pregel_node_readiness(node_id, pregel_delivery_.current[node_id]) ==
  ...
  if (node == nullptr) {
    ...
    return pregel_action::terminal_error(node_id, cause, wh::core::errc::not_found);
  }
