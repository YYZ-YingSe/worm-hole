...
[[nodiscard]] auto
start_bound_graph(const wh::compose::graph &graph, wh::core::run_context &context,
...
class graph {
  ...
  template <typename receiver_t> class invoke_operation;

  template <typename request_t> class invoke_sender;

  using invoke_completion_signatures =
      stdexec::completion_signatures<stdexec::set_value_t(wh::core::result<graph_invoke_result>)>;

  template <typename request_t>
    requires std::same_as<std::remove_cvref_t<request_t>, graph_invoke_request>
  [[nodiscard]] auto make_invoke_sender(request_t &&request, wh::core::run_context &context) const
      -> invoke_sender<graph_invoke_request>;

  template <typename request_t>
    requires std::same_as<std::remove_cvref_t<request_t>, graph_invoke_request>
  [[nodiscard]] auto make_invoke_sender(request_t &&request, wh::core::run_context &context,
...
public:
  ...
  [[nodiscard]] auto restore_shape() const noexcept -> const graph_restore_shape &;
  ...
  [[nodiscard]] auto node_id(const std::string_view key) const -> wh::core::result<std::uint32_t>;
...
private:
  [[nodiscard]] auto core() noexcept -> detail::graph_core & { return core_storage_; }

  [[nodiscard]] auto core() const noexcept -> const detail::graph_core & { return core_storage_; }
  ...
  detail::start_bound_graph(const graph &graph, wh::core::run_context &context, graph_value &input,
      ...
      -> graph_sender;
  friend auto detail::start_nested_graph(const graph &graph, wh::core::run_context &context,
  ...
  using indexed_value_branch_definition = detail::graph_core::indexed_value_branch_definition;
  using indexed_stream_branch_definition = detail::graph_core::indexed_stream_branch_definition;
  ...
  using compiled_execution_index = detail::graph_core::compiled_execution_index;
  using control_graph_index = detail::graph_core::control_graph_index;
  using value_branch_definition = detail::graph_core::value_branch_definition;
  using stream_branch_definition = detail::graph_core::stream_branch_definition;
  ...
  struct value_output_consumers {
    ...
    [[nodiscard]] auto owner_count() const noexcept -> std::size_t {
      return value_edges.size() + stream_edges.size() + static_cast<std::size_t>(final_output);
    }
  };

  template <typename node_t> auto add_node_impl(node_t &&node) -> wh::core::result<void>;

  [[nodiscard]] static constexpr auto
  default_edge_lowering_kind(const node_contract source_output,
                             const node_contract target_input) noexcept -> edge_lowering_kind;

  [[nodiscard]] auto resolve_edge_adapter(const graph_edge &edge) const
      -> wh::core::result<edge_adapter>;

  template <typename edge_t> auto add_edge_impl(edge_t &&edge) -> wh::core::result<void>;
  ...
  [[nodiscard]] static auto to_compile_node_options_info(const graph_add_node_options &options,
  ...
  [[nodiscard]] static auto make_csr_offsets(const std::vector<std::uint32_t> &counts)
  ...
  [[nodiscard]] auto validate_call_scope_for_runtime(const graph_call_scope &call_scope) const
      -> wh::core::result<void>;

  [[nodiscard]] auto make_node_designation_path(const std::uint32_t node_id) const -> node_path;
  ...
  auto publish_node_run_error(detail::runtime_state::invoke_outputs &outputs,
                              const node_path &runtime_path, const std::uint32_t node_id,
                              const wh::core::error_code code, const std::string_view message) const
      -> void;

  auto publish_graph_run_error(
      detail::runtime_state::invoke_outputs &outputs, const std::optional<node_path> &runtime_path,
      ...
      const std::optional<wh::core::error_code> raw_error = std::nullopt) const -> void;

  auto publish_stream_read_error(detail::runtime_state::invoke_outputs &outputs,
                                 node_path runtime_path, const std::string_view node_key,
  ...
  [[nodiscard]] static auto has_descendant_designation_target(const graph_call_scope &call_options,
                                                              const node_path &path) -> bool;

  [[nodiscard]] static auto has_active_designation(const graph_call_scope &options) noexcept
      -> bool;

  [[nodiscard]] auto is_node_designated(const std::uint32_t node_id,
                                        const graph_call_scope &call_options) const -> bool;

  auto emit_debug_stream_event(wh::core::run_context &context,
  ...
  [[nodiscard]] auto make_stream_scope(const std::string_view node_key) const -> graph_event_scope;

  [[nodiscard]] auto make_stream_scope(const std::string_view node_key,
                                       const node_path &runtime_path) const -> graph_event_scope;

  auto apply_state_phase(wh::core::run_context &context, const graph_node_state_handlers *handlers,
  ...
  [[nodiscard]] auto apply_state_phase_async(
      ...
      detail::state_runtime::state_phase phase, const std::string_view node_key,
      const graph_state_cause &cause, graph_process_state &process_state, graph_value payload,
      const node_path &runtime_path, detail::runtime_state::invoke_outputs &outputs,
  ...
  [[nodiscard]] auto
  evaluate_interrupt_hook(wh::core::run_context &context, const graph_interrupt_node_hook &hook,
                          const std::string_view node_key, const graph_value &payload) const
      -> wh::core::result<std::optional<wh::core::interrupt_signal>>;

  [[nodiscard]] static auto make_missing_pending_input_default(const node_contract contract)
      -> wh::core::result<graph_value>;

  auto resolve_missing_pending_input(const node_contract input_contract) const
      -> wh::core::result<graph_value>;

  auto apply_runtime_resume_controls(wh::core::run_context &context,
                                     const detail::runtime_state::invoke_config &config) const
      -> wh::core::result<void>;

  auto prepare_restore_checkpoint(wh::core::run_context &context,
  ...
  resolve_edge_status_indexed(const indexed_edge &edge,
                              ...
                              const std::vector<branch_state> &branch_states) const -> edge_status;

  [[nodiscard]] auto
  classify_node_readiness_indexed(const std::uint32_t node_id,
                                  const std::vector<dag_node_phase> &dag_node_phases,
                                  const std::vector<branch_state> &branch_states,
                                  const dynamic_bitset &output_valid) const -> ready_state;

  [[nodiscard]] auto classify_pregel_node_readiness(const std::uint32_t node_id,
                                                    const pregel_node_inputs &inputs) const
      -> pregel_ready_state;

  [[nodiscard]] static auto
  collect_reader_value(graph_stream_reader reader, edge_limits limits,
                       const wh::core::detail::any_resume_scheduler_t &graph_scheduler)
      -> graph_sender;

  [[nodiscard]] static constexpr auto needs_reader_lowering(const indexed_edge &edge) noexcept
      -> bool {
    return edge.source_output == node_contract::stream && edge.target_input == node_contract::value;
  }
  ...
  [[nodiscard]] static auto
  lower_reader(graph_stream_reader reader, reader_lowering lowering, wh::core::run_context &context,
               const wh::core::detail::any_resume_scheduler_t &graph_scheduler) -> graph_sender;

  [[nodiscard]] auto needs_reader_merge(const std::uint32_t node_id) const noexcept -> bool;

  [[nodiscard]] auto adapt_edge_output(const indexed_edge &edge, graph_value &source_output,
                                       wh::core::run_context &context) const
      -> wh::core::result<graph_value>;

  [[nodiscard]] auto
  plan_value_output_consumers(const std::uint32_t source_node_id,
                              const std::optional<std::vector<std::uint32_t>> &selection) const
      -> value_output_consumers;

  [[nodiscard]] auto lower_value_output_reader(const indexed_edge &edge, graph_value value,
                                               wh::core::run_context &context) const
      -> wh::core::result<graph_stream_reader>;

  auto store_node_output(const std::uint32_t node_id, io_storage &io_storage,
                         graph_value value) const -> wh::core::result<void>;

  auto commit_value_output(const std::uint32_t source_node_id, io_storage &io_storage,
                           graph_value value,
                           const std::optional<std::vector<std::uint32_t>> &selection,
                           wh::core::run_context &context) const -> wh::core::result<void>;

  [[nodiscard]] auto view_node_output(const std::uint32_t node_id, io_storage &io_storage) const
      -> wh::core::result<graph_value>;

  [[nodiscard]] auto take_node_output(const std::uint32_t node_id, io_storage &io_storage) const
      -> wh::core::result<graph_value>;

  auto commit_stream_output(const std::uint32_t source_node_id, io_storage &io_storage,
                            graph_stream_reader reader,
                            const std::optional<std::vector<std::uint32_t>> &selection) const
      -> wh::core::result<void>;

  [[nodiscard]] auto collect_input_lanes(const std::uint32_t node_id,
                                         const std::vector<dag_node_phase> &dag_node_phases,
                                         const std::vector<branch_state> &branch_states,
  ...
  [[nodiscard]] static auto borrow_input(graph_value &value, const node_contract contract)
      -> wh::core::result<resolved_input>;

  [[nodiscard]] static auto own_input(graph_value value, const node_contract contract)
      -> wh::core::result<resolved_input>;

  [[nodiscard]] auto resolve_edge_value(const std::uint32_t edge_id, io_storage &io_storage,
                                        wh::core::run_context &context) const
      -> wh::core::result<graph_value *>;

  [[nodiscard]] auto resolve_edge_reader(const std::uint32_t edge_id, io_storage &io_storage) const
      -> wh::core::result<graph_stream_reader *>;

  [[nodiscard]] auto take_edge_reader(const std::uint32_t edge_id, io_storage &io_storage) const
      -> wh::core::result<graph_stream_reader>;

  [[nodiscard]] auto merged_reader(const std::uint32_t node_id, io_storage &io_storage) const
      -> wh::core::result<graph_stream_reader *>;

  auto update_merged_reader(const std::uint32_t node_id, io_storage &io_storage,
                            input_lane_span lanes) const -> wh::core::result<void>;

  auto refresh_merged_reader(const std::uint32_t node_id, io_storage &io_storage,
                             const std::vector<dag_node_phase> &dag_node_phases,
                             const std::vector<branch_state> &branch_states) const
      -> wh::core::result<void>;

  [[nodiscard]] auto build_reader_input(const compiled_node &node, const std::uint32_t node_id,
                                        io_storage &io_storage, input_lane_span lanes) const
      -> wh::core::result<resolved_input>;

  [[nodiscard]] auto build_value_input(const compiled_node &node, io_storage &io_storage,
                                       input_lane_span lanes, wh::core::run_context &context) const
      -> wh::core::result<resolved_input>;

  [[nodiscard]] auto finish_value_input(const compiled_node &node, value_batch batch) const
      -> wh::core::result<resolved_input>;

  auto refresh_source_readers(const std::uint32_t source_node_id, io_storage &io_storage,
                              const std::vector<dag_node_phase> &dag_node_phases,
                              const std::vector<branch_state> &branch_states) const
      -> wh::core::result<void>;

  auto reset_pregel_source_caches(const std::uint32_t source_node_id, io_storage &io_storage) const
      -> void;

  auto seed_pregel_successors(const std::uint32_t source_node_id,
                              const std::optional<std::vector<std::uint32_t>> &selection,
                              pregel_delivery_store &deliveries) const -> void;

  auto stage_pregel_successors(const std::uint32_t source_node_id,
                               const std::optional<std::vector<std::uint32_t>> &selection,
                               pregel_delivery_store &deliveries) const -> void;

  [[nodiscard]] auto build_node_input(const std::uint32_t node_id, io_storage &io_storage,
                                      const std::vector<dag_node_phase> &dag_node_phases,
                                      const std::vector<branch_state> &branch_states,
  ...
  [[nodiscard]] auto build_pregel_node_input_sender(
      const std::uint32_t node_id, const pregel_node_inputs &inputs, io_storage &io_storage,
  ...
  [[nodiscard]] auto resolve_node_parallel_gate(const std::uint32_t node_id) const -> std::size_t;

  [[nodiscard]] auto resolve_node_sync_dispatch(const std::uint32_t node_id) const -> sync_dispatch;

  [[nodiscard]] static auto
  resolve_branch_merge(const detail::runtime_state::invoke_config &config) noexcept
      -> graph_branch_merge;

  [[nodiscard]] static auto merge_branch_selected_nodes(
      const std::vector<std::uint32_t> &existing_sorted, std::vector<std::uint32_t> incoming_sorted,
      const graph_branch_merge strategy) -> wh::core::result<std::vector<std::uint32_t>>;

  auto commit_branch_selection(const std::uint32_t node_id,
                               std::optional<std::vector<std::uint32_t>> selection,
                               dag_schedule &dag_schedule,
                               const detail::runtime_state::invoke_config &config) const
      -> wh::core::result<void>;

  [[nodiscard]] auto evaluate_value_branch_indexed(const std::uint32_t source_node_id,
                                                   const graph_value &source_output,
                                                   wh::core::run_context &context,
                                                   const graph_call_scope &call_options) const
      -> wh::core::result<std::optional<std::vector<std::uint32_t>>>;

  [[nodiscard]] auto evaluate_stream_branch_indexed(const std::uint32_t source_node_id,
                                                    graph_stream_reader &source_output,
                                                    wh::core::run_context &context,
                                                    const graph_call_scope &call_options) const
      -> wh::core::result<std::optional<std::vector<std::uint32_t>>>;

  [[nodiscard]] auto build_control_graph_index() const -> wh::core::result<control_graph_index>;
  ...
  [[nodiscard]] auto is_reachable_start_to_end(const control_graph_index &index) const -> bool;

  [[nodiscard]] auto find_cycle_path(const control_graph_index &index) const
      -> std::optional<std::vector<std::string>>;

  [[nodiscard]] auto has_edge(const std::string &from, const std::string &to) const -> bool;
  ...
  auto fail_fast(const wh::core::error_code code, message_t &&message) -> wh::core::result<void>;

  [[nodiscard]] auto is_cycle_allowed() const noexcept -> bool;

  [[nodiscard]] auto resolve_step_budget(const detail::runtime_state::invoke_config &config,
                                         const graph_call_scope &call_options) const
