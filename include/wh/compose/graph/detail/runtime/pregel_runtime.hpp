...
public:
  ...
  auto commit_skip_action(const pregel_action &action) -> wh::core::result<void>;

  [[nodiscard]] auto take_ready_action(const std::uint32_t node_id, const std::size_t step)
      -> pregel_action;
  ...
  auto pregel_delivery() -> input_runtime::pregel_delivery_store & { return pregel_delivery_; }

  [[nodiscard]] auto superstep_active() const noexcept -> bool { return superstep_active_; }

  auto set_superstep_active(const bool active) noexcept -> void { superstep_active_ = active; }
