...
inline auto apply_stream_codecs_for_save(checkpoint_state &checkpoint,
                                         wh::core::run_context &context,
  ...
  if (start_iter != checkpoint.rerun_inputs.end()) {
    auto converted_start = convert_one(graph_start_node_key, start_iter->second, false);
...
inline auto apply_stream_codecs_for_load(checkpoint_state &checkpoint,
                                         wh::core::run_context &context,
                                         const checkpoint_stream_codecs *registry)
    -> wh::core::result<void> {
  const bool has_registry = registry != nullptr;
...
load_rerun_inputs(checkpoint_state &checkpoint, runtime_state::rerun_state &rerun_state,
...
[[nodiscard]] inline auto save_rerun_inputs(runtime_state::rerun_state &rerun_state,
  ...
  for (std::uint32_t node_id = 0U; node_id < static_cast<std::uint32_t>(node_keys.size());
    ...
    if (node == nullptr) {
      return wh::core::result<graph_value_map>::failure(wh::core::errc::not_found);
    }
...
[[nodiscard]] inline auto validate_runtime_configuration(const runtime_state::invoke_config &config,
  ...
  if (config.checkpoint_backend != nullptr &&
      (!config.checkpoint_backend->prepare_restore || !config.checkpoint_backend->save)) {
    set_error_detail(outputs, wh::core::errc::invalid_argument, "", "validate_runtime_config");
    return wh::core::result<void>::failure(wh::core::errc::invalid_argument);
  }
  if (config.checkpoint_serializer != nullptr &&
      (!config.checkpoint_serializer->encode || !config.checkpoint_serializer->decode)) {
    set_error_detail(outputs, wh::core::errc::invalid_argument, "", "validate_runtime_config");
    return wh::core::result<void>::failure(wh::core::errc::invalid_argument);
  }
...
inline auto maybe_restore(graph_value &input, wh::core::run_context &context,
  ...
  auto stream_restored =
      apply_stream_codecs_for_load(checkpoint, context, config.checkpoint_stream_codecs);
  ...
  auto rerun_restored = load_rerun_inputs(checkpoint, rerun_state, resolver, start_id);
  if (rerun_restored.has_error()) {
    set_error_detail(outputs, rerun_restored.error(), checkpoint_id_hint, "restore_rerun_inputs");
  ...
  if (restored_states.has_error()) {
    set_error_detail(outputs, restored_states.error(), checkpoint_id_hint, "restore_node_states");
...
inline auto
maybe_persist(wh::core::run_context &context, const graph_state_table &state_table,
              runtime_state::rerun_state &rerun_state, const std::span<const std::string> node_keys,
              const std::span<const compiled_node *const> nodes_by_id,
              const std::string_view graph_name, const graph_restore_shape &current_restore_shape,
              const runtime_state::invoke_config &config, runtime_state::invoke_outputs &outputs)
    -> wh::core::result<void> {
  auto runtime_backend = resolve_runtime_backend(config);
  if (runtime_backend.has_error()) {
    set_error_detail(outputs, runtime_backend.error(), graph_name, "persist_store_lookup");
    return wh::core::result<void>::failure(runtime_backend.error());
  }
  ...
  if (context.interrupt_info.has_value()) {
    ...
    if (interrupt_snapshot.has_error()) {
      ...
      return wh::core::result<void>::failure(interrupt_snapshot.error());
    }
    checkpoint.interrupt_snapshot = std::move(interrupt_snapshot).value();
  }
  auto stream_persisted =
      apply_stream_codecs_for_save(checkpoint, context, config.checkpoint_stream_codecs);
  if (stream_persisted.has_error()) {
    set_error_detail(outputs, stream_persisted.error(), graph_name, "persist_stream_convert");
  ...
  if (owned_checkpoint.has_error()) {
    set_error_detail(outputs, owned_checkpoint.error(), graph_name, "persist_snapshot_own");
