...
template <typename runtime_t, template <typename, typename> class run_t>
class basic_run_sender;
template <typename sender_t, typename runtime_t>
[[nodiscard]] auto start_initialized_runtime(runtime_t runtime)
    -> graph_sender;
...
protected:
  ...
  [[nodiscard]] auto initialize_start_entry(graph_value input)
      -> wh::core::result<void>;
  ...
  [[nodiscard]] auto capture_common_checkpoint_state()
  ...
  friend class basic_run_sender;
  template <typename sender_t, typename runtime_t>
  friend auto start_initialized_runtime(runtime_t runtime)
      -> graph_sender;
