...
inline auto append_state_snapshot_event(detail::runtime_state::invoke_outputs &outputs,
  ...
  outputs.state_snapshot_events.push_back(graph_state_snapshot_event{
      ...
      .snapshot = wh::core::any(event),
  });
}

inline auto append_state_delta_event(detail::runtime_state::invoke_outputs &outputs,
                                     const graph_state_transition_event &event,
                                     graph_event_scope scope) -> void {
  outputs.state_delta_events.push_back(graph_state_delta_event{
      .scope = std::move(scope),
      .step = event.cause.step,
      .delta = wh::core::any(event),
  });
}

inline auto append_runtime_message_event(detail::runtime_state::invoke_outputs &outputs,
...
inline auto emit_debug_event(wh::core::run_context &context,
                             ...
                             const graph_call_scope &options, const graph_debug_stream_event &event,
                             const graph_event_scope &scope) -> void {
  ...
  if (!options.isolate_debug_stream()) {
    append_runtime_message_event(outputs, scope, event.step, "debug-decision");
  }
}
...
inline auto append_custom_events(detail::runtime_state::invoke_outputs &outputs,
  ...
  for (const auto &subscription : options.options().stream_subscriptions) {
    if (!subscription.enabled || subscription.kind != graph_stream_channel_kind::custom ||
        subscription.custom_channel.empty()) {
      continue;
    }
    outputs.custom_events.push_back(graph_custom_event{
        .scope = scope,
        .step = event.cause.step,
...
inline auto append_state_transition_events(
    detail::runtime_state::invoke_outputs &outputs, const graph_call_scope &options,
    const graph_state_transition_event &event, const graph_event_scope &scope,
    const bool emit_state_snapshot_events, const bool emit_state_delta_events,
    const bool emit_runtime_message_events, const bool emit_custom_events) -> void {
  ...
  if (emit_runtime_message_events) {
    append_runtime_message_event(outputs, scope, event.cause.step, "state-transition");
  }
}

inline auto
append_state_transition(graph_transition_log &log, detail::runtime_state::invoke_outputs &outputs,
  ...
  if (record_log) {
    log.push_back(event);
    append_state_transition_events(outputs, options, log.back(), scope, emit_state_snapshot_events,
                                   emit_state_delta_events, emit_runtime_message_events,
                                   emit_custom_events);
    return;
  }
  append_state_transition_events(outputs, options, event, scope, emit_state_snapshot_events,
                                 emit_state_delta_events, emit_runtime_message_events,
                                 emit_custom_events);
}
