...
[[nodiscard]] inline auto to_dag_node_phase(const graph_node_lifecycle_state lifecycle) noexcept
...
inline auto restore_dag_node_phases(const std::span<const graph_node_state> lifecycle,
...
capture_dag_runtime(input_runtime::io_storage &storage, input_runtime::dag_schedule &dag_schedule,
                    detail::dag_frontier &frontier, std::vector<std::uint32_t> suspended_nodes)
    -> wh::core::result<checkpoint_dag_runtime_state> {
  auto captured_io = capture_runtime_io(storage);
  if (captured_io.has_error()) {
    return wh::core::result<checkpoint_dag_runtime_state>::failure(captured_io.error());
  }
...
inline auto restore_dag_runtime_io(input_runtime::io_storage &storage,
  ...
  return restore_runtime_slots(storage, dag.node_outputs, dag.edge_values, dag.edge_readers,
                               dag.merged_readers, dag.merged_reader_lanes,
                               dag.final_output_reader);
}

inline auto
restore_dag_runtime(checkpoint_dag_runtime_state &dag, input_runtime::io_storage &storage,
                    const std::span<const graph_node_state> lifecycle,
                    std::vector<input_runtime::dag_node_phase> &dag_node_phases,
                    input_runtime::dag_schedule &dag_schedule, detail::dag_frontier &frontier,
                    std::vector<std::uint32_t> &suspended_nodes) -> wh::core::result<void> {
  ...
  for (auto &branch : dag.branch_states) {
    ...
    dag_schedule.mark_branch_decided(branch.node_id, std::move(branch.selected_end_nodes_sorted));
  }

  std::vector<std::uint32_t> restored_current{};
  const auto frontier_head = std::min(dag.current_frontier_head, dag.current_frontier.size());
  ...
  for (std::size_t index = frontier_head; index < dag.current_frontier.size(); ++index) {
...
inline auto
mark_restored_dag_pending_nodes(runtime_state::pending_inputs &pending_inputs,
                                const std::vector<input_runtime::dag_node_phase> &dag_node_phases)
    -> void {
  for (std::uint32_t node_id = 0U; node_id < static_cast<std::uint32_t>(dag_node_phases.size());
       ++node_id) {
