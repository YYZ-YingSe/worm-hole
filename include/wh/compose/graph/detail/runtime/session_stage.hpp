...
inline auto
detail::invoke_runtime::invoke_session::make_input_attempt(
...
inline auto detail::invoke_runtime::invoke_session::prepare_execution_input(
  ...
  return state_step{
      ...
      .sender = owner_->lower_reader(std::move(*reader), std::move(lowering),
                                     context_, *invoke_state().work_scheduler),
  };
}
...
inline auto detail::invoke_runtime::invoke_session::begin_state_post(
  ...
  if (post_state.has_error()) {
    ...
    release_attempt(attempt);
    return wh::core::result<std::optional<graph_sender>>::failure(
        post_state.error());
  }
...
inline auto detail::invoke_runtime::invoke_session::apply_node_timeout(
  ...
  return make_node_timeout_failure(outputs, slot.cause.node_key, slot.attempt,
                                   *slot.timeout_budget, attempt_start);
}
...
inline auto detail::invoke_runtime::invoke_session::make_async_timed_node_sender(
  ...
  auto timeout_sender =
      ...
      stdexec::then([&outputs, node_key = slot.cause.node_key,
                     attempt = slot.attempt, timeout_budget,
...
inline auto detail::invoke_runtime::invoke_session::make_sync_node_attempt_sender(
    const compiled_node &node, graph_value &input_value,
    wh::core::run_context &context, const graph_call_scope &bound_call_options,
    invoke_session *state, attempt_slot &slot) -> graph_sender {
  const auto attempt_start = std::chrono::steady_clock::now();
  auto sender = stdexec::schedule(*state->invoke_state().work_scheduler) |
                stdexec::then([&node, &input_value, &context,
