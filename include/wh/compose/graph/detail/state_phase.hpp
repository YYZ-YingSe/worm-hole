...
inline auto graph::apply_state_phase(
    ...
    const detail::state_runtime::state_phase phase, const std::string_view node_key,
    const graph_state_cause &cause, graph_process_state &process_state, graph_value &payload,
    const node_path &runtime_path, detail::runtime_state::invoke_outputs &outputs) const
    -> wh::core::result<void> {
  return phase == detail::state_runtime::state_phase::pre
             ? detail::state_runtime::apply_pre_handlers(
                   ...
                   [this, &outputs, &runtime_path](
                     ...
                     publish_stream_read_error(outputs, runtime_path, key, code, message);
                   })
...
inline auto graph::apply_state_phase_async(
  ...
  struct phase_state {
    ...
    detail::state_runtime::state_phase phase{detail::state_runtime::state_phase::pre};
    graph_value payload{};
    state_handler_t value_handler{nullptr};
    state_handler_t stream_handler{nullptr};
  };

  const auto fail_stream = [](phase_state &state, const wh::core::error_code code,
                              const std::string_view message) -> graph_sender {
    state.owner->publish_stream_read_error(*state.outputs, state.runtime_path, state.node_key, code,
                                           message);
    return detail::failure_graph_sender(code);
  };

  const auto finish = [](phase_state state) -> graph_sender {
    return detail::ready_graph_sender(wh::core::result<graph_value>{std::move(state.payload)});
  };

  const auto run_stream = [finish, fail_stream,
                           &graph_scheduler](phase_state state) -> graph_sender {
    if (state.stream_handler == nullptr) {
      return finish(std::move(state));
    }

    if (auto *reader = wh::core::any_cast<graph_stream_reader>(&state.payload); reader != nullptr) {
      auto source = std::move(*reader);
      auto [writer, rewritten] = make_graph_stream();
      auto handler = [cause = state.cause, context = state.context,
                      process_state = state.process_state, stream_handler = state.stream_handler](
      ...
      return detail::bridge_graph_sender(
          detail::make_rewrite_stream_sender(std::move(source), std::move(writer),
                                             std::move(handler), graph_scheduler) |
          stdexec::let_value([state = std::move(state), finish, fail_stream](
                                 wh::core::result<graph_value> status) mutable -> graph_sender {
            if (status.has_error()) {
    ...
    auto handled =
        state.stream_handler(state.cause, *state.process_state, state.payload, *state.context);
  ...
  auto handled =
      state.value_handler(state.cause, *state.process_state, state.payload, *state.context);
  if (handled.has_error()) {
    return detail::failure_graph_sender(handled.error());
  }
  return run_stream(std::move(state));
