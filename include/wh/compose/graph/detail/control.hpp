...
inline auto graph::validate_call_scope_for_runtime(const graph_call_scope &call_scope) const
  ...
  if (call_scope.prefix().empty()) {
    for (const auto &node_key : call_options.designated_top_level_nodes) {
      ...
      if (!core().compiled_execution_index_.index.key_to_id.contains(node_key)) {
  ...
  auto validate_path = [&](const node_path &path) -> wh::core::result<void> {
    ...
    if (segments.empty() || !core().compiled_execution_index_.index.key_to_id.contains(
  ...
  auto validate_scoped_path = [&](const node_path &path) -> wh::core::result<void> {
    ...
    if (relative->empty()) {
      return call_scope.prefix().empty()
                 ? wh::core::result<void>::failure(wh::core::errc::invalid_argument)
                 : wh::core::result<void>{};
    }
...
inline auto graph::make_node_designation_path(const std::uint32_t node_id) const -> node_path {
...
inline auto graph::make_runtime_node_path(const node_path &prefix,
  ...
  if (!prefix.empty()) {
    ...
    const auto &node_key = core().compiled_execution_index_.index.id_to_key[node_id];
...
inline auto graph::publish_node_run_error(detail::runtime_state::invoke_outputs &outputs,
...
inline auto
graph::publish_graph_run_error(detail::runtime_state::invoke_outputs &outputs,
...
inline auto graph::publish_stream_read_error(detail::runtime_state::invoke_outputs &outputs,
...
inline auto graph::make_node_execution_address(const node_path &runtime_path) const
    -> wh::core::address {
...
inline auto graph::has_descendant_designation_target(const graph_call_scope &call_options,
                                                     const node_path &path) -> bool {
  const auto absolute = call_options.absolute_path(path);
  return std::ranges::any_of(
      call_options.options().designated_node_paths,
      [&](const node_path &candidate) { return candidate.starts_with(absolute); });
}

inline auto graph::has_active_designation(const graph_call_scope &options) noexcept -> bool {
  return (options.prefix().empty() && !options.options().designated_top_level_nodes.empty()) ||
         !options.options().designated_node_paths.empty();
}

inline auto graph::is_node_designated(const std::uint32_t node_id,
  ...
  if (is_graph_node_designated(call_options,
                               core().compiled_execution_index_.index.id_to_key[node_id],
                               designation_path)) {
    return true;
  }
...
inline auto graph::emit_debug_stream_event(wh::core::run_context &context,
...
inline auto graph::make_stream_scope(const std::string_view node_key) const -> graph_event_scope {
  const std::array<std::string_view, 1U> segments{node_key};
  const auto path =
      make_node_path(std::span<const std::string_view>{segments.data(), segments.size()});
...
inline auto graph::append_state_transition(detail::runtime_state::invoke_outputs &outputs,
                                           const graph_call_scope &options,
                                           const graph_state_transition_event &event,
                                           const node_path &runtime_path) const -> void {
  const auto emit_state_snapshot_events =
      has_graph_stream_subscription(options, graph_stream_channel_kind::state_snapshot);
  const auto emit_state_delta_events =
      has_graph_stream_subscription(options, graph_stream_channel_kind::state_delta);
  ...
  const auto emit_custom_events = std::ranges::any_of(
      options.options().stream_subscriptions, [](const graph_stream_subscription &subscription) {
        return subscription.enabled && subscription.kind == graph_stream_channel_kind::custom &&
  ...
  detail::stream_runtime::append_state_transition(
      ...
      make_stream_scope(event.cause.node_key, runtime_path), true, emit_state_snapshot_events,
...
inline auto graph::evaluate_interrupt_hook(wh::core::run_context &context,
                                           const graph_interrupt_node_hook &hook,
                                           const std::string_view node_key,
                                           const graph_value &payload) const
    -> wh::core::result<std::optional<wh::core::interrupt_signal>> {
  return detail::interrupt_runtime::evaluate_hook(context, hook, node_key, payload);
}

inline auto graph::make_missing_pending_input_default(const node_contract contract)
...
inline auto graph::resolve_missing_pending_input(const node_contract input_contract) const
    -> wh::core::result<graph_value> {
  return make_missing_pending_input_default(input_contract);
}

inline auto
graph::apply_runtime_resume_controls(wh::core::run_context &context,
...
inline auto graph::prepare_restore_checkpoint(wh::core::run_context &context,
