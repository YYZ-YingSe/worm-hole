...
inline auto
detail::invoke_runtime::invoke_session::rebind_moved_runtime_storage() noexcept
    -> void {
...
inline auto detail::invoke_runtime::start_initialized_runtime(runtime_t runtime)
    -> graph_sender {
...
inline auto detail::invoke_runtime::start_dag_run(invoke_session &&session)
    -> graph_sender {
  return start_initialized_runtime<dag_run_sender>(
      detail::invoke_runtime::dag_runtime{std::move(session)});
}

inline auto detail::invoke_runtime::start_pregel_run(invoke_session &&session)
    -> graph_sender {
...
inline auto detail::start_session(
    const graph &graph, detail::invoke_runtime::invoke_session session)
    -> graph_sender {
...
inline auto detail::start_request(
  ...
  const bool has_restore_source =
      controls.checkpoint.load.has_value() ||
      !controls.checkpoint.forwarded_once.empty();
  auto input = detail::normalize_graph_input(graph.boundary().input,
                                             has_restore_source,
...
inline auto detail::start_bound_graph(
    const graph &graph, wh::core::run_context &context, graph_value &input,
...
inline auto detail::start_scoped_graph(
    const graph &graph, wh::core::run_context &context, graph_value &input,
...
inline auto detail::start_nested_graph(const graph &graph,
