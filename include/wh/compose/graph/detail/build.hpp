...
tools_node_required_endpoint(const node_exec_mode exec_mode,
...
[[nodiscard]] inline auto tools_entry_matches_endpoint(
    const tool_entry &entry, const node_exec_mode exec_mode,
    const node_contract output_contract) noexcept -> bool {
  if (output_contract == node_contract::stream) {
    return exec_mode == node_exec_mode::async
...
[[nodiscard]] inline auto validate_tools_node_entrypoints(
    const std::string_view node_key, const node_descriptor &descriptor,
    const tools_payload &payload) -> std::optional<std::string> {
  if (descriptor.exec_mode == node_exec_mode::sync &&
      !payload.runtime_options.sequential) {
    return "tools node `" + std::string{node_key} +
           "` does not support non-sequential sync execution";
  }
  const auto required = tools_node_required_endpoint(
      descriptor.exec_mode, descriptor.output_contract);
  for (const auto &[tool_name, entry] : payload.registry) {
    if (tools_entry_matches_endpoint(entry, descriptor.exec_mode,
                                     descriptor.output_contract)) {
      continue;
    }
    return "tools node `" + std::string{node_key} + "` requires `" +
           std::string{required} + "` for tool `" + tool_name + "`";
  }
  if (payload.runtime_options.missing.has_value() &&
      !tools_entry_matches_endpoint(*payload.runtime_options.missing,
                                    descriptor.exec_mode,
                                    descriptor.output_contract)) {
    return "tools node `" + std::string{node_key} + "` requires `" +
           std::string{required} + "` for missing-tool entry";
  }
  return std::nullopt;
}
...
inline auto graph::add_lambda(const lambda_node &node)
    -> wh::core::result<void> {
  return add_node_impl(node);
}
...
inline auto graph::add_subgraph(key_t &&key, graph_t &&subgraph,
                                options_t &&options) -> wh::core::result<void> {
  return add_subgraph(make_subgraph_node(std::forward<key_t>(key),
...
inline auto graph::add_passthrough(key_t &&key) -> wh::core::result<void> {
  return add_passthrough(
      make_passthrough_node<Contract>(std::forward<key_t>(key)));
}
...
inline auto graph::add_value_branch(const graph_value_branch &branch)
    -> wh::core::result<void> {
  return add_value_branch_impl(branch);
}

inline auto graph::add_value_branch(graph_value_branch &&branch)
    -> wh::core::result<void> {
  return add_value_branch_impl(std::move(branch));
}

inline auto graph::add_stream_branch(const graph_stream_branch &branch)
    -> wh::core::result<void> {
  return add_stream_branch_impl(branch);
}

inline auto graph::add_stream_branch(graph_stream_branch &&branch)
    -> wh::core::result<void> {
...
inline auto graph::add_node_impl(node_t &&node) -> wh::core::result<void> {
  ...
  if constexpr (std::same_as<std::remove_cvref_t<node_t>, tools_node>) {
    if (auto failure = detail::validate_tools_node_entrypoints(
            node_key_view, node.descriptor_, node.payload_);
        failure.has_value()) {
  ...
  const auto next_node_id =
      static_cast<std::uint32_t>(core().node_insertion_order_.size());
...
graph::default_edge_lowering_kind(const node_contract source_output,
...
inline auto graph::resolve_edge_adapter(const graph_edge &edge) const
  ...
  if (source_iter == core().nodes_.end() ||
      target_iter == core().nodes_.end()) {
...
inline auto graph::add_value_branch_impl(branch_t &&branch)
    -> wh::core::result<void> {
  auto writable = ensure_writable();
  if (writable.has_error()) {
    return writable;
  ...
  if (branch.from.empty()) {
    return fail_fast(wh::core::errc::invalid_argument,
                     "value-branch source key is empty");
  }
  ...
  if (branch.end_nodes.empty()) {
    return fail_fast(wh::core::errc::invalid_argument,
                     "value-branch end-nodes are empty");
  }

  const auto source_output =
      authored_output_contract(core().nodes_.at(branch.from));
  if (source_output != node_contract::value) {
    return fail_fast(wh::core::errc::contract_violation,
                     "value-branch source must produce value output: " +
                         branch.from);
  }

  std::unordered_set<std::string_view, detail::string_hash,
                     detail::string_equal>
      unique_end_nodes{};
  ...
  for (const auto &to : branch.end_nodes) {
    ...
    if (!unique_end_nodes.insert(to).second) {
      return fail_fast(wh::core::errc::already_exists,
                       "value-branch end-node duplicated: " + to);
    }
  ...
  if constexpr (std::is_lvalue_reference_v<branch_t>) {
    ...
    core().value_branches_.emplace(std::move(branch.from),
                                   std::move(definition));
  }
  return {};
}
...
template <typename branch_t>
inline auto graph::add_stream_branch_impl(branch_t &&branch)
    -> wh::core::result<void> {
  auto writable = ensure_writable();
  if (writable.has_error()) {
    return writable;
  ...
  if (branch.from.empty()) {
    return fail_fast(wh::core::errc::invalid_argument,
                     "stream-branch source key is empty");
  }
  if (!core().nodes_.contains(branch.from)) {
    return fail_fast(wh::core::errc::not_found,
  ...
  if (branch.end_nodes.empty()) {
    return fail_fast(wh::core::errc::invalid_argument,
                     "stream-branch end-nodes are empty");
  }
  ...
  if (source_output != node_contract::stream) {
    return fail_fast(wh::core::errc::contract_violation,
                     "stream-branch source must produce stream output: " +
                         branch.from);
  }
  ...
  for (const auto &to : branch.end_nodes) {
    ...
    if (!unique_end_nodes.insert(to).second) {
      return fail_fast(wh::core::errc::already_exists,
                       "stream-branch end-node duplicated: " + to);
    }
    auto added = add_edge(branch.from, to);
    if (added.has_error()) {
      return added;
  ...
  if constexpr (std::is_lvalue_reference_v<branch_t>) {
    ...
    definition.end_nodes = std::move(branch.end_nodes);
    definition.selector_ids = std::move(branch.selector_ids);
    core().stream_branches_.emplace(std::move(branch.from),
                                    std::move(definition));
  }
  return {};
}

inline auto graph::has_edge(const std::string &from,
                            const std::string &to) const -> bool {
  return std::ranges::any_of(core().edges_, [&](const graph_edge &edge) {
...
inline auto graph::init_reserved_nodes() -> void {
  ...
  auto start_node =
      make_boundary_node(graph_start_node_key, core().options_.boundary.input);
  auto end_node =
      make_boundary_node(graph_end_node_key, core().options_.boundary.output);
  core().nodes_.emplace(std::string{graph_start_node_key},
                        std::move(start_node));
