...
inline auto detail::invoke_runtime::pregel_runtime::restore_entry(
  ...
  auto restored_pending = detail::checkpoint_runtime::restore_pending_inputs(
      std::move(prepared.checkpoint.runtime.pregel->pending_inputs), session.pending_inputs_,
  ...
  auto restored_runtime = detail::checkpoint_runtime::restore_pregel_runtime(
      *prepared.checkpoint.runtime.pregel, session.io_storage_,
      prepared.checkpoint.runtime.lifecycle, pregel_delivery_, session.node_count(),
      superstep_active_);
  ...
  if (session.restore_skip_pre_handlers_) {
    auto marked = detail::checkpoint_runtime::mark_restored_pregel_pending_nodes(
        session.pending_inputs_, *prepared.checkpoint.runtime.pregel, session.node_count());
    if (marked.has_error()) {
      return marked;
    }
  }
...
inline auto detail::invoke_runtime::pregel_runtime::start_entry(graph_value input)
    -> wh::core::result<void> {
  auto &session = session_;
  ...
  session.owner_->seed_pregel_successors(
      index.start_id, session.invoke_state().start_entry_selection, pregel_delivery_);
...
inline auto detail::invoke_runtime::pregel_runtime::initialize_entry() -> void {
  auto &session = session_;
  ...
  if (session.restore_plan_.has_value()) {
    ...
    if (restored.has_error()) {
      fail_restore(*session.restore_plan_, restored.error(), "restore_pregel_entry");
    }
    return;
  }
