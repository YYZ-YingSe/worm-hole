...
inline auto detail::invoke_runtime::pregel_runtime::make_input_sender(const attempt_id attempt)
    -> graph_sender {
  ...
  return session_.owner_->build_pregel_node_input_sender(
      attempt_slot.node_id, pregel_delivery_.current[attempt_slot.node_id], session_.io_storage_,
      session_.context_, std::addressof(attempt_slot), invoke.config, *invoke.work_scheduler);
}

inline auto detail::invoke_runtime::pregel_runtime::make_input_attempt(const std::uint32_t node_id,
                                                                       const std::size_t step)
    -> wh::core::result<attempt_id> {
  return session_.make_input_attempt(node_id, step);
}

inline auto detail::invoke_runtime::pregel_runtime::begin_state_pre(const attempt_id attempt)
    -> wh::core::result<state_step> {
  return session_.begin_state_pre(attempt);
}

inline auto detail::invoke_runtime::pregel_runtime::commit_terminal_input(const attempt_id attempt,
                                                                          graph_value input)
    -> wh::core::result<void> {
  auto &attempt_slot = session_.slot(attempt);
...
inline auto detail::invoke_runtime::pregel_runtime::finish() -> wh::core::result<graph_value> {
  auto &invoke = session_.invoke_state();
...
class detail::invoke_runtime::pregel_run
    : public detail::invoke_runtime::invoke_stage_run<
          ...
          detail::invoke_runtime::pregel_run<receiver_t, graph_scheduler_t>, graph_scheduler_t> {
  using base_t = invoke_stage_run<detail::invoke_runtime::pregel_runtime, receiver_t,
                                  pregel_run<receiver_t, graph_scheduler_t>, graph_scheduler_t>;
  friend base_t;

public:
  template <typename graph_scheduler_u>
  pregel_run(detail::invoke_runtime::pregel_runtime &&state, receiver_t &&receiver,
  ...
  auto prepare_superstep(const bool advance_step) -> wh::core::result<void> {
    ...
    for (const auto node_id : frontier) {
      prepared_actions_.push_back(this->state().take_ready_action(node_id, step));
    }
    prepared_head_ = 0U;
  ...
  auto resume() noexcept -> void {
    while (true) {
      ...
      if (prepared_head_ >= prepared_actions_.size() && this->active_child_count() == 0U) {
