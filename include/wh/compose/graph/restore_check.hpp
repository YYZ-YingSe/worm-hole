// Defines core graph-restore validation helpers shared by public and runtime paths.
#pragma once
...
enum class issue_kind : std::uint8_t {
  ...
  /// One persisted interrupt target address no longer exists in the current graph.
...
inline auto compare_graph_options(restore_diff &restore_diff,
  ...
  if (baseline.options.boundary.input != current.options.boundary.input ||
    ...
    append_diff(
        restore_diff, restore_diff_kind::graph_option, subject, "boundary",
...
inline auto compare_edges(restore_diff &restore_diff,
  ...
  while (baseline_index < baseline.edges.size() ||
    ...
    const auto baseline_key =
        std::tie(baseline_edge.from, baseline_edge.to);
    ...
    if (baseline_edge.no_control != current_edge.no_control) {
      append_diff(restore_diff, restore_diff_kind::edge_shape, subject,
                  "no_control",
                  bool_text(baseline_edge.no_control),
                  bool_text(current_edge.no_control));
    }
    if (baseline_edge.no_data != current_edge.no_data) {
      append_diff(restore_diff, restore_diff_kind::edge_shape, subject,
                  "no_data",
                  bool_text(baseline_edge.no_data),
                  bool_text(current_edge.no_data));
    }
    if (baseline_edge.adapter_kind != current_edge.adapter_kind) {
      append_diff(restore_diff, restore_diff_kind::edge_shape, subject,
                  "adapter_kind",
                  std::string{adapter_kind_text(baseline_edge.adapter_kind)},
                  std::string{adapter_kind_text(current_edge.adapter_kind)});
    }
    if (baseline_edge.has_custom_value_to_stream !=
        current_edge.has_custom_value_to_stream) {
      append_diff(restore_diff, restore_diff_kind::edge_shape, subject,
                  "has_custom_value_to_stream",
                  bool_text(baseline_edge.has_custom_value_to_stream),
                  bool_text(current_edge.has_custom_value_to_stream));
    }
    if (baseline_edge.has_custom_stream_to_value !=
        current_edge.has_custom_stream_to_value) {
      append_diff(restore_diff, restore_diff_kind::edge_shape, subject,
                  "has_custom_stream_to_value",
                  bool_text(baseline_edge.has_custom_stream_to_value),
                  bool_text(current_edge.has_custom_stream_to_value));
    }
    ++baseline_index;
    ++current_index;
  }
  ...
  while (baseline_index < baseline.branches.size() ||
    ...
    if (current_branch.from < baseline_branch.from) {
      append_diff(restore_diff, restore_diff_kind::branch_destinations,
                  ...
                  "",
                  std::to_string(current_branch.end_nodes.size()) +
                      " destinations");
      ++current_index;
      continue;
    }
    ...
    if (baseline_branch.end_nodes != current_branch.end_nodes) {
      auto join_end_nodes = [](const std::vector<std::string> &end_nodes)
          -> std::string {
        std::string text{};
...
[[nodiscard]] inline auto compare_restore_shapes(
    const graph_restore_shape &baseline,
...
[[nodiscard]] inline auto has_blocking_diff(
    const restore_diff &restore_diff) noexcept
    -> bool {
...
[[nodiscard]] inline auto validate(const graph_restore_shape &current_shape,
  ...
  for (const auto &[node_key, _] : checkpoint.rerun_inputs) {
    ...
    if (!valid_paths.contains(node_key)) {
      append_issue(validation, issue_kind::missing_rerun_input, node_key,
  ...
  for (const auto &interrupt_id : checkpoint.resume_snapshot.interrupt_ids()) {
    ...
    if (!path.has_value() || path->empty() || !valid_paths.contains(*path)) {
      append_issue(validation, issue_kind::missing_resume_target,
                   ...
                   "checkpoint resume target address no longer exists in the current graph");
    }
  }
  ...
  for (const auto &[interrupt_id, location] :
    ...
    if (!path.has_value() || path->empty() || !valid_paths.contains(*path)) {
      append_issue(validation, issue_kind::missing_interrupt_target,
                   location.to_string(),
                   "checkpoint interrupt target address no longer exists in the current graph");
    }
  }
