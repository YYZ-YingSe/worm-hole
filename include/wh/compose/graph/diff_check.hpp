...
[[nodiscard]] inline auto exec_mode_text(const node_exec_mode mode)
    -> std::string_view {
  return mode == node_exec_mode::async ? "async" : "sync";
}

[[nodiscard]] inline auto exec_origin_text(const node_exec_origin origin)
    -> std::string_view {
...
[[nodiscard]] inline auto join_strings(const std::vector<std::string> &values)
    -> std::string {
  std::string text{};
...
[[nodiscard]] inline auto graph_subject(const std::string_view prefix)
    -> std::string {
  if (prefix.empty()) {
    return "graph";
  }
...
[[nodiscard]] inline auto join_subject(const std::string_view prefix,
...
[[nodiscard]] inline auto edge_label(const std::string_view from,
...
inline auto append_diff(graph_diff &diff, const graph_diff_kind kind,
                        std::string subject, std::string detail,
                        std::string before, std::string after) -> void {
...
inline auto mark_incompatible(bool &incompatible, std::string &message,
...
inline auto compare_node_options(graph_diff &diff,
  ...
  const auto compare_policy = [&](const std::string_view detail,
    ...
    append_diff(diff, graph_diff_kind::node_policy, std::string{subject},
                std::string{detail}, std::move(before), std::move(after));
    if (breaking) {
  ...
  const auto compare_signature = [&](const std::string_view detail,
                                     std::string before,
                                     std::string after) -> void {
    if (before == after) {
      return;
    }
    append_diff(diff, graph_diff_kind::node_signature, std::string{subject},
                std::string{detail}, std::move(before), std::move(after));
    mark_incompatible(incompatible, message,
  ...
  compare_signature("input_key", baseline.options.input_key,
                    candidate.options.input_key);
  compare_signature("output_key", baseline.options.output_key,
                    candidate.options.output_key);
  ...
  compare_policy("exec_origin",
                 std::string{exec_origin_text(baseline.exec_origin)},
                 std::string{exec_origin_text(candidate.exec_origin)});
  compare_policy("input_contract_kind",
                 std::string{to_string(baseline.input_contract)},
                 std::string{to_string(candidate.input_contract)}, true);
  compare_policy("output_contract_kind",
                 std::string{to_string(baseline.output_contract)},
  ...
  compare_policy("rerun_input_contract",
                 std::to_string(baseline.options.input_contract),
                 std::to_string(candidate.options.input_contract));
  compare_policy("allow_no_control",
                 bool_text(baseline.options.allow_no_control),
  ...
  compare_policy("timeout_override_ms",
                 duration_text(baseline.options.timeout_override),
                 duration_text(candidate.options.timeout_override));
  compare_policy("retry_window_override_ms",
                 duration_text(baseline.options.retry_window_override),
  ...
  compare_policy("state_handlers.post",
                 bool_text(baseline.options.state_handlers.post),
                 bool_text(candidate.options.state_handlers.post));
  compare_policy("state_handlers.stream_pre",
                 bool_text(baseline.options.state_handlers.stream_pre),
...
inline auto compare_graph_snapshots(graph_diff &diff,
  ...
  const auto compare_compile_option = [&](const std::string_view detail,
    ...
    append_diff(diff, graph_diff_kind::compile_option, graph_name,
                std::string{detail}, std::move(before), std::move(after));
  };

  compare_compile_option("name", baseline.compile_options.name,
                         candidate.compile_options.name);
  ...
  compare_compile_option("retry_budget",
                         size_text(baseline.compile_options.retry_budget),
                         size_text(candidate.compile_options.retry_budget));
  compare_compile_option("node_timeout_ms",
                         duration_text(baseline.compile_options.node_timeout),
  ...
  while (baseline_node_index < baseline.nodes.size() ||
    ...
    if (candidate_node_index == candidate.nodes.size() ||
      ...
      append_diff(diff, graph_diff_kind::node_removed,
                  join_subject(prefix, baseline.nodes[baseline_node_index].key),
                  "node", "present", "absent");
      ++baseline_node_index;
      continue;
    }
    ...
    compare_node_options(diff, subject, baseline_node, candidate_node,
    ...
    if (baseline_subgraph == baseline.subgraphs.end() &&
      ...
      append_diff(diff, graph_diff_kind::node_policy, subject, "subgraph",
                  "absent", "present");
  ...
  while (baseline_edge_index < baseline.edges.size() ||
    ...
    const auto baseline_key =
        baseline_edge_index < baseline.edges.size()
    ...
    const auto subject =
        join_subject(prefix, edge_label(baseline_edge.from, baseline_edge.to));
    const auto compare_edge_topology = [&](const std::string_view detail,
                                           std::string before,
                                           std::string after) -> void {
      if (before == after) {
        return;
      }
      append_diff(diff, graph_diff_kind::edge_topology, std::string{subject},
                  std::string{detail}, std::move(before), std::move(after));
    };
    const auto compare_edge_policy = [&](const std::string_view detail,
                                         std::string before,
                                         std::string after) -> void {
      if (before == after) {
        return;
      }
      append_diff(diff, graph_diff_kind::edge_policy, std::string{subject},
                  std::string{detail}, std::move(before), std::move(after));
    };
  ...
  while (baseline_branch_index < baseline.branches.size() ||
    ...
    if (candidate_branch_index == candidate.branches.size() ||
      ...
      append_diff(
          diff, graph_diff_kind::branch_destinations,
          join_subject(prefix, baseline.branches[baseline_branch_index].from),
          "destinations",
          join_strings(baseline.branches[baseline_branch_index].end_nodes),
          "absent");
...
compare_graph_snapshots(const graph_snapshot &baseline,
  ...
  compare_graph_snapshots(report.diff, baseline, candidate, "",
                          report.compatible = true, report.message);
  return report;
}
