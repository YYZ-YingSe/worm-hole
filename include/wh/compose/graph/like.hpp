...
concept graph_viewable =
    std::same_as<std::remove_cvref_t<graph_t>, graph> || requires(const graph_t &value) {
...
concept graph_owning =
    (std::same_as<std::remove_cvref_t<graph_t>, graph> && std::is_rvalue_reference_v<graph_t &&>) ||
    requires(graph_t &&value) {
      { std::forward<graph_t>(value).release_graph() } -> std::same_as<wh::compose::graph>;
    };
...
[[nodiscard]] auto start_nested_graph(const graph &graph, wh::core::run_context &context,
...
template <typename graph_t>
  requires graph_viewable<graph_t>
[[nodiscard]] inline auto start_nested_graph(const graph_t &graph, wh::core::run_context &context,
                                             graph_value &input, const node_runtime &runtime)
    -> graph_sender {
