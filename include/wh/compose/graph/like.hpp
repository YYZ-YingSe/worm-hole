...
concept graph_viewable = std::same_as<std::remove_cvref_t<graph_t>, graph> ||
                         requires(const graph_t &value) {
                           {
                             value.graph_view()
                           } -> std::same_as<const wh::compose::graph &>;
                         };

/// Surface that can relinquish ownership of one materialized graph instance.
template <typename graph_t>
concept graph_owning = (std::same_as<std::remove_cvref_t<graph_t>, graph> &&
...
[[nodiscard]] auto
start_nested_graph(const graph &graph, wh::core::run_context &context,
...
start_nested_graph(const graph_t &graph, wh::core::run_context &context,
                   graph_value &input, const node_runtime &runtime)
    -> graph_sender {
