...
private:
  ...
  [[nodiscard]] static auto normalize_options(options_t &&options)
      -> graph_compile_options {
...
public:
  ...
  auto add_lambda(key_t &&key, lambda_t &&lambda, options_t &&options = {})
      -> wh::core::result<void> {
    return graph_.add_lambda<From, To, Exec>(std::forward<key_t>(key),
  ...
  auto add_component(key_t &&key, component_t &&component,
    ...
    return graph_.add_component<Kind, From, To, Exec>(
  ...
  auto add_edge(from_t &&from, to_t &&to, edge_options options = {})
      -> wh::core::result<void> {
    return graph_.add_edge(std::forward<from_t>(from), std::forward<to_t>(to),
                           std::move(options));
  }
