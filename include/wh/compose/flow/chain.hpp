// Defines linear compose builder that lowers append operations into graph IR.
#pragma once
...
#include "wh/compose/flow/branch.hpp"
#include "wh/compose/graph.hpp"
#include "wh/compose/flow/parallel.hpp"
...
public:
  ...
  auto append(const branch &value) -> wh::core::result<void> {
    return append_branch(value);
  }

  /// Appends one branch from current single-tail predecessor.
  auto append(branch &&value) -> wh::core::result<void> {
  ...
  /// Appends one branch from current single-tail predecessor.
  auto append_branch(const branch &value) -> wh::core::result<void> {
    auto writable = ensure_writable();
    if (writable.has_error()) {
      return writable;
    }
  ...
  /// Appends one branch from current single-tail predecessor.
  auto append_branch(branch &&value) -> wh::core::result<void> {
    auto writable = ensure_writable();
    if (writable.has_error()) {
      return writable;
    }
