...
concept StagePayloadCallbackLike = wh::core::StagePayloadCallbackLike<callback_t>;

/// Returns true for callback stages that execute in reverse registration order.
[[nodiscard]] constexpr auto is_reverse_stage(const stage current_stage) noexcept -> bool {
  return wh::core::is_reverse_callback_stage(current_stage);
}

template <typename payload_t>
/// Builds an event view from mutable callback payload.
[[nodiscard]] inline auto make_event_view(payload_t &payload) noexcept -> event_view {
  return wh::core::make_callback_event_view(payload);
}

template <typename payload_t>
/// Builds an event view from const callback payload.
[[nodiscard]] inline auto make_event_view(const payload_t &payload) noexcept -> event_view {
  return wh::core::make_callback_event_view(payload);
}
...
[[nodiscard]] inline auto make_event_view(payload_t &&) noexcept -> event_view = delete;

template <typename payload_t>
/// Builds an owning event payload by type-erasing `payload`.
[[nodiscard]] inline auto make_event_payload(payload_t &&payload) -> event_payload {
...
[[nodiscard]] inline auto event_get_if(const event_payload &payload) -> const value_t * {
...
[[nodiscard]] inline auto event_as(const event_payload &payload) -> wh::core::result<value_t> {
  return wh::core::callback_event_as<value_t>(payload);
}

template <typename value_t>
/// Moves payload by value or returns `errc::type_mismatch`.
[[nodiscard]] inline auto event_as(event_payload &&payload) -> wh::core::result<value_t> {
