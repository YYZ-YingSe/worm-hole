...
[[nodiscard]] inline auto make_callback_config(timing_checker_t &&checker, name_t &&name = {})
    -> callback_config {
  return wh::internal::make_callback_config(std::forward<timing_checker_t>(checker),
                                            std::forward<name_t>(name));
}

template <typename... config_t>
/// Merges multiple callback configs into one effective config.
[[nodiscard]] inline auto merge_callback_config(config_t &&...configs) -> callback_config {
  return wh::internal::merge_callback_config(std::forward<config_t>(configs)...);
}
