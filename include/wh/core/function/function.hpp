...
template <typename signature_t,
          template <typename, template <typename, template <typename> class> class,
                    ...
                    template <typename> class> class storage_policy_t = owning_storage,
          ...
          typename error_policy_t = assert_on_error, std::size_t buffer_size_v = sizeof(void *),
          template <typename> class allocator_t = std::allocator>
class function;

template <typename signature_t>
/// Move-only function wrapper alias.
using move_only_function = function<signature_t, owning_storage, deep_copy, non_copyable_accept>;

template <typename signature_t>
/// Non-owning callable reference wrapper alias.
using function_ref =
    function<signature_t, ref_only_storage, fn_detail::local_ownership, standard_accept>;

template <typename signature_t>
/// `std::function`-like wrapper alias (throw-on-error policy).
using standard_function = function<signature_t, owning_storage, deep_copy, standard_accept,
                                   throw_on_error, fn_detail::member_pointer_size>;

template <typename signature_t>
/// Callback-oriented callable wrapper alias.
using callback_function = function<signature_t, owning_storage, deep_copy, standard_accept,
...
inline constexpr bool
    is_function_v<function<signature_t, storage_policy_t, ownership_policy_t, acceptance_policy_t,
                           error_policy_t, buffer_size_v, allocator_t>> = true;
...
#define WH_DEFINE_FUNCTION(CV, REF, INV_QUALS)                                                     \
  ...
  public:                                                                                          \
    ...
    static constexpr bool using_soo = storage::template traits<fun_t>::using_soo;                  \
    ...
    function(fun_t &&fun) noexcept(base::template check_nothrow<decayed_fun_t, fun_t>())           \
    ...
    explicit function(std::in_place_type_t<fun_t>,                                                 \
                      args_t &&...args) noexcept(base::template check_nothrow<fun_t, args_t...>()) \
    ...
    template <typename fun_t, typename type_t, typename... args_t>                                 \
      requires(                                                                                    \
          fn_detail::is_direct_constructible_v<fun_t, std::initializer_list<type_t> &,             \
                                               args_t...> &&                                       \
          storage::template can_create_from<fun_t, std::initializer_list<type_t> &, args_t...>())  \
    explicit function(                                                                             \
        ...
        args_t &&...args) noexcept(base::template check_nothrow<fun_t,                             \
                                                                ...
                                                                args_t...>())                      \
        : base(std::in_place_type<fun_t>, init_list, std::forward<args_t>(args)...) {              \
      static_assert(std::is_same_v<std::decay_t<fun_t>, fun_t>);                                   \
      static_assert(storage::template is_invocable_v<std::decay_t<fun_t>>);                        \
    }                                                                                              \
    template <typename fun_t, typename decayed_fun_t = std::decay_t<fun_t>>                        \
    ...
    auto operator=(fun_t &&fun) noexcept(base::template check_nothrow<decayed_fun_t, fun_t>())     \
        -> function & {                                                                            \
    ...
    auto operator()(param_types... params) CV REF noexcept(is_noexcept) -> return_t {              \
      ...
      return std::invoke(storage::access(std::forward<buffer_inv_quals>(local_buffer_)),           \
                         std::forward<param_types>(params)...);                                    \
    }                                                                                              \
    auto swap(function &other) noexcept -> void { base::swap(other); }                             \
    friend auto swap(function &lhs, function &rhs) noexcept -> void { lhs.swap(rhs); }             \
    friend auto operator==(const function &lhs, std::nullptr_t) noexcept -> bool {                 \
