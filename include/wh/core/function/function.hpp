...
template <typename signature_t,
          ...
          class storage_policy_t = owning_storage,
          template <typename, template <typename> class> class
              ownership_policy_t = reference_counting,
...
using function_ref =
    function<signature_t, ref_only_storage, fn_detail::local_ownership,
             standard_accept>;
...
template <typename signature_t,
          template <typename,
                    ...
                    template <typename> class, class, std::size_t,
                    template <typename> class>
          class storage_policy_t,
          template <typename, template <typename> class> class ownership_policy_t,
          template <typename> class acceptance_policy_t,
          typename error_policy_t, std::size_t buffer_size_v,
...
#define WH_DEFINE_FUNCTION(CV, REF, INV_QUALS)                                 \
  ...
  public:                                                                      \
    ...
    template <typename fun_t, typename... args_t>                              \
      requires std::is_constructible_v<fun_t, args_t...> &&                    \
               storage::template is_invocable_v<std::decay_t<fun_t>>           \
    ...
    explicit function(                                                         \
        ...
        args_t &&...args) noexcept(base::template check_nothrow<               \
                                   fun_t, std::initializer_list<type_t> &,     \
                                   args_t...>())                               \
