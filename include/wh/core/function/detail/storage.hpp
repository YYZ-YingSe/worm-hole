...
template <std::size_t size_v> union any_storage {
  ...
  [[nodiscard]] auto address() const noexcept -> const void * { return &data_[0]; }
  ...
  [[nodiscard]] auto interpret_as() const noexcept -> type_t {
    static_assert(sizeof(type_t) <= size_v, "any_storage is smaller than the requested object!");
  ...
  template <typename type_t> [[nodiscard]] auto interpret_as() noexcept -> type_t {
