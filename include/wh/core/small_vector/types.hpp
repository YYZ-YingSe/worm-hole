...
using default_small_vector = small_vector<value_t, inline_capacity, allocator_t, options_t>;
...
[[nodiscard]] auto describe_capacity(
    const small_vector<value_t, inline_capacity, allocator_t, options_t> &value) noexcept
    -> small_vector_capacity_snapshot {
  return small_vector_capacity_snapshot{value.size(), value.capacity(), inline_capacity};
}

/// Returns compile-time growth policy as runtime metadata.
template <typename value_t, std::size_t inline_capacity, typename allocator_t, typename options_t>
[[nodiscard]] constexpr auto describe_growth_policy(
    const small_vector<value_t, inline_capacity, allocator_t, options_t> &) noexcept
    -> small_vector_growth_policy {
  return small_vector_growth_policy{options_t::minimum_dynamic_capacity,
                                    options_t::growth_numerator, options_t::growth_denominator,
                                    options_t::heap_enabled, options_t::shrink_to_inline};
}

/// Returns behavioral contract for the concrete small-vector type.
template <typename value_t, std::size_t inline_capacity, typename allocator_t, typename options_t>
[[nodiscard]] constexpr auto
describe_contract(const small_vector<value_t, inline_capacity, allocator_t, options_t> &) noexcept
    -> small_vector_contract {
