...
[[nodiscard]] constexpr auto is_reverse_callback_stage(const callback_stage stage) noexcept
    -> bool {
...
template <typename payload_t>
[[nodiscard]] inline auto make_callback_event_view(const payload_t &value) noexcept
    -> callback_event_view {
  return forward_as_any(value);
}
...
[[nodiscard]] inline auto make_callback_event_payload(value_t &&value) -> callback_event_payload {
  return any{std::forward<value_t>(value)};
}

/// Returns typed payload pointer when stored type matches `value_t`.
template <typename value_t>
[[nodiscard]] inline auto callback_event_get_if(const callback_event_payload &extra)
    -> const value_t * {
...
template <typename value_t>
[[nodiscard]] inline auto callback_event_get_if(callback_event_payload &extra) -> value_t * {
  return any_cast<value_t>(&extra);
}
...
template <typename value_t>
[[nodiscard]] inline auto callback_event_as(callback_event_payload &&extra) -> result<value_t> {
...
template <typename value_t>
[[nodiscard]] inline auto callback_event_cref_as(const callback_event_payload &extra)
...
using callback_timing_checker = wh::core::callback_function<bool(callback_stage) const>;
...
struct callback_run_metadata {
  ...
  [[nodiscard]] auto empty() const noexcept -> bool {
    return trace_id.empty() && span_id.empty() && parent_span_id.empty() && node_path.empty();
  }
};

/// Applies callback metadata on top of one emitted run-info payload.
[[nodiscard]] inline auto apply_callback_run_metadata(callback_run_info run_info,
...
[[nodiscard]] inline auto apply_component_run_info(callback_run_info run_info,
...
using stage_view_callback = wh::core::callback_function<void(callback_stage, callback_event_view,
                                                             const callback_run_info &) const>;
/// Single-consumer callback signature for owning payload dispatch.
using stage_payload_callback = wh::core::callback_function<void(
    callback_stage, callback_event_payload &&, const callback_run_info &) const>;
...
struct callback_fatal_error {
  ...
  [[nodiscard]] auto to_string() const -> std::string {
    return std::string{"code="} + code.message() + ", exception=" + exception_message +
           ", stack=" + call_stack;
  }
};
