...
concept TimingChecker = requires(const timing_checker_t checker, const callback_stage stage) {
...
concept StageViewCallbackLike =
    requires(callback_t callback, const callback_stage stage, const callback_event_view event,
             const callback_run_info &info) {
...
concept StagePayloadCallbackLike =
    requires(callback_t callback, const callback_stage stage, callback_event_payload payload,
             const callback_run_info &info) {
...
concept CallbackConfigLike = requires(const config_t config, const callback_stage stage) {
