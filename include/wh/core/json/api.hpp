...
[[nodiscard]] inline auto json_kind(const json_value &value) noexcept -> json_value_kind {
  if (value.IsNull()) {
...
[[nodiscard]] inline auto parse_json(const std::string_view text) -> result<json_document> {
...
[[nodiscard]] inline auto parse_json_with_error(const std::string_view text)
  ...
  document.Parse(text.data(), text.size());
  if (document.HasParseError()) {
    const json_parse_error parse_error{document.GetParseError(), document.GetErrorOffset()};
...
[[nodiscard]] inline auto json_to_string(const json_value &value) -> result<std::string> {
...
[[nodiscard]] inline auto json_find_member(const json_value &value, const std::string_view key)
...
[[nodiscard]] inline auto json_at(const json_value &value, const std::size_t index)
    -> result<const json_value *> {
  if (!value.IsArray()) {
    return result<const json_value *>::failure(errc::type_mismatch);
  }
...
[[nodiscard]] inline auto parse_error_message(const json_parse_error &error) -> std::string {
