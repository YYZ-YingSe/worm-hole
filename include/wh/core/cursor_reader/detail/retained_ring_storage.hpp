...
public:
  ...
  retained_ring_storage(retained_ring_storage &&other) noexcept
      : allocator_(std::move(other.allocator_)), capacity_(other.capacity_),
        ...
        head_sequence_(other.head_sequence_), storage_(other.storage_) {
    other.capacity_ = 0U;
    other.size_ = 0U;
    other.head_ = 0U;
  ...
  [[nodiscard]] auto front_sequence() const noexcept -> std::uint64_t {
    ...
    requires std::copy_constructible<value_type>
  {
    ...
    try {
      for (std::size_t offset = 0U; offset < size_; ++offset) {
        allocator_traits::construct(allocator_, new_storage + offset,
...
private:
  ...
  [[nodiscard]] auto contains(const std::uint64_t sequence) const noexcept
      -> bool {
  ...
  [[nodiscard]] auto slot_index(const std::uint64_t sequence) const noexcept
      -> std::size_t {
    const auto offset =
        static_cast<std::size_t>(sequence - head_sequence_);
