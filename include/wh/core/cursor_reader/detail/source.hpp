...
using cursor_reader_result_t = std::remove_cvref_t<decltype(std::declval<source_t &>().read())>;
...
template <typename result_t> struct try_result_traits<result_t, result_t> {
  [[nodiscard]] static constexpr auto is_pending(const result_t &) noexcept -> bool {
    return false;
  }

  [[nodiscard]] static auto project(result_t status) noexcept -> result_t { return status; }
};

template <typename result_t> struct try_result_traits<std::optional<result_t>, result_t> {
  [[nodiscard]] static auto is_pending(const std::optional<result_t> &status) noexcept -> bool {
    return !status.has_value();
  }

  [[nodiscard]] static auto project(std::optional<result_t> status) noexcept -> result_t {
...
struct try_result_traits<std::variant<option_ts...>, result_t> {
  ...
  [[nodiscard]] static auto is_pending(const std::variant<option_ts...> &status) noexcept -> bool {
    return !std::holds_alternative<result_t>(status);
  }

  [[nodiscard]] static auto project(std::variant<option_ts...> status) noexcept -> result_t {
...
concept try_result_like = requires(try_t status) {
  ...
  {
    try_result_traits<std::remove_cvref_t<try_t>, result_t>::project(std::move(status))
  } -> std::same_as<result_t>;
};

template <typename error_t>
[[nodiscard]] inline auto to_exception_ptr(error_t &&error) noexcept -> std::exception_ptr {
...
concept cursor_reader_source = requires(source_t &source, const source_t &) {
  ...
  requires wh::core::cursor_reader_detail::try_result_like<cursor_reader_source_try_t<source_t>,
                                                           cursor_reader_result_t<source_t>>;
...
template <typename source_t>
concept async_source = wh::core::cursor_reader_source<source_t> && requires(source_t &source) {
...
template <typename source_t> struct default_policy {
  ...
  [[nodiscard]] static auto is_terminal(const result_type &status) noexcept -> bool {
    return status.has_error();
  }

  [[nodiscard]] static auto is_pending(const source_try_type &status) noexcept -> bool {
    return try_result_traits<source_try_type, result_type>::is_pending(status);
  }

  [[nodiscard]] static auto project_try(source_try_type status) noexcept -> result_type {
    return try_result_traits<source_try_type, result_type>::project(std::move(status));
  }

  [[nodiscard]] static auto pending() noexcept -> try_result_type { return std::nullopt; }
...
concept policy_for =
    ...
    requires(source_t &source, const wh::core::cursor_reader_result_t<source_t> &status,
             const wh::core::cursor_reader_source_try_t<source_t> &try_status, const bool enabled) {
      ...
      { policy_t::pending() } -> std::same_as<typename policy_t::try_result_type>;
      {
        policy_t::ready(std::declval<typename policy_t::result_type>())
      } -> std::same_as<typename policy_t::try_result_type>;
      { policy_t::closed_result() } -> std::same_as<typename policy_t::result_type>;
      { policy_t::internal_result() } -> std::same_as<typename policy_t::result_type>;
