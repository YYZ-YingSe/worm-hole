...
[[nodiscard]] constexpr auto resume_if(sender_t &&sender, resume_passthrough_t) {
  static_assert(Mode == wh::core::resume_mode::unchanged);
  return std::forward<sender_t>(sender);
}

template <wh::core::resume_mode Mode, stdexec::sender sender_t, stdexec::scheduler scheduler_t>
[[nodiscard]] constexpr auto resume_if(sender_t &&sender, scheduler_t scheduler) {
  if constexpr (Mode == wh::core::resume_mode::restore) {
    return wh::core::resume_on(std::forward<sender_t>(sender), std::move(scheduler));
  } else {
    return std::forward<sender_t>(sender);
  }
}
...
[[nodiscard]] constexpr auto defer_resume_sender(factory_t &&factory) {
  ...
  if constexpr (Mode == wh::core::resume_mode::restore) {
    return wh::core::read_resume_scheduler(
        [factory = stored_factory_t{std::forward<factory_t>(factory)}](auto scheduler) mutable {
          return std::invoke(factory, std::move(scheduler));
        });
  } else {
    return defer_sender([factory = stored_factory_t{std::forward<factory_t>(factory)}]() mutable {
