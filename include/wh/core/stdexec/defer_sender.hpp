...
public:
  ...
  STDEXEC_EXPLICIT_THIS_BEGIN(auto connect)(this self_t &&self,
    ...
    return operation<receiver_t>{std::forward<self_t>(self).factory_,
                                 std::move(receiver)};
  }
  ...
  template <typename self_t, typename... env_t>
    requires std::same_as<std::remove_cvref_t<self_t>, defer_sender_impl> &&
             (!std::is_const_v<std::remove_reference_t<self_t>>) &&
             (sizeof...(env_t) == 1U) && std::invocable<factory_t &> &&
             stdexec::sender<
                 std::remove_cvref_t<std::invoke_result_t<factory_t &>>>
...
template <typename factory_t>
[[nodiscard]] constexpr auto defer_sender(factory_t &&factory) {
  return defer_sender_impl<std::remove_cvref_t<factory_t>>{
