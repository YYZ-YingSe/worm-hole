...
[[nodiscard]] constexpr auto map_result_value(mapped_t &&mapped) -> next_result_t {
...
template <typename next_result_t, typename mapper_t, typename error_t>
  requires result_like<next_result_t>
[[nodiscard]] constexpr auto map_result_status(mapper_t &&mapper,
                                               wh::core::result<void, error_t> status)
    -> next_result_t {
  static_assert(std::convertible_to<error_t, typename next_result_t::error_type>,
                "map_result_sender requires compatible result error types");
  if (status.has_error()) {
    return next_result_t::failure(std::move(status).error());
  }

  static_assert(std::invocable<mapper_t>, "map_result_sender requires a zero-argument mapper for "
                                          "result<void> input");
  ...
  if constexpr (std::same_as<mapped_t, void>) {
    ...
    return map_result_value<next_result_t>(std::invoke(std::forward<mapper_t>(mapper)));
  }
}

template <typename next_result_t, typename mapper_t, typename value_t, typename error_t>
  requires result_like<next_result_t>
[[nodiscard]] constexpr auto map_result_status(mapper_t &&mapper,
                                               wh::core::result<value_t, error_t> status)
    -> next_result_t {
  static_assert(std::convertible_to<error_t, typename next_result_t::error_type>,
                "map_result_sender requires compatible result error types");
  if (status.has_error()) {
...
template <typename next_result_t, stdexec::sender sender_t, typename mapper_t>
  requires result_like<next_result_t>
[[nodiscard]] constexpr auto map_result_sender(sender_t &&sender, mapper_t &&mapper) {
  return std::forward<sender_t>(sender) | stdexec::then([mapper = std::forward<mapper_t>(mapper)](
           ...
           return map_result_status<next_result_t>(std::move(mapper), std::move(status));
         });
}
