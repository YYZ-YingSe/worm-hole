...
concept env_with_completion_scheduler = requires(const std::remove_cvref_t<env_t> &env) {
  stdexec::get_completion_scheduler<cpo_t>(env);
};

template <typename env_t>
concept env_with_scheduler =
    requires(const std::remove_cvref_t<env_t> &env) { stdexec::get_scheduler(env); };
...
template <typename cpo_t, typename env_t,
          bool use_completion_scheduler = env_with_completion_scheduler<cpo_t, env_t>>
...
struct resume_scheduler_selector<cpo_t, env_t, true> {
  using type = std::remove_cvref_t<decltype(stdexec::get_completion_scheduler<cpo_t>(
      std::declval<const env_t &>()))>;

  [[nodiscard]] static constexpr auto
  get(const env_t &env) noexcept(noexcept(stdexec::get_completion_scheduler<cpo_t>(env))) -> type {
    return stdexec::get_completion_scheduler<cpo_t>(env);
  }
};

template <typename cpo_t, typename env_t>
  requires(!env_with_completion_scheduler<cpo_t, env_t> && env_with_scheduler<env_t>)
struct resume_scheduler_selector<cpo_t, env_t, false> {
  using type = std::remove_cvref_t<decltype(stdexec::get_scheduler(std::declval<const env_t &>()))>;
...
[[nodiscard]] constexpr auto select_resume_scheduler(const env_t &env) noexcept(
    noexcept(resume_scheduler_selector<cpo_t, std::remove_cvref_t<env_t>>::get(env)))
    -> selected_resume_scheduler_t<cpo_t, env_t> {
  return resume_scheduler_selector<cpo_t, std::remove_cvref_t<env_t>>::get(env);
}

template <typename env_t>
using resume_scheduler_t = selected_resume_scheduler_t<stdexec::set_value_t, env_t>;
...
concept env_with_launch_scheduler =
    env_with_scheduler<env_t> || env_with_completion_scheduler<stdexec::set_value_t, env_t>;
...
template <typename env_t>
  requires(!env_with_scheduler<env_t> && env_with_completion_scheduler<stdexec::set_value_t, env_t>)
struct launch_scheduler_selector<env_t, false> {
  using type = std::remove_cvref_t<decltype(stdexec::get_completion_scheduler<stdexec::set_value_t>(
      std::declval<const env_t &>()))>;

  [[nodiscard]] static constexpr auto get(const env_t &env) noexcept(
      noexcept(stdexec::get_completion_scheduler<stdexec::set_value_t>(env))) -> type {
...
template <typename env_t> using launch_scheduler_t = selected_launch_scheduler_t<env_t>;

struct get_launch_scheduler_t {
  template <typename env_t>
    requires env_with_launch_scheduler<env_t>
  [[nodiscard]] constexpr auto operator()(const env_t &env) const
      noexcept(noexcept(select_launch_scheduler(env))) -> launch_scheduler_t<env_t> {
...
using any_resume_scheduler_t = exec::any_receiver_ref<
    stdexec::completion_signatures<stdexec::set_error_t(std::exception_ptr),
                                   stdexec::set_stopped_t()>>::any_sender<>::any_scheduler<>;

template <stdexec::scheduler scheduler_t>
[[nodiscard]] inline auto erase_resume_scheduler(scheduler_t scheduler) -> any_resume_scheduler_t {
...
struct async_completion_env {
  ...
  template <typename cpo_t, typename env_t>
    requires requires(const env_t &env) {
      stdexec::get_completion_domain<cpo_t>(select_resume_scheduler<cpo_t>(env), env);
    }
  [[nodiscard]] constexpr auto query(stdexec::get_completion_domain_t<cpo_t>,
                                     const env_t &env) const noexcept
      -> decltype(stdexec::get_completion_domain<cpo_t>(select_resume_scheduler<cpo_t>(env), env)) {
    return stdexec::get_completion_domain<cpo_t>(select_resume_scheduler<cpo_t>(env), env);
  }
};

template <typename receiver_t>
concept receiver_with_resume_scheduler =
    env_with_resume_scheduler<stdexec::set_value_t, stdexec::env_of_t<receiver_t>>;
...
template <typename receiver_t>
concept receiver_with_launch_scheduler = env_with_launch_scheduler<stdexec::env_of_t<receiver_t>>;

template <typename query_t> inline constexpr bool scheduler_query_v = false;

template <> inline constexpr bool scheduler_query_v<stdexec::get_scheduler_t> = true;

template <typename cpo_t>
inline constexpr bool scheduler_query_v<stdexec::get_completion_scheduler_t<cpo_t>> = true;

template <typename cpo_t>
inline constexpr bool scheduler_query_v<stdexec::get_completion_domain_t<cpo_t>> = true;
...
template <stdexec::scheduler scheduler_t> struct scheduler_query_env {
  ...
  [[nodiscard]] auto query(stdexec::get_scheduler_t) const noexcept -> const scheduler_t & {
    return *scheduler;
  }
...
template <typename outer_env_t, stdexec::scheduler scheduler_t> struct scheduler_env {
  ...
  template <typename query_t>
    requires(!scheduler_query_v<std::remove_cvref_t<query_t>> &&
             requires(const outer_env_t &env, const query_t &query) { query(env); })
  [[nodiscard]] auto query(const query_t &query) const noexcept(noexcept(query(*outer_env)))
      -> decltype(query(*outer_env)) {
    return query(*outer_env);
  }
};

template <stdexec::scheduler scheduler_t>
[[nodiscard]] constexpr auto make_scheduler_queries(const scheduler_t &scheduler) noexcept
    -> scheduler_query_env<scheduler_t> {
  return scheduler_query_env<scheduler_t>{.scheduler = &scheduler};
}

template <stdexec::scheduler scheduler_t> auto make_scheduler_queries(scheduler_t &&) = delete;

template <typename outer_env_t, stdexec::scheduler scheduler_t>
[[nodiscard]] constexpr auto make_scheduler_env(const outer_env_t &outer_env,
...
[[nodiscard]] constexpr auto write_sender_scheduler(sender_t &&sender,
                                                    const scheduler_t &scheduler) {
  return stdexec::write_env(std::forward<sender_t>(sender), make_scheduler_queries(scheduler));
}
...
[[nodiscard]] constexpr auto read_resume_scheduler(factory_t &&factory) {
  return stdexec::read_env(detail::get_resume_scheduler) |
         stdexec::let_value([factory = std::forward<factory_t>(factory)](auto scheduler) mutable {
...
[[nodiscard]] constexpr auto resume_on(sender_t &&sender, scheduler_t scheduler) {
  return stdexec::continues_on(std::forward<sender_t>(sender), std::move(scheduler));
}
