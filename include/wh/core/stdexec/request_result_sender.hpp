...
[[nodiscard]] constexpr auto request_result_sender(request_t &&request, call_t &&call) {
  ...
  if constexpr (std::is_lvalue_reference_v<request_t> ||
    ...
    if constexpr (std::invocable<call_t, request_t>) {
      return normalize_result_sender<result_t>(
          std::invoke(std::forward<call_t>(call), std::forward<request_t>(request)));
    } else {
    ...
    if constexpr (std::invocable<call_t, request_t>) {
      ...
      return request_result_sender<result_t>(static_cast<const request_value_t &>(request),
...
template <typename result_t, typename request_t, typename call_t>
  requires result_like<result_t>
[[nodiscard]] constexpr auto defer_request_result_sender(request_t &&request, call_t &&call) {
  ...
  return defer_result_sender<result_t>(
      [request = stored_request_t{std::forward<request_t>(request)},
       call = stored_call_t{std::forward<call_t>(call)}]() mutable {
        return request_result_sender<result_t>(std::move(request), std::move(call));
      });
}
