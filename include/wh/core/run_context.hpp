...
struct run_context {
  using session_store = std::unordered_map<std::string, wh::core::any, detail::session_string_hash,
...
[[nodiscard]] auto clone_run_context(const run_context &context) -> result<run_context>;

/// Stores a typed session value under `key`.
template <typename key_t, typename value_t>
  requires std::constructible_from<std::string, key_t &&>
auto set_session_value(run_context &context, key_t &&key, value_t &&value) -> result<void> {
  ...
  context.session_values.insert_or_assign(std::move(stored_key), std::move(owned).value());
  return {};
}

/// Gets an immutable typed reference from session storage.
template <typename value_t>
[[nodiscard]] auto session_value_ref(const run_context &context, const std::string_view key)
...
template <typename value_t>
[[nodiscard]] auto session_value_ref(run_context &context, const std::string_view key)
  ...
  if (typed == nullptr) {
    return result<std::reference_wrapper<value_t>>::failure(errc::type_mismatch);
  }
...
template <typename value_t>
[[nodiscard]] auto consume_session_value(run_context &context, const std::string_view key)
    -> result<value_t> {
  const auto iter = context.session_values.find(key);
  if (iter == context.session_values.end()) {
...
[[nodiscard]] inline auto has_callback_manager(const run_context &context) noexcept -> bool {
...
[[nodiscard]] inline auto register_local_callbacks(wh::core::run_context &&context,
                                                   config_t &&config, callbacks_t &&callbacks)
    -> wh::core::result<wh::core::run_context> {
  if (!context.callbacks.has_value()) {
    return wh::core::result<wh::core::run_context>::failure(wh::core::errc::not_found);
  }

  context.callbacks->manager.register_local_callbacks(std::forward<config_t>(config),
...
template <typename config_t, typename callbacks_t>
  requires wh::core::CallbackConfigLike<wh::core::remove_cvref_t<config_t>> &&
           std::same_as<wh::core::remove_cvref_t<callbacks_t>, wh::core::stage_callbacks>
[[nodiscard]] inline auto register_local_callbacks(const wh::core::run_context &context,
                                                   config_t &&config, callbacks_t &&callbacks)
    -> wh::core::result<wh::core::run_context> {
  ...
  return register_local_callbacks(std::move(copied).value(), std::forward<config_t>(config),
                                  std::forward<callbacks_t>(callbacks));
}
...
[[nodiscard]] inline auto register_local_callbacks(wh::core::run_context &&context,
  ...
  return register_local_callbacks(
      std::move(context),
      wh::internal::make_callback_config(std::forward<timing_checker_t>(timing_checker),
...
auto inject_callback_event(wh::core::run_context &context, const wh::core::callback_stage stage,
                           const payload_t &payload, const wh::core::callback_run_info &run_info)
    -> void {
  if (!context.callbacks.has_value()) {
    return;
  }
  ...
  if (!context.callbacks->metadata.empty()) {
    context.callbacks->manager.dispatch(
        ...
        wh::core::apply_callback_run_metadata(run_info, context.callbacks->metadata));
    return;
  }

  context.callbacks->manager.dispatch(stage, wh::core::make_callback_event_view(payload), run_info);
}
...
[[nodiscard]] inline auto capture_fatal_error() -> wh::core::callback_fatal_error {
  try {
    ...
    return wh::core::callback_fatal_error{wh::core::map_exception(error), error.what(),
                                          wh::internal::capture_call_stack()};
  } catch (...) {
    return wh::core::callback_fatal_error{wh::core::make_error(wh::core::errc::internal_error),
                                          "unknown", wh::internal::capture_call_stack()};
  }
}
...
[[nodiscard]] auto run_with_fatal_error_capture(callable_t &&callable)
  ...
  try {
    ...
    return wh::core::result<void, wh::core::callback_fatal_error>::failure(capture_fatal_error());
  }
}

/// Executes callable and emits fatal callback event when it throws.
template <typename callable_t>
auto run_with_fatal_event(
    wh::core::run_context &context, callable_t &&callable,
    const wh::core::callback_stage fatal_stage = wh::core::callback_stage::error) -> void {
  auto executed = run_with_fatal_error_capture(std::forward<callable_t>(callable));
  if (executed.has_value()) {
    return;
  }

  inject_callback_event(context, fatal_stage, executed.error(), wh::core::callback_run_info{});
}

template <> struct any_owned_traits<run_context> {
  [[nodiscard]] static auto into_owned(const run_context &value) -> result<run_context> {
  ...
  [[nodiscard]] static auto into_owned(run_context &&value) -> result<run_context> {
    auto session_values = wh::core::into_owned_any_map(std::move(value.session_values));
    if (session_values.has_error()) {
      return result<run_context>::failure(session_values.error());
    }
...
[[nodiscard]] inline auto clone_run_context(const run_context &context) -> result<run_context> {
