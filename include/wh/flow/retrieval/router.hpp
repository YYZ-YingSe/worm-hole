...
[[nodiscard]] inline auto join_names(const std::vector<std::string> &names) -> std::string {
  std::string joined{};
...
[[nodiscard]] inline auto make_callback_state(const std::string_view name,
...
concept route_policy = requires(route_t route, const wh::retriever::retriever_request &request,
                                const std::vector<std::string> &names) {
  { route(request, names) } -> std::same_as<wh::core::result<std::vector<std::string>>>;
};

template <typename fusion_t>
concept fusion_policy =
    requires(fusion_t fusion, const std::vector<routed_retriever_result> &results) {
...
template <retriever_component retriever_t, route_policy route_t, fusion_policy fusion_t>
struct authored_state {
  explicit authored_state(route_t route,
                          fusion_t fusion) noexcept(std::is_nothrow_move_constructible_v<route_t> &&
...
struct runtime_state {
  runtime_state(route_t route, fusion_t fusion,
                std::vector<retriever_binding<retriever_t>>
                    authored_retrievers) noexcept(std::is_nothrow_move_constructible_v<route_t> &&
                                                  std::is_nothrow_move_constructible_v<fusion_t> &&
...
make_retrieve_sender(const retriever_t &retriever, std::string retriever_name,
  ...
  if constexpr (requires(const retriever_t &value, wh::retriever::retriever_request &&owned_request,
    ...
    return wh::core::detail::map_result_sender<retrieve_status>(
        ...
        [retriever_name = std::move(retriever_name)](
            wh::retriever::retriever_response documents) mutable -> retrieve_status {
...
using retrieve_sender_t =
    decltype(make_retrieve_sender(std::declval<const retriever_t &>(), std::declval<std::string>(),
                                  std::declval<wh::retriever::retriever_request>(),
                                  std::declval<wh::core::run_context &>()));

template <retriever_component retriever_t, route_policy route_t, fusion_policy fusion_t>
[[nodiscard]] inline auto
build_route_plan(const runtime_state<retriever_t, route_t, fusion_t> &state,
                 const wh::retriever::retriever_request &request, wh::core::run_context &context)
    -> wh::core::result<route_plan> {
  auto sink = wh::callbacks::filter_callback_sink(wh::callbacks::borrow_callback_sink(context),
                                                  request.options);
  auto callback_state = make_callback_state(router_stage_name, request, request.query);
...
template <retriever_component retriever_t, route_policy route_t, fusion_policy fusion_t>
[[nodiscard]] inline auto
fuse_results(const runtime_state<retriever_t, route_t, fusion_t> &state, const route_plan &plan,
             std::vector<routed_retriever_result> results, wh::core::run_context &context)
    -> wh::core::result<wh::retriever::retriever_response> {
  auto sink = wh::callbacks::filter_callback_sink(wh::callbacks::borrow_callback_sink(context),
                                                  plan.request.options);
  auto callback_state =
      make_callback_state(fusion_stage_name, plan.request, std::to_string(results.size()));
  emit_callback(sink, wh::callbacks::stage::start, callback_state);
...
template <retriever_component retriever_t, route_policy route_t, fusion_policy fusion_t>
[[nodiscard]] inline auto
make_fanout_sender(std::shared_ptr<const runtime_state<retriever_t, route_t, fusion_t>> state,
  ...
  for (std::size_t offset = 0U; offset < plan.selected_indices.size(); ++offset) {
    ...
    senders.push_back(make_retrieve_sender(binding.retriever, binding.name, plan.request, context));
  }

  return wh::core::detail::make_concurrent_sender_vector<retrieve_status>(
             std::move(senders), plan.selected_indices.size()) |
         stdexec::then([state, plan = std::move(plan),
                        &context](std::vector<retrieve_status> statuses) mutable -> output_status {
...
template <retriever_component retriever_t, route_policy route_t, fusion_policy fusion_t>
[[nodiscard]] inline auto
make_pipeline_sender(std::shared_ptr<const runtime_state<retriever_t, route_t, fusion_t>> state,
                     wh::retriever::retriever_request request, wh::core::run_context &context) {
  ...
  using fanout_sender_t = decltype(make_fanout_sender(
      std::declval<std::shared_ptr<const runtime_state<retriever_t, route_t, fusion_t>>>(),
  ...
  auto route_sender = stdexec::just() | stdexec::then([state, request = std::move(request),
                                                       &context]() mutable -> route_status {
                        return build_route_plan(*state, request, context);
                      });

  return stdexec::let_value(std::move(route_sender), [state,
                                                      &context](route_status routed) mutable {
    if (routed.has_error()) {
      return route_dispatch_sender_t{stdexec::just(output_status::failure(routed.error()))};
    }
    return route_dispatch_sender_t{make_fanout_sender(state, std::move(routed).value(), context)};
  });
}
...
template <detail::router::retriever_component retriever_t,
          detail::router::route_policy route_t = detail::router::route_all_retrievers,
          detail::router::fusion_policy fusion_t = detail::router::reciprocal_rank_fusion>
class router {
  using authored_state_t = detail::router::authored_state<retriever_t, route_t, fusion_t>;
  using runtime_state_t = detail::router::runtime_state<retriever_t, route_t, fusion_t>;

public:
  router()
    requires std::default_initializable<route_t> && std::default_initializable<fusion_t>
      : router(route_t{}, fusion_t{}) {}

  explicit router(route_t route_policy, fusion_t fusion_policy = {}) noexcept
      : authored_(std::in_place, std::move(route_policy), std::move(fusion_policy)) {}
  ...
  auto add_retriever(std::string name, retriever_t retriever) -> wh::core::result<void> {
  ...
  auto freeze() -> wh::core::result<void> {
    ...
    runtime_ = std::make_shared<runtime_state_t>(std::move(authored_->route_policy),
                                                 std::move(authored_->fusion_policy),
...
private:
  ...
  [[nodiscard]] auto dispatch_request(request_t &&request, wh::core::run_context &context) const {
    ...
    using pipeline_sender_t = decltype(detail::router::make_pipeline_sender(
        ...
        std::declval<wh::retriever::retriever_request>(), std::declval<wh::core::run_context &>()));
    ...
    return dispatch_sender_t{detail::router::make_pipeline_sender(
        runtime_, wh::retriever::retriever_request{std::forward<request_t>(request)}, context)};
  }
