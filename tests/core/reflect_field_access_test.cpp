...
template <>
struct type_alias<registry_alpha> {
  static constexpr std::string_view value{"registry_alpha"};
};

template <>
struct type_alias<registry_beta> {
  static constexpr std::string_view value{"registry_beta"};
};

}  // namespace wh::internal

TEST_CASE("reflect field_map validates and exposes metadata", "[core][reflect][condition]") {
  constexpr auto id_field = wh::core::field("id", &user_profile::id);
  constexpr auto name_field = wh::core::field("name", &user_profile::name);

  static_assert(wh::core::validate_field_map<user_profile>(id_field, name_field));

  constexpr auto field_map = wh::core::make_field_map<user_profile>(id_field, name_field);
...
TEST_CASE("reflect visit_field supports found and not_found branches", "[core][reflect][branch]") {
  constexpr auto id_field = wh::core::field("id", &user_profile::id);
  constexpr auto name_field = wh::core::field("name", &user_profile::name);
  constexpr auto field_map = wh::core::make_field_map<user_profile>(id_field, name_field);
  ...
  const bool found_id = wh::core::visit_field(field_map, "id", [&](const auto& binding) {
    using field_t = typename std::remove_cvref_t<decltype(binding)>::value_type;
    if constexpr (std::is_same_v<field_t, int>) {
      wh::core::field_ref(profile, binding) = 42;
    }
  });

  const bool found_missing = wh::core::visit_field(field_map, "missing", [&](const auto&) {
...
TEST_CASE("reflect key lookup stays stable", "[core][reflect][extreme]") {
  ...
  const bool found_by_key = wh::core::visit_field_by_key(field_map, key, [&](const auto& binding) {
    using field_t = typename std::remove_cvref_t<decltype(binding)>::value_type;
...
TEST_CASE("reflect type key registry lookup contract", "[core][reflect][extreme]") {
