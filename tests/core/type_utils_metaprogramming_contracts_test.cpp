...
TEST_CASE("type_utils metaprogramming traits contract", "[core][type_utils][condition]") {
  static_assert(std::same_as<wh::core::type_of_t<const int&>, int>);
  static_assert(std::same_as<decltype(wh::core::type_of<const int&>()), wh::core::type_tag<int>>);
  ...
  static_assert(std::same_as<wh::core::callable_result_t<sample_functor, int, double>, long>);

  using args_t = wh::core::function_argument_types_t<sample_functor>;
  static_assert(wh::core::type_list_size<args_t>::value == 2);
  static_assert(std::same_as<typename wh::core::type_list_at<0, args_t>::type, int>);
  static_assert(std::same_as<typename wh::core::type_list_at<1, args_t>::type, double>);

  using reversed_t = typename wh::core::type_list_reverse<args_t>::type;
  static_assert(std::same_as<typename wh::core::type_list_at<0, reversed_t>::type, double>);
  static_assert(std::same_as<typename wh::core::type_list_at<1, reversed_t>::type, int>);

  SUCCEED();
}

TEST_CASE("type_utils object helpers branch behavior", "[core][type_utils][branch]") {
...
TEST_CASE("type_utils default instance for container edge path", "[core][type_utils][extreme]") {
