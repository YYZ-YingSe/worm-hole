...
TEST_CASE(
    "loader callback event defaults to empty uri and zero loaded bytes",
...
TEST_CASE("loader callback event aggregate initialization survives copy and move",
          "[UT][wh/document/"
          "callback_event_loader.hpp][loader_callback_event][condition][branch][boundary]") {
  const wh::document::loader_callback_event source{.uri = "file:///tmp/a.txt", .loaded_bytes = 42U};
