...
struct concept_memory_pool {
  auto acquire(const wh::net::memory_acquire_request &) const -> wh::net::memory_acquire_result;
  auto acquire(wh::net::memory_acquire_request &&) const -> wh::net::memory_acquire_result;
...
TEST_CASE("memory pool concept accepts complete pool adapters",
          "[UT][wh/net/concepts/"
          "memory_pool_concept.hpp][memory_pool_like][condition][branch][boundary]") {
...
TEST_CASE("memory pool concept rejects adapters missing release handling",
          "[UT][wh/net/concepts/"
          "memory_pool_concept.hpp][memory_pool_like][condition][branch][boundary][negative]") {
