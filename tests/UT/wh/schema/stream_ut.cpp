...
TEST_CASE("stream facade also exposes value readers and empty collection through the umbrella header",
          "[UT][wh/schema/stream.hpp][collect_stream_reader][condition][boundary]") {
  auto reader =
      wh::schema::stream::make_values_stream_reader(std::vector<int>{});

  auto collected =
      wh::schema::stream::collect_stream_reader(std::move(reader));
  REQUIRE(collected.has_value());
