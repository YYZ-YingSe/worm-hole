...
TEST_CASE("message parser detail helpers cover role parsing key-path extraction and json object decoding",
  ...
  REQUIRE(std::holds_alternative<wh::schema::text_part>(
      parsed_message.value().parts.front()));
}

TEST_CASE("message parser covers content full-json tool-call and invalid branches",
          "[UT][wh/schema/message/parser.hpp][parse_message][condition][branch][boundary]") {
  auto plain = wh::schema::parse_message(
      "hello", {.from = wh::schema::message_parse_from::content,
  ...
  REQUIRE(std::get<wh::schema::text_part>(full_json.value().parts.front()).text ==
          "json-content");
  ...
  auto tool_calls = wh::schema::parse_message(
      R"({"tool_calls":[{"function":{"arguments":"{\"k\":1}"}}]})",
      {.from = wh::schema::message_parse_from::tool_calls});
  REQUIRE(tool_calls.has_value());
  REQUIRE(std::get<wh::schema::text_part>(tool_calls.value().parts.front()).text ==
          R"({"k":1})");
