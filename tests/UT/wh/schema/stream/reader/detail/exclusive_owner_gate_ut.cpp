...
TEST_CASE("exclusive owner gate claims phases releases and token matching enforce single-owner contract",
          "[UT][wh/schema/stream/reader/detail/exclusive_owner_gate.hpp][exclusive_owner_gate::try_claim][condition][branch][boundary]") {
  ...
  REQUIRE(gate.set_phase(claim->token,
                         wh::schema::stream::detail::owner_phase::topology_wait));
  REQUIRE(gate.current_phase() ==
          wh::schema::stream::detail::owner_phase::topology_wait);
  REQUIRE_FALSE(gate.set_phase(claim->token + 1U,
                               wh::schema::stream::detail::owner_phase::round_active));
...
TEST_CASE("exclusive owner gate rejects stale tokens without disturbing active owner",
          "[UT][wh/schema/stream/reader/detail/exclusive_owner_gate.hpp][exclusive_owner_gate::release][condition][branch][boundary]") {
  wh::schema::stream::detail::exclusive_owner_gate gate{};
  auto claim = gate.try_claim(wh::schema::stream::detail::owner_kind::sync);
  REQUIRE(claim.has_value());
  ...
  REQUIRE(gate.set_phase(claim->token,
                         wh::schema::stream::detail::owner_phase::finishing));
  REQUIRE(gate.current_phase() ==
          wh::schema::stream::detail::owner_phase::finishing);
  REQUIRE(gate.release(claim->token));
  REQUIRE_FALSE(gate.claimed());
...
TEST_CASE("exclusive owner gate allows only one concurrent claimant at a time",
          "[UT][wh/schema/stream/reader/detail/exclusive_owner_gate.hpp][exclusive_owner_gate::try_claim][concurrency][branch]") {
  wh::schema::stream::detail::exclusive_owner_gate gate{};
