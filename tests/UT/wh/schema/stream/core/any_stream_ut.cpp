...
struct auto_close_probe_reader
    : wh::schema::stream::stream_base<auto_close_probe_reader, int> {
  bool automatic_close{true};

  [[nodiscard]] auto read_impl() -> wh::schema::stream::stream_result<
      wh::schema::stream::stream_chunk<int>> {
    return wh::schema::stream::stream_chunk<int>::make_eof();
  }

  [[nodiscard]] auto try_read_impl() -> wh::schema::stream::stream_try_result<
      wh::schema::stream::stream_chunk<int>> {
    return wh::schema::stream::stream_result<
        wh::schema::stream::stream_chunk<int>>{
  ...
  [[nodiscard]] auto read_async() {
    using result_t = wh::schema::stream::stream_result<
        wh::schema::stream::stream_chunk<int>>;
    return stdexec::just(
        result_t{wh::schema::stream::stream_chunk<int>::make_eof()});
  }
...
static_assert(requires {
  wh::schema::stream::any_stream_reader<non_nothrow_value_t>{std::declval<
      wh::schema::stream::pipe_stream_reader<non_nothrow_value_t> &&>()};
  wh::schema::stream::any_stream_writer<non_nothrow_value_t>{std::declval<
      wh::schema::stream::pipe_stream_writer<non_nothrow_value_t> &&>()};
});
...
TEST_CASE("any stream reader covers empty target move target_if and auto-close "
  ...
  auto missing_async =
      wh::testing::helper::wait_value_on_test_thread(empty.read_async());
...
TEST_CASE("any stream read and write async paths preserve stop and deliver "
  ...
  auto read_op = stdexec::connect(
      ...
      wh::testing::helper::sender_capture_receiver<read_result_t, read_env_t>{
          ...
          {.scheduler = stdexec::inline_scheduler{},
           .stop_token = read_stop.get_token()}});
  ...
  wh::schema::stream::any_stream_writer<int> erased_writer{
      std::move(raw_writer)};
  ...
  auto write_op = stdexec::connect(
      ...
      wh::testing::helper::sender_capture_receiver<wh::core::result<void>,
                                                   write_env_t>{
          &write_capture,
          {.scheduler = stdexec::inline_scheduler{},
           .stop_token = write_stop.get_token()}});
  ...
  REQUIRE(write_capture.terminal ==
          wh::testing::helper::sender_terminal_kind::stopped);
  ...
  auto missing_write = wh::testing::helper::wait_value_on_test_thread(
      default_writer.write_async(7));
  REQUIRE(missing_write.has_error());
  REQUIRE(missing_write.error() == wh::core::errc::not_found);
}

TEST_CASE(
    "any stream erases pipe backends for values without nothrow copy or move",
    "[UT][wh/schema/stream/core/"
    "any_stream.hpp][any_stream_reader::read_async][condition][boundary]") {
  auto [raw_writer, raw_reader] =
      wh::schema::stream::make_pipe_stream<non_nothrow_value_t>(2U);
  wh::schema::stream::any_stream_writer<non_nothrow_value_t> erased_writer{
      std::move(raw_writer)};
  wh::schema::stream::any_stream_reader<non_nothrow_value_t> erased_reader{
      std::move(raw_reader)};

  REQUIRE(erased_writer.try_write(non_nothrow_value_t{21}).has_value());
  auto first = wh::testing::helper::wait_value_on_test_thread(
      erased_reader.read_async());
  REQUIRE(first.has_value());
  REQUIRE(first.value().value ==
          std::optional<non_nothrow_value_t>{non_nothrow_value_t{21}});
  ...
  REQUIRE(second.value().value ==
          std::optional<non_nothrow_value_t>{non_nothrow_value_t{22}});
}
