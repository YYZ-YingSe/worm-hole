...
TEST_CASE("any stream reader covers empty target move target_if and auto-close forwarding",
          "[UT][wh/schema/stream/core/any_stream.hpp][any_stream_reader::read_async][branch][boundary]") {
  using result_t =
      wh::schema::stream::stream_result<wh::schema::stream::stream_chunk<int>>;
...
TEST_CASE("any stream read and write async paths preserve stop and deliver through erased backend",
          "[UT][wh/schema/stream/core/any_stream.hpp][any_stream_writer::write_async][condition][concurrency][branch]") {
  ...
  using read_result_t =
      wh::schema::stream::stream_result<wh::schema::stream::stream_chunk<int>>;
  using read_env_t = wh::testing::helper::scheduler_env<
  ...
  auto read_op = stdexec::connect(
      ...
      wh::testing::helper::sender_capture_receiver<read_result_t, read_env_t>{
          &read_capture, {.scheduler = stdexec::inline_scheduler{},
  ...
  auto recovered =
      wh::testing::helper::wait_value_on_test_thread(erased_reader.read_async());
  ...
  using write_env_t = wh::testing::helper::scheduler_env<
      stdexec::inline_scheduler, stdexec::inplace_stop_token>;
  ...
  auto write_op = stdexec::connect(
      ...
      wh::testing::helper::sender_capture_receiver<wh::core::result<void>, write_env_t>{
          &write_capture, {.scheduler = stdexec::inline_scheduler{},
