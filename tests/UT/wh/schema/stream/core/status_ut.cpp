...
TEST_CASE("stream status exposes canonical signal text aliases and try-result shape",
  ...
  STATIC_REQUIRE(std::same_as<
                 try_result_t,
                 std::variant<wh::schema::stream::stream_signal, result_t>>);

  REQUIRE(wh::schema::stream::stream_pending ==
          wh::schema::stream::stream_signal::pending);
  REQUIRE(wh::schema::stream::to_string(
              wh::schema::stream::stream_signal::pending) == "pending");
...
TEST_CASE("stream status try-result can hold pending and ready branches",
  ...
  wh::schema::stream::stream_try_result<int> pending =
      wh::schema::stream::stream_pending;
  REQUIRE(
      std::holds_alternative<wh::schema::stream::stream_signal>(pending));

  wh::schema::stream::stream_try_result<int> ready =
      wh::schema::stream::stream_result<int>{7};
