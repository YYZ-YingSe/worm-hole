...
TEST_CASE(
    "transform stream reader maps values and converts callback failures into terminal error chunks",
    "[UT][wh/schema/stream/adapter/"
    "transform_stream_reader.hpp][make_transform_stream_reader][branch][boundary]") {
  auto reader = wh::schema::stream::make_transform_stream_reader(
      wh::schema::stream::make_values_stream_reader(std::vector<std::string>{"3", "skip"}),
...
TEST_CASE(
    "transform stream reader rejects chunks without payload and preserves stopped async completion",
    "[UT][wh/schema/stream/adapter/"
    "transform_stream_reader.hpp][transform_stream_reader::read_async][condition][concurrency]["
    "branch]") {
  auto invalid_reader = wh::schema::stream::make_transform_stream_reader(
      invalid_async_reader{}, [](const std::string &input) -> wh::core::result<int> {
...
TEST_CASE(
    "transform stream async sender keeps state alive after wrapper destruction",
    "[UT][wh/schema/stream/adapter/"
    "transform_stream_reader.hpp][transform_stream_reader::read_async][lifetime][concurrency]") {
  auto sender = []() {
    auto reader = wh::schema::stream::make_transform_stream_reader(
        wh::schema::stream::make_values_stream_reader(std::vector<std::string>{"11"}),
        [](const std::string &input) -> wh::core::result<int> { return std::stoi(input); });
...
TEST_CASE(
    "transform stream reader forwards owned chunks as movable inputs when source is not borrowed",
    "[UT][wh/schema/stream/adapter/"
    "transform_stream_reader.hpp][make_transform_stream_reader][move_only][ownership]") {
