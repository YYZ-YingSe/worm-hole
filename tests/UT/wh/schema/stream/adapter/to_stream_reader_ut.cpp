...
class throwing_reader final : public wh::schema::stream::stream_base<throwing_reader, int> {
public:
  ...
  [[nodiscard]] auto read_impl()
    ...
    return std::get<wh::schema::stream::stream_result<wh::schema::stream::stream_chunk<int>>>(
        std::move(next));
  }
  ...
  [[nodiscard]] auto is_closed_impl() const noexcept -> bool { return !state_ || state_->closed; }
...
TEST_CASE("to stream reader converts thrown reads into error chunks and supports disabling "
          "automatic close",
          "[UT][wh/schema/stream/adapter/"
          "to_stream_reader.hpp][make_to_stream_reader][branch][boundary]") {
  ...
  auto third = std::get<wh::schema::stream::stream_result<wh::schema::stream::stream_chunk<int>>>(
      reader.try_read());
  REQUIRE(third.has_value());
  REQUIRE(third.value().is_terminal_eof());

  auto disabled_state = std::make_shared<throwing_reader_state>();
  auto disabled = wh::schema::stream::make_to_stream_reader(throwing_reader{disabled_state});
...
TEST_CASE("to stream reader async read preserves stopped completion",
          "[UT][wh/schema/stream/adapter/"
          "to_stream_reader.hpp][to_stream_reader::read_async][condition][concurrency][branch]") {
  ...
  auto operation = stdexec::connect(
      reader.read_async(), wh::testing::helper::sender_capture_receiver<result_t>{&capture});
...
TEST_CASE("to stream async sender keeps state alive after wrapper destruction",
          "[UT][wh/schema/stream/adapter/"
          "to_stream_reader.hpp][to_stream_reader::read_async][lifetime][concurrency]") {
  auto sender = []() {
