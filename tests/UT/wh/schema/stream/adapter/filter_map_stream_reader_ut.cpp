...
class invalid_borrowed_async_reader final
    : public wh::schema::stream::stream_base<invalid_borrowed_async_reader, std::string> {
public:
  using chunk_t = wh::schema::stream::stream_chunk<std::string>;
  using chunk_view_t = wh::schema::stream::stream_chunk_view<std::string>;

  [[nodiscard]] auto read_impl() -> wh::schema::stream::stream_result<chunk_t> { return chunk_t{}; }

  [[nodiscard]] auto try_read_impl() -> wh::schema::stream::stream_try_result<chunk_t> {
    return wh::schema::stream::stream_result<chunk_t>{chunk_t{}};
  }

  [[nodiscard]] auto read_borrowed() -> wh::schema::stream::stream_result<chunk_view_t> {
    return chunk_view_t{};
  }

  [[nodiscard]] auto try_read_borrowed() -> wh::schema::stream::stream_try_result<chunk_view_t> {
...
TEST_CASE("filter map stream reader skips mapped items and turns callback failures into terminal "
          "error chunks",
          "[UT][wh/schema/stream/adapter/"
          "filter_map_stream_reader.hpp][make_filter_map_stream_reader][branch][boundary]") {
  auto reader = wh::schema::stream::make_filter_map_stream_reader(
      wh::schema::stream::make_values_stream_reader(std::vector<std::string>{"1", "skip", "bad"}),
      [](const std::string &input) -> wh::core::result<wh::schema::stream::filter_map_step<int>> {
        if (input == "skip") {
          return wh::schema::stream::skip;
        }
        if (input == "bad") {
          return wh::core::result<wh::schema::stream::filter_map_step<int>>::failure(
...
TEST_CASE("filter map stream reader rejects missing borrowed payload and preserves stopped async "
          "completion",
          "[UT][wh/schema/stream/adapter/"
          "filter_map_stream_reader.hpp][filter_map_stream_reader::read_async][condition]["
          "concurrency][branch]") {
  ...
  auto operation =
      stdexec::connect(stopped_reader.read_async(),
...
TEST_CASE(
    "filter map stream async sender skips values and keeps state alive after wrapper destruction",
    "[UT][wh/schema/stream/adapter/"
    "filter_map_stream_reader.hpp][filter_map_stream_reader::read_async][lifetime][branch]["
    "concurrency]") {
  auto sender = []() {
    auto reader = wh::schema::stream::make_filter_map_stream_reader(
        wh::schema::stream::make_values_stream_reader(std::vector<std::string>{"skip", "7"}),
        [](const std::string &input) -> wh::core::result<wh::schema::stream::filter_map_step<int>> {
          if (input == "skip") {
            return wh::schema::stream::skip;
          }
  ...
  auto operation = stdexec::connect(
      std::move(sender), wh::testing::helper::sender_capture_receiver<result_t>{&capture});
  stdexec::start(operation);
...
TEST_CASE("filter map stream reader forwards owned chunks to move-only callbacks",
          "[UT][wh/schema/stream/adapter/"
          "filter_map_stream_reader.hpp][make_filter_map_stream_reader][move_only][ownership]") {
  ...
  auto reader = wh::schema::stream::make_filter_map_stream_reader(
      ...
      [](std::unique_ptr<int> value) -> wh::core::result<wh::schema::stream::filter_map_step<int>> {
        if (value == nullptr) {
          return wh::core::result<wh::schema::stream::filter_map_step<int>>::failure(
