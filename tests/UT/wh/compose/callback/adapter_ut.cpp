...
TEST_CASE("compose callback adapter returns success when hooks are not configured",
          "[UT][wh/compose/callback/"
          "adapter.hpp][callback_adapter::emit_start][condition][branch][boundary]") {
  ...
  REQUIRE(adapter.emit_error(wh::core::make_error(wh::core::errc::canceled), context).has_value());
}

TEST_CASE("compose callback adapter forwards configured hook payloads",
          "[UT][wh/compose/callback/"
          "adapter.hpp][callback_adapter::emit_end][condition][branch][boundary]") {
  wh::compose::callback_adapter adapter{};
  ...
  adapter.on_error = [&](const wh::core::error_code error,
                         wh::core::run_context &) -> wh::core::result<void> {
    seen_error = error;
    return {};
  };
...
TEST_CASE("compose callback adapter propagates hook failures unchanged",
          "[UT][wh/compose/callback/"
          "adapter.hpp][callback_adapter::emit_error][condition][branch][boundary]") {
  wh::compose::callback_adapter adapter{};
  wh::core::run_context context{};
  ...
  adapter.on_error = [](const wh::core::error_code,
                        wh::core::run_context &) -> wh::core::result<void> {
  ...
  REQUIRE(adapter.emit_error(wh::core::make_error(wh::core::errc::canceled), context).error() ==
