...
TEST_CASE("compose graph error helpers convert details into diagnostics",
  ...
  const auto step_error = wh::compose::to_compose_error(
  ...
  const auto timeout_error = wh::compose::to_compose_error(
  ...
  auto diagnostic = wh::compose::to_graph_diagnostic(
      wh::compose::compose_error{
