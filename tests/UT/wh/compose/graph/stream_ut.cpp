...
[[nodiscard]] auto extract_string(const wh::compose::graph_value &value)
    -> std::string {
...
TEST_CASE("make_graph_stream creates connected writer reader endpoints",
  ...
  REQUIRE(
      writer.try_write(wh::compose::graph_value{std::string{"two"}}).has_value());
...
TEST_CASE("copy_graph_readers duplicates source readers and handles zero fanout",
          "[UT][wh/compose/graph/stream.hpp][detail::copy_graph_readers][condition][branch][boundary]") {
  auto empty =
      wh::compose::detail::copy_graph_readers(wh::compose::graph_stream_reader{},
                                              0U);
  REQUIRE(empty.has_value());
  REQUIRE(empty.value().empty());

  auto source = wh::compose::make_values_stream_reader(
      std::vector<wh::compose::graph_value>{wh::compose::graph_value{3},
                                            wh::compose::graph_value{5}});
  REQUIRE(source.has_value());

  auto copies =
      wh::compose::detail::copy_graph_readers(std::move(source).value(), 2U);
  REQUIRE(copies.has_value());
  REQUIRE(copies.value().size() == 2U);

  auto first =
      wh::compose::collect_graph_stream_reader(std::move(copies.value()[0]));
  ...
  auto second =
      wh::compose::collect_graph_stream_reader(std::move(copies.value()[1]));
...
TEST_CASE("make_graph_merge_reader merges named readers and builds dynamic source shells",
          "[UT][wh/compose/graph/stream.hpp][detail::make_graph_merge_reader][condition][branch][boundary]") {
  ...
  auto topology_shell = wh::compose::detail::make_graph_merge_reader(
...
TEST_CASE("fork_graph_reader_payload duplicates stream payloads and rejects non-reader values",
          "[UT][wh/compose/graph/stream.hpp][detail::fork_graph_reader_payload][condition][branch][boundary]") {
  ...
  auto first_collected =
      wh::compose::collect_graph_stream_reader(std::move(*first_reader));
  REQUIRE(first_collected.has_value());
  REQUIRE(first_collected.value().size() == 1U);
  REQUIRE(extract_int(first_collected.value()[0]) == 7);

  auto *second_reader =
      wh::core::any_cast<wh::compose::graph_stream_reader>(&sibling.value());
  REQUIRE(second_reader != nullptr);
  auto second_collected =
      wh::compose::collect_graph_stream_reader(std::move(*second_reader));
...
TEST_CASE("value payload validators reject reader and move-only payload violations",
          "[UT][wh/compose/graph/stream.hpp][detail::validate_value_contract_payload][condition][branch][boundary]") {
  wh::compose::graph_value copyable{std::string{"value"}};
  REQUIRE(wh::compose::detail::is_reader_value_payload(copyable) == false);
  REQUIRE(
      wh::compose::detail::validate_value_boundary_payload(copyable).has_value());
  REQUIRE(
      wh::compose::detail::validate_copyable_value_payload(copyable).has_value());
  REQUIRE(
      wh::compose::detail::validate_value_contract_payload(copyable).has_value());
  ...
  auto boundary =
      wh::compose::detail::validate_value_boundary_payload(reader_payload);
  REQUIRE(boundary.has_error());
  REQUIRE(boundary.error() == wh::core::errc::contract_violation);
  auto contract =
      wh::compose::detail::validate_value_contract_payload(reader_payload);
  ...
  auto copyable_status =
      wh::compose::detail::validate_copyable_value_payload(move_only);
  REQUIRE(copyable_status.has_error());
  REQUIRE(copyable_status.error() == wh::core::errc::contract_violation);
}

TEST_CASE("to_graph_stream_reader accepts graph readers and converts typed readers",
          "[UT][wh/compose/graph/stream.hpp][to_graph_stream_reader][condition][branch][boundary]") {
  STATIC_REQUIRE(
      wh::compose::graph_stream_status<
  ...
  auto canonical_identity =
      wh::compose::to_graph_stream_reader(std::move(identity).value());
  REQUIRE(canonical_identity.has_value());
  auto identity_values = wh::compose::collect_graph_stream_reader(
      std::move(canonical_identity).value());
  ...
  auto graph_reader = wh::schema::stream::make_values_stream_reader(
      std::vector<wh::compose::graph_value>{wh::compose::graph_value{
          std::string{"graph"}}});
  auto canonical_graph =
      wh::compose::to_graph_stream_reader(std::move(graph_reader));
  REQUIRE(canonical_graph.has_value());
  auto graph_values =
      wh::compose::collect_graph_stream_reader(std::move(canonical_graph).value());
  ...
  auto canonical_typed =
      wh::compose::to_graph_stream_reader(std::move(typed_reader));
  REQUIRE(canonical_typed.has_value());
  auto typed_values =
      wh::compose::collect_graph_stream_reader(std::move(canonical_typed).value());
...
TEST_CASE("make_values_stream_reader make_single_value_stream_reader and fork_graph_value preserve copyable values",
  ...
  auto single_collected =
      wh::compose::collect_graph_stream_reader(std::move(single).value());
  ...
  REQUIRE(move_only_fork.error() == wh::core::errc::contract_violation);
}
