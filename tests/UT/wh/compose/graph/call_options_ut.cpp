...
TEST_CASE("graph call scope resolves paths designations and subscriptions",
          "[UT][wh/compose/graph/"
          "call_options.hpp][graph_call_scope::absolute_path][condition][branch][boundary]") {
  ...
  auto scope = wh::compose::graph_call_scope::root(options).with_trace({.trace_id = "t"});
  ...
  REQUIRE(scope.relative_path(wh::compose::make_node_path({"other", "node-a"})).has_value());

  wh::compose::graph_call_scope nested_scope{options, wh::compose::make_node_path({"root"})};
  auto nested_relative =
      nested_scope.relative_path(wh::compose::make_node_path({"root", "node-a"}));
  REQUIRE(nested_relative.has_value());
  REQUIRE(nested_relative->to_string() == "node-a");
  REQUIRE_FALSE(
      nested_scope.relative_path(wh::compose::make_node_path({"other", "node-a"})).has_value());
  ...
  REQUIRE(wh::compose::is_graph_node_designated(options, "node-a",
...
TEST_CASE("graph call options resolve debug emission interrupt policy and component overrides",
          "[UT][wh/compose/graph/"
          "call_options.hpp][resolve_external_interrupt_policy][condition][branch]") {
  wh::compose::graph_call_options options{};
  ...
  options.component_defaults.insert_or_assign("temperature", wh::compose::graph_value{0.1});
  ...
  REQUIRE(resolved_policy.timeout ==
  ...
  REQUIRE(*wh::core::any_cast<double>(&resolved_options.at("temperature").value) == 0.5);
  REQUIRE(*wh::core::any_cast<std::string>(&resolved_options.at("label").value) == "hot");

  auto extracted_temperature =
      wh::compose::extract_graph_component_option<double>(resolved_options, "temperature");
  ...
  auto wrong_type =
      wh::compose::extract_graph_component_option<int>(resolved_options, "temperature");
  REQUIRE(wrong_type.has_error());
  REQUIRE(wrong_type.error() == wh::core::errc::type_mismatch);
}

TEST_CASE(
    "graph call options freeze external interrupt policy once and dispatch matching debug "
    "observers",
  ...
  auto first = wh::compose::freeze_external_interrupt_policy(
      latch, wh::compose::graph_external_interrupt_policy{.timeout = std::chrono::milliseconds{5}});
  auto second = wh::compose::freeze_external_interrupt_policy(
      latch,
      wh::compose::graph_external_interrupt_policy{.timeout = std::chrono::milliseconds{99}});
  REQUIRE(first.timeout == std::optional<std::chrono::milliseconds>{std::chrono::milliseconds{5}});
  REQUIRE(second.timeout == first.timeout);

  wh::compose::graph_call_options options{};
  int graph_hits = 0;
  int node_hits = 0;
  options.graph_debug_observer = [&graph_hits](const wh::compose::graph_debug_stream_event &,
  ...
  wh::compose::dispatch_graph_debug_observers(wh::compose::graph_call_scope::root(options), event,
                                              context);
