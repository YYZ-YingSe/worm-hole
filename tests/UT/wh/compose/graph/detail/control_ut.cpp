...
TEST_CASE("control helpers reject invalid call options through public invoke validation",
          "[UT][wh/compose/graph/detail/control.hpp][graph::validate_call_scope_for_runtime][condition][branch][boundary]") {
  ...
  SECTION("invalid component default key") {
    ...
    REQUIRE(std::get<0>(*waited)->output_status.error() ==
...
TEST_CASE("control helpers respect valid designation paths through public invoke",
  ...
  REQUIRE(*wh::core::any_cast<int>(&std::get<0>(*waited)->output_status.value()) ==
          9);
}
