...
TEST_CASE("dag run sender connects initialized dag runtime state into a working sender surface",
          "[UT][wh/compose/graph/detail/dag_start.hpp][dag_run_sender::connect][condition][branch][boundary]") {
  ...
  auto base = wh::testing::helper::make_base_run_state(
      graph.value(), wh::compose::graph_value{8}, context);
  ...
  auto waited = stdexec::sync_wait(
      wh::compose::detail::invoke_runtime::dag_run_sender{std::move(dag_state)});
...
TEST_CASE("dag run sender preserves owned call options when rebinding moved runtime storage",
          "[UT][wh/compose/graph/detail/dag_start.hpp][dag_run_sender::connect][condition][branch]") {
  auto graph = wh::testing::helper::make_runtime_identity_graph(
  ...
  REQUIRE(waited.has_value());
  REQUIRE(std::get<0>(*waited).has_value());
  REQUIRE(*wh::core::any_cast<int>(&std::get<0>(*waited).value()) == 18);
}
