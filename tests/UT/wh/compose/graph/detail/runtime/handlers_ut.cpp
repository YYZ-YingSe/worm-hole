...
TEST_CASE("runtime handler resolution prefers authored handlers and validates external contracts",
          ...
          "handlers.hpp][resolve_node_state_handlers][condition][branch][boundary]") {
  ...
  auto authored = wh::compose::detail::state_runtime::resolve_node_state_handlers(nullptr, "node",
                                                                                  authored_options);
  ...
  auto conflict = wh::compose::detail::state_runtime::resolve_node_state_handlers(&registry, "node",
                                                                                  authored_options);
  ...
  auto missing = wh::compose::detail::state_runtime::resolve_node_state_handlers(nullptr, "node",
                                                                                 required_external);
  REQUIRE(missing.has_error());
  REQUIRE(missing.error() == wh::core::errc::not_found);

  auto incomplete = wh::compose::detail::state_runtime::resolve_node_state_handlers(
...
TEST_CASE("runtime handlers select phase handlers and apply value and stream payloads",
          "[UT][wh/compose/graph/detail/runtime/"
          "handlers.hpp][apply_pre_handlers][condition][branch][boundary]") {
  wh::compose::graph_node_state_handlers handlers{};
  handlers.pre = [](const wh::compose::graph_state_cause &, wh::compose::graph_process_state &,
  ...
  handlers.stream_post = [](const wh::compose::graph_state_cause &,
  ...
  auto pre = wh::compose::detail::state_runtime::apply_pre_handlers(
      ...
      [](wh::core::run_context &, std::string_view, wh::core::error_code, std::string_view) {});
  REQUIRE(pre.has_value());
  REQUIRE(*wh::core::any_cast<int>(&value_payload) == 7);

  auto reader_status = wh::compose::make_single_value_stream_reader(std::string{"x"});
