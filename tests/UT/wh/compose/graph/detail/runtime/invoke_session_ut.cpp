...
TEST_CASE(
    "runtime invoke_session launches compiled dag and pregel states through concrete entrypoints",
    "[UT][wh/compose/graph/detail/runtime/"
    "invoke_session.hpp][start_dag_run][start_pregel_run][condition][branch][boundary]") {
  for (const auto mode :
       {wh::compose::graph_runtime_mode::dag, wh::compose::graph_runtime_mode::pregel}) {
    auto graph = wh::testing::helper::make_runtime_identity_graph(mode);
    REQUIRE(graph.has_value());

    wh::core::run_context context{};
    auto state = wh::testing::helper::make_invoke_session(graph.value(),
                                                          wh::compose::graph_value{5}, context);
    auto waited = stdexec::sync_wait(
        mode == wh::compose::graph_runtime_mode::dag
            ...
            : wh::compose::detail::invoke_runtime::start_pregel_run(std::move(state)));
    REQUIRE(waited.has_value());

    auto status = std::get<0>(std::move(*waited));
    REQUIRE(status.has_value());
    auto typed = wh::testing::helper::read_graph_value<int>(std::move(status).value());
...
TEST_CASE("runtime invoke_session preserves bound trace context after move-time storage rebinding",
          "[UT][wh/compose/graph/detail/runtime/"
          "invoke_session.hpp][invoke_session::rebind_moved_runtime_storage][condition][branch]["
          "boundary]") {
  ...
  REQUIRE(moved.invoke_state().bound_call_scope.trace()->parent_span_id == "parent-span");
}

TEST_CASE("runtime invoke_session captures initialization failures from invalid invoke controls",
          "[UT][wh/compose/graph/detail/runtime/"
          "invoke_session.hpp][invoke_session::invoke_session][condition][branch][boundary]") {
  auto graph = wh::testing::helper::make_runtime_identity_graph(
