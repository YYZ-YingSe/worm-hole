...
TEST_CASE(
    "invoke runtime action helpers preserve payload and terminal metadata",
  ...
  auto terminal = ready_action::terminal_error(wh::core::errc::contract_violation);
  ...
  auto skipped = pregel_action::skip(9U, wh::compose::graph_state_cause{
                                             .run_id = 2U,
                                             .step = 4U,
                                             .node_key = "skip",
                                         });
  ...
  auto launched_pregel = pregel_action::launch(7U,
  ...
  auto failed = pregel_action::terminal_error(5U,
                                              wh::compose::graph_state_cause{
                                                  .run_id = 4U,
                                                  .step = 8U,
                                                  .node_key = "fail",
...
TEST_CASE(
    "invoke runtime state carriers default initialize to safe empty state",
...
TEST_CASE(
    "invoke runtime attempt slot and actions default initialize to empty carriers",
  ...
  auto terminal = ready_action::terminal_error(wh::core::errc::invalid_argument);
  REQUIRE(terminal.kind == ready_action_kind::terminal_error);
...
TEST_CASE(
    "invoke session stage helpers keep live input inside attempt slot",
  ...
  auto stored = session.store_attempt_input(attempt.value(), wh::compose::graph_value{7});
  REQUIRE(stored.has_value());
  auto &slot = session.slot(attempt.value());
  REQUIRE(slot.input.has_value());
  REQUIRE(slot.input->payload.has_value());
  auto before_pre = wh::testing::helper::read_graph_value<int>(*slot.input->payload);
  ...
  REQUIRE(slot.input.has_value());
  REQUIRE(slot.input->payload.has_value());
  auto after_pre = wh::testing::helper::read_graph_value<int>(*slot.input->payload);
  ...
  REQUIRE(slot.input.has_value());
  REQUIRE(slot.input->payload.has_value());
  auto after_prepare = wh::testing::helper::read_graph_value<int>(*slot.input->payload);
  ...
  REQUIRE(slot.current_stage == wh::compose::detail::invoke_runtime::stage::node);
  REQUIRE(slot.input.has_value());
  REQUIRE(slot.input->payload.has_value());
  auto after_finalize = wh::testing::helper::read_graph_value<int>(*slot.input->payload);
...
TEST_CASE("invoke session prepare_execution_input consumes lowering from attempt slot",
          ...
          "invoke.hpp][attempt_input][prepare_execution_input][stream]") {
  auto graph = wh::testing::helper::make_runtime_identity_graph(
  ...
  auto reader = wh::compose::make_single_value_stream_reader(std::string{"slot-reader"});
  REQUIRE(reader.has_value());
  auto stored = session.store_attempt_input(attempt.value(),
                                            wh::compose::graph_value{std::move(reader).value()});
  REQUIRE(stored.has_value());
  ...
  REQUIRE(slot.current_stage == wh::compose::detail::invoke_runtime::stage::prepare);
  REQUIRE(slot.input.has_value());
  ...
  auto *chunks = wh::core::any_cast<std::vector<wh::compose::graph_value>>(&status.value());
