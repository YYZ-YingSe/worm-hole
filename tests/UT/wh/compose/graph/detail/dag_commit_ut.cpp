...
TEST_CASE("dag commit path surfaces canceled when post-node interrupt hook fires on stream output",
          "[UT][wh/compose/graph/detail/"
          "dag_commit.hpp][dag_runtime::commit_node_output][condition][branch][stream][error]") {
  auto graph = wh::testing::helper::make_runtime_stream_identity_graph(
      wh::compose::graph_runtime_mode::dag, "dag_commit_stream_interrupt");
  REQUIRE(graph.has_value());
...
TEST_CASE(
    "dag commit path propagates post-node interrupt hook failure on stream output",
    "[UT][wh/compose/graph/detail/"
    "dag_commit.hpp][dag_runtime::commit_node_output][condition][branch][stream][boundary][error]") {
  auto graph = wh::testing::helper::make_runtime_stream_identity_graph(
      wh::compose::graph_runtime_mode::dag, "dag_commit_stream_hook_error");
  REQUIRE(graph.has_value());
