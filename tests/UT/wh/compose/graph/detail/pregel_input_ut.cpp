...
TEST_CASE("pregel input aggregates predecessor values into one keyed payload per superstep",
          ...
          "pregel_input.hpp][graph::build_pregel_node_input_sender][condition][branch][boundary]") {
  ...
  REQUIRE(graph.add_lambda(wh::testing::helper::make_int_add_node("a", 1)).has_value());
  REQUIRE(graph.add_lambda(wh::testing::helper::make_int_add_node("b", 2)).has_value());
  REQUIRE(graph
              .add_lambda(
                  "join",
                  [](const wh::compose::graph_value &input, wh::core::run_context &,
                    ...
                    return wh::compose::graph_value{left.value() + right.value() + 3};
                  })
              .has_value());
  ...
  auto invoked = wh::testing::helper::invoke_graph_sync(graph, wh::compose::graph_value{1}, context,
...
TEST_CASE("pregel input readiness marks control-free nodes ready when allow_no_control is enabled",
          "[UT][wh/compose/graph/detail/"
          "pregel_input.hpp][graph::classify_pregel_node_readiness][condition][branch][boundary]") {
  wh::compose::graph_compile_options options{};
  options.mode = wh::compose::graph_runtime_mode::pregel;
  options.trigger_mode = wh::compose::graph_trigger_mode::all_predecessors;
  ...
  wh::core::run_context context{};
  auto base = wh::testing::helper::make_invoke_session(graph, wh::compose::graph_value{0}, context);
  ...
  REQUIRE(action.action == wh::compose::detail::invoke_runtime::pregel_action::kind::launch);
...
TEST_CASE("pregel input preserves stream pre-state readers as live input lowering",
          "[UT][wh/compose/graph/detail/"
          "pregel_input.hpp][graph::build_pregel_node_input_sender][stream_pre][boundary]") {
  wh::compose::graph_compile_options options{};
  options.mode = wh::compose::graph_runtime_mode::pregel;
  options.boundary = {
  ...
  auto input_reader = wh::compose::make_single_value_stream_reader(std::string{"pregel-stream"});
  ...
  auto waited = stdexec::sync_wait(pregel_state.make_input_sender(action.attempt));
