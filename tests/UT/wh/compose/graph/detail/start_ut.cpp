...
TEST_CASE("start dag/pregel run entrypoints preserve output for their concrete runtime",
          ...
          "start.hpp][start_dag_run][start_pregel_run][condition][branch][boundary]") {
  ...
  auto dag_waited =
      stdexec::sync_wait(wh::compose::detail::invoke_runtime::start_dag_run(std::move(dag_state)));
  ...
  auto pregel_waited = stdexec::sync_wait(
      wh::compose::detail::invoke_runtime::start_pregel_run(std::move(pregel_state)));
...
TEST_CASE("start dag run short-circuits immediate failures discovered during initialization",
  ...
  auto waited =
      stdexec::sync_wait(wh::compose::detail::invoke_runtime::start_dag_run(std::move(state)));
