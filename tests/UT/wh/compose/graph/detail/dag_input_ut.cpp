...
TEST_CASE("dag input preserves stream pre-state readers as live input lowering",
          "[UT][wh/compose/graph/detail/dag_input.hpp][graph::build_node_input_sender][stream_pre][boundary]") {
  wh::compose::graph_compile_options options{};
  options.mode = wh::compose::graph_runtime_mode::dag;
  options.boundary = {
  ...
  auto input_reader =
      wh::compose::make_single_value_stream_reader(std::string{"dag-stream"});
