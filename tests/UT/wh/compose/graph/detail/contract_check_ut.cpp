...
TEST_CASE("contract check helpers describe gates lift lowerings and validate compiled graphs",
          "[UT][wh/compose/graph/detail/contract_check.hpp][graph::validate_contracts][condition][branch][boundary]") {
  ...
  auto passthrough_gate = lifted_value_gate(
      passthrough_edge, wh::compose::output_gate::exact<int>());
  ...
  auto collected_gate = lifted_value_gate(
  ...
  REQUIRE(compatible_value_edge(passthrough_edge,
                                wh::compose::output_gate::exact<int>(),
                                wh::compose::input_gate::exact<int>()));
  REQUIRE_FALSE(compatible_value_edge(passthrough_edge,
                                      wh::compose::output_gate::exact<int>(),
                                      wh::compose::input_gate::exact<std::string>()));
  REQUIRE(compatible_value_edge(collect_edge, wh::compose::output_gate::reader(),
                                wh::compose::input_gate::exact<
                                    std::vector<wh::compose::graph_value>>()));

  auto incompatibility = incompatible_edge_message(
      "left", "right", passthrough_edge, wh::compose::output_gate::exact<int>(),
      wh::compose::input_gate::exact<std::string>());
  ...
  REQUIRE(fan_in_gate->value.key() ==
          wh::core::any_type_key_v<wh::compose::graph_value_map>);

  REQUIRE(accepts_fan_in_value_map(
      index, wh::compose::input_gate::exact<wh::compose::graph_value_map>(), 1U));
  REQUIRE_FALSE(accepts_fan_in_value_map(index,
                                         wh::compose::input_gate::exact<int>(),
                                         1U));

  wh::compose::graph valid{};
  REQUIRE(valid.add_passthrough(wh::compose::make_passthrough_node("worker"))
              .has_value());
...
TEST_CASE("contract check helpers cover dynamic lowerings reader compatibility and fan-in diagnostics",
          "[UT][wh/compose/graph/detail/contract_check.hpp][lifted_value_gate][condition][branch][boundary]") {
  using namespace wh::compose::detail;

  wh::compose::detail::graph_core::indexed_edge custom_edge{};
  custom_edge.lowering_kind = wh::compose::edge_lowering_kind::custom;
  auto custom_gate =
      lifted_value_gate(custom_edge, wh::compose::output_gate::reader());
  ...
  value_to_stream_edge.lowering_kind =
      wh::compose::edge_lowering_kind::value_to_stream;
  auto value_to_stream_gate = lifted_value_gate(
      value_to_stream_edge, wh::compose::output_gate::exact<int>());
  REQUIRE(value_to_stream_gate.kind ==
          wh::compose::output_gate_kind::value_dynamic);

  REQUIRE(compatible_value_edge(custom_edge,
                                wh::compose::output_gate::exact<int>(),
                                wh::compose::input_gate::reader()));
  REQUIRE(compatible_value_edge(custom_edge,
                                wh::compose::output_gate::dynamic(),
                                wh::compose::input_gate::exact<std::string>()));
  ...
  auto fan_in_message = incompatible_value_fan_in_message(
      "join", wh::compose::output_gate::exact<int>(),
