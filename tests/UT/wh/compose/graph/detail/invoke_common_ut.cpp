...
TEST_CASE("invoke common trace token formatter keeps stable compact ids",
          ...
          "invoke_common.hpp][format_trace_token][condition][branch][boundary]") {
  REQUIRE(wh::compose::detail::format_trace_token("trace-", 7U) == "trace-7");
  REQUIRE(wh::compose::detail::format_trace_token("trace-", 7U, 3U) == "trace-7.3");
...
TEST_CASE("invoke common runtime forwards call options into async node attempts",
          "[UT][wh/compose/graph/detail/"
          "invoke_common.hpp][invoke_session::bind_node_runtime_call_options][condition][branch]["
          "boundary]") {
  ...
  auto invoked = wh::testing::helper::invoke_value_sync(graph, wh::compose::graph_value{0}, context,
