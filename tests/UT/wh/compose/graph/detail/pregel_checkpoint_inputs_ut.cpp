...
TEST_CASE("pregel pending-input capture emits unit completion for current frontier",
          ...
          "pregel_checkpoint_inputs.hpp][pregel_runtime::capture_pending_inputs][condition][branch]"
          "[boundary]") {
  ...
  auto base = wh::testing::helper::make_invoke_session(graph.value(), wh::compose::graph_value{19},
                                                       context);
...
TEST_CASE(
    "pregel pending-input capture becomes a no-op after inputs snapshot already exists",
    "[UT][wh/compose/graph/detail/"
    "pregel_checkpoint_inputs.hpp][pregel_runtime::capture_pending_inputs][condition][boundary]") {
  auto graph = wh::testing::helper::make_runtime_identity_graph(
...
TEST_CASE(
    "pregel runtime checkpoint capture records live deliveries and edge readers",
    "[UT][wh/compose/graph/detail/"
    "pregel_checkpoint.hpp][pregel_runtime::capture_checkpoint_runtime][condition][boundary]") {
