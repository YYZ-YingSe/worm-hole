...
TEST_CASE("graph invoke report promotes node run error into graph run error",
  ...
  const auto status =
      wh::core::result<wh::compose::graph_value>::failure(wh::core::errc::internal_error);
  auto report = wh::compose::detail::make_graph_run_report(status, std::move(outputs));

  REQUIRE(report.graph_run_error.has_value());
  REQUIRE(report.graph_run_error->phase == wh::compose::compose_error_phase::execute);
...
TEST_CASE("graph invoke report promotes stream read error when node run error is absent",
  ...
  const auto status =
      wh::core::result<wh::compose::graph_value>::failure(wh::core::errc::invalid_argument);
  auto report = wh::compose::detail::make_graph_run_report(status, std::move(outputs));
...
TEST_CASE("graph invoke report keeps existing graph run error detail intact",
  ...
  const auto status =
      wh::core::result<wh::compose::graph_value>::failure(wh::core::errc::config_error);
  auto report = wh::compose::detail::make_graph_run_report(status, std::move(outputs));

  REQUIRE(report.graph_run_error.has_value());
  REQUIRE(report.graph_run_error->phase == wh::compose::compose_error_phase::checkpoint);
  REQUIRE(report.graph_run_error->path ==
...
TEST_CASE("graph invoke sender returns output status and report through public completion channel",
  ...
  REQUIRE(invoke_status->output_status.error() == wh::core::errc::invalid_argument);
...
TEST_CASE(
    "start_bound_graph and start_scoped_graph run nested graph entrypoints with bound scope data",
  ...
  const auto scheduler = wh::core::detail::erase_resume_scheduler(stdexec::inline_scheduler{});

  auto waited = stdexec::sync_wait(wh::compose::detail::start_bound_graph(
      graph.value(), context, input, &call_options, nullptr, nullptr, nullptr, scheduler, scheduler,
      nullptr, nullptr, {}));
  REQUIRE(waited.has_value());
  ...
  auto scoped_waited = stdexec::sync_wait(wh::compose::detail::start_scoped_graph(
      graph.value(), context, second_input, &scope, nullptr, nullptr, nullptr, scheduler, scheduler,
      nullptr, nullptr, {}));
