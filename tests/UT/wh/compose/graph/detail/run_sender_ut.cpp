...
TEST_CASE("graph run sender aliases connect initialized concrete runtimes into a working sender surface",
          "[UT][wh/compose/graph/detail/run_sender.hpp][dag_run_sender][pregel_run_sender][condition][branch][boundary]") {
  ...
  auto dag_waited = stdexec::sync_wait(
      wh::compose::detail::invoke_runtime::dag_run_sender{
          std::move(dag_state)});
  ...
  auto *pregel_value =
      wh::core::any_cast<int>(&std::get<0>(*pregel_waited).value());
  REQUIRE(pregel_value != nullptr);
  REQUIRE(*pregel_value == 9);
}

TEST_CASE("graph run sender aliases preserve owned call options when rebinding moved runtime storage",
          "[UT][wh/compose/graph/detail/run_sender.hpp][dag_run_sender][pregel_run_sender][condition][branch]") {
