...
TEST_CASE("invoke stage run retries node execution until graph retry budget is exhausted",
          "[UT][wh/compose/graph/detail/invoke_stage_run.hpp][invoke_stage_run::settle_node_stage][condition][branch][boundary]") {
  ...
  REQUIRE(graph
              .add_lambda(
                  ...
                  [&attempts](const wh::compose::graph_value &input,
                    ...
                    auto typed =
                        wh::testing::helper::read_graph_value<int>(input);
                    if (typed.has_error()) {
                      return wh::core::result<wh::compose::graph_value>::failure(
                          typed.error());
                    }
  ...
  auto invoked =
      wh::testing::helper::invoke_value_sync(graph, wh::compose::graph_value{3},
                                             context);
...
TEST_CASE("invoke stage run honors node timeout overrides when settling execution output",
          "[UT][wh/compose/graph/detail/invoke_stage_run.hpp][invoke_stage_run::launch_node_stage][branch][timeout]") {
  wh::compose::graph_compile_options options{};
  options.mode = wh::compose::graph_runtime_mode::dag;
