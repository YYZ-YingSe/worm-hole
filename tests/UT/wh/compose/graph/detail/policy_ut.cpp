...
TEST_CASE("policy helpers enforce step-budget rules across dag and pregel invoke paths",
          "[UT][wh/compose/graph/detail/policy.hpp][graph::resolve_step_budget][condition][branch][boundary]") {
  ...
  auto zero_step_waited = stdexec::sync_wait(
      pregel_graph->invoke(zero_step_context, std::move(zero_step_request)));
  ...
  auto pregel_waited = stdexec::sync_wait(
      pregel_graph->invoke(pregel_context, std::move(pregel_request)));
  ...
  REQUIRE(*wh::core::any_cast<int>(
              &std::get<0>(*pregel_waited)->output_status.value()) == 5);
}

TEST_CASE("policy helpers reject invalid compile-time pregel budgets when no per-call override is provided",
