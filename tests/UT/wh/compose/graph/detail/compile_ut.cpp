...
TEST_CASE("compile helpers derive gates snapshots and lowering kinds from graph contracts",
          "[UT][wh/compose/graph/detail/compile.hpp][detail::to_snapshot_edge][condition][branch][boundary]") {
  ...
  REQUIRE(wh::compose::detail::resolve_edge_lowering_kind(
              ...
              wh::compose::node_contract::stream, auto_adapter) ==
          wh::compose::edge_lowering_kind::value_to_stream);
  REQUIRE(wh::compose::detail::resolve_edge_lowering_kind(
              wh::compose::node_contract::stream,
              wh::compose::node_contract::value, auto_adapter) ==
  ...
  auto snapshot_edge = wh::compose::detail::to_snapshot_edge(
      edge, wh::compose::node_contract::stream,
      wh::compose::node_contract::value);
  REQUIRE(snapshot_edge.from == "left");
  REQUIRE(snapshot_edge.to == "right");
  REQUIRE(snapshot_edge.lowering_kind ==
          wh::compose::edge_lowering_kind::stream_to_value);
  REQUIRE(snapshot_edge.limits.max_items == 4U);
}

TEST_CASE("compile graph constructors preserve boundary metadata and report cycle diagnostics",
  ...
  REQUIRE(compiled_graph.boundary_input_gate() ==
          wh::compose::input_gate::reader());
  REQUIRE(compiled_graph.boundary_output_gate() ==
          wh::compose::output_gate::reader());
  REQUIRE(compiled_graph
              .add_passthrough(wh::compose::make_passthrough_node("worker"))
              .has_value());
  ...
  REQUIRE(cycle_graph.add_lambda(
  ...
  REQUIRE(cycle_graph.diagnostics().back().message.find("cycle") !=
          std::string::npos);
}
