...
TEST_CASE("dag entry initialization commits start routing and exposes ready worker frontier",
          ...
          "dag_entry.hpp][dag_runtime::initialize_entry][condition][branch][boundary]") {
  ...
  auto base =
      wh::testing::helper::make_invoke_session(graph.value(), wh::compose::graph_value{5}, context);
...
TEST_CASE("dag entry initialization drains to a single ready worker on the identity runtime graph",
          "[UT][wh/compose/graph/detail/"
          "dag_entry.hpp][dag_runtime::frontier][condition][branch][boundary]") {
  auto graph = wh::testing::helper::make_runtime_identity_graph(
