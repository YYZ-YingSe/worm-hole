...
TEST_CASE("restore check reports shape and checkpoint target mismatches",
  ...
  REQUIRE(checkpoint.resume_snapshot
              .upsert("interrupt-1", wh::core::address{}.append("graph").append("ghost"), 1)
              .has_value());
  ...
  REQUIRE(validation.diff.contains(wh::compose::restore_check::restore_diff_kind::node_added));
  REQUIRE_FALSE(validation.issues.empty());
}

TEST_CASE(
    "restore check helper diff counters and text helpers expose stable values",
  ...
  diff.entries.push_back({.kind = wh::compose::restore_check::restore_diff_kind::node_added});
  diff.entries.push_back({.kind = wh::compose::restore_check::restore_diff_kind::node_added});
  diff.entries.push_back({.kind = wh::compose::restore_check::restore_diff_kind::edge_removed});

  REQUIRE(diff.count(wh::compose::restore_check::restore_diff_kind::node_added) == 2U);
  REQUIRE(diff.contains(wh::compose::restore_check::restore_diff_kind::edge_removed));
  REQUIRE(wh::compose::restore_check::mode_text(wh::compose::graph_runtime_mode::pregel) ==
          "pregel");
  REQUIRE(wh::compose::restore_check::trigger_mode_text(
              wh::compose::graph_trigger_mode::any_predecessor) == "any_predecessor");
}
