...
TEST_CASE("compose graph policy enums expose expected stable ordinals",
  ...
  REQUIRE(static_cast<int>(wh::compose::graph_trigger_mode::all_predecessors) == 1);
  REQUIRE(static_cast<int>(wh::compose::graph_fan_in_policy::require_all_sources) == 1);
}
...
TEST_CASE("compose graph policy enums keep uint8 underlying representation and full enum ordering",
  ...
  STATIC_REQUIRE(
      std::same_as<std::underlying_type_t<wh::compose::graph_runtime_mode>, std::uint8_t>);
  STATIC_REQUIRE(
      std::same_as<std::underlying_type_t<wh::compose::graph_dispatch_policy>, std::uint8_t>);
  STATIC_REQUIRE(
      std::same_as<std::underlying_type_t<wh::compose::graph_trigger_mode>, std::uint8_t>);
  STATIC_REQUIRE(
      std::same_as<std::underlying_type_t<wh::compose::graph_fan_in_policy>, std::uint8_t>);
