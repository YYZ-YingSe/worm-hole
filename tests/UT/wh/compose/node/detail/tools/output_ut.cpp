...
TEST_CASE("build_value_output preserves indexed slots for non-return-direct plans",
          ...
          "output.hpp][build_value_output][condition][branch][boundary]") {
  ...
  auto indexed = wh::compose::detail::build_value_output(
      indexed_state, std::vector<wh::compose::detail::call_completion>{
                         {.index = 0U,
  ...
  auto *indexed_results =
      wh::core::any_cast<std::vector<wh::compose::tool_result>>(&indexed.value());
  ...
  REQUIRE(*wh::core::any_cast<std::string>(&indexed_state.rerun().extra.at("c")) == "extra-c");
}
...
TEST_CASE("build_value_output compacts direct-return outputs in sorted call order",
  ...
  auto direct = wh::compose::detail::build_value_output(
      direct_state, std::vector<wh::compose::detail::call_completion>{
                        {.index = 0U,
                         .call = {.call_id = "a", .tool_name = "alpha"},
                         .value = wh::compose::graph_value{10},
                         .rerun_extra = wh::compose::graph_value{std::string{"extra-a"}}},
  ...
  auto *direct_results = wh::core::any_cast<std::vector<wh::compose::tool_result>>(&direct.value());
  ...
  REQUIRE(*wh::core::any_cast<std::string>(&direct_state.rerun().extra.at("a")) == "extra-a");
}
...
TEST_CASE("build_value_output accepts empty completions and produces an empty result vector",
  ...
  auto *results = wh::core::any_cast<std::vector<wh::compose::tool_result>>(&output.value());
...
TEST_CASE("build_stream_output merges per-call streams and applies after middleware",
          "[UT][wh/compose/node/detail/tools/"
          "output.hpp][build_stream_output][condition][branch][boundary]") {
  wh::compose::tools_options options{};
  ...
  completions.push_back(
      {.index = 0U,
       .call = {.call_id = "a", .tool_name = "alpha"},
       .stream = wh::compose::make_values_stream_reader(make_graph_values({1})).value(),
       .after_context = wh::core::run_context{},
       .rerun_extra = wh::compose::graph_value{std::string{"extra-a"}}});
  completions.push_back(
      {.index = 1U,
       .call = {.call_id = "b", .tool_name = "beta"},
       .stream = wh::compose::make_values_stream_reader(make_graph_values({2})).value(),
       .after_context = wh::core::run_context{},
       .rerun_extra = wh::compose::graph_value{std::string{"extra-b"}}});

  auto stream_output = wh::compose::detail::build_stream_output(state, std::move(completions));
  REQUIRE(stream_output.has_value());

  auto *reader = wh::core::any_cast<wh::compose::graph_stream_reader>(&stream_output.value());
  ...
  std::sort(events.begin(), events.end(),
            [](const wh::compose::tool_event &left, const wh::compose::tool_event &right) {
  ...
  REQUIRE(*wh::core::any_cast<std::string>(&state.rerun().extra.at("a")) == "extra-a");
  REQUIRE(*wh::core::any_cast<std::string>(&state.rerun().extra.at("b")) == "extra-b");
}
