...
[[nodiscard]] auto collect_tool_events(wh::compose::graph_value &value)
  ...
  if (collected.has_error()) {
    return wh::core::result<std::vector<wh::compose::tool_event>>::failure(collected.error());
  }
...
TEST_CASE(
    "tools runtime sync covers invalid input sequential guard value output and stream output paths",
  ...
  options.middleware.push_back({
      .after = [](const wh::compose::tool_call &, wh::compose::graph_value &value,
        ...
        if (auto *typed = wh::core::any_cast<std::string>(&value); typed != nullptr) {
  ...
  auto type_mismatch = wh::compose::detail::run_tools_sync<wh::compose::node_contract::value>(
      wh::compose::graph_value{1}, context, runtime, registry, options);
  REQUIRE(type_mismatch.has_error());
  REQUIRE(type_mismatch.error() == wh::core::errc::type_mismatch);

  options.sequential = false;
  auto not_supported = wh::compose::detail::run_tools_sync<wh::compose::node_contract::value>(
      make_batch({{.call_id = "a", .tool_name = "echo", .arguments = "x"}}), context, runtime,
      registry, options);
  REQUIRE(not_supported.has_error());
  REQUIRE(not_supported.error() == wh::core::errc::not_supported);

  options.sequential = true;
  auto value_status = wh::compose::detail::run_tools_sync<wh::compose::node_contract::value>(
  ...
  auto stream_status = wh::compose::detail::run_tools_sync<wh::compose::node_contract::stream>(
      make_batch({{.call_id = "stream", .tool_name = "echo", .arguments = "z"}}), context, runtime,
      registry, options);
  ...
  REQUIRE(*wh::core::any_cast<std::string>(&events.value().front().value) == "z-after");
}

TEST_CASE("tools runtime async covers immediate error paths and parallel gate fanout for value and "
          "stream outputs",
  ...
  auto immediate_error =
      await_graph_sender(wh::compose::detail::run_tools_async<wh::compose::node_contract::value>(
          wh::compose::graph_value{1}, context, runtime, registry, options));
  ...
  auto stream_value =
      await_graph_sender(wh::compose::detail::run_tools_async<wh::compose::node_contract::stream>(
          make_batch({{.call_id = "stream", .tool_name = "echo", .arguments = "chunk"}}), context,
