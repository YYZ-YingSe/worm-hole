...
TEST_CASE(
    "passthrough node factories expose stable contracts for value and stream nodes",
  ...
  auto stream_node =
      wh::compose::make_passthrough_node<wh::compose::node_contract::stream>("stream");
...
TEST_CASE("passthrough node graph execution forwards payload unchanged",
  ...
  REQUIRE(*wh::core::any_cast<int>(&std::get<0>(*awaited).value().output_status.value()) == 19);
}
...
TEST_CASE("stream passthrough node preserves reader payloads through graph invoke",
  ...
  wh::compose::graph graph{};
  REQUIRE(graph.add_passthrough<wh::compose::node_contract::stream>("worker").has_value());
  REQUIRE(graph.add_entry_edge("worker").has_value());
  REQUIRE(graph.add_exit_edge("worker").has_value());
  REQUIRE(graph.compile().has_value());

  auto input_reader = wh::compose::make_values_stream_reader(std::vector<wh::compose::graph_value>{
      wh::compose::graph_value{1}, wh::compose::graph_value{2}});
  REQUIRE(input_reader.has_value());

  wh::compose::graph_invoke_request request{};
  request.input = wh::compose::graph_input::value(std::move(input_reader).value());
  wh::core::run_context context{};
  auto awaited = stdexec::sync_wait(graph.invoke(context, std::move(request)));
  REQUIRE(awaited.has_value());
  REQUIRE(std::get<0>(*awaited).has_value());
...
TEST_CASE("stream passthrough node collects stream into value output at default graph boundary",
  ...
  wh::compose::graph_invoke_request request{};
  request.input = wh::compose::graph_input::stream(std::move(input_reader).value());
  wh::core::run_context context{};
  auto awaited = stdexec::sync_wait(graph.invoke(context, std::move(request)));
  REQUIRE(awaited.has_value());
  REQUIRE(std::get<0>(*awaited).has_value());
...
TEST_CASE("stream passthrough node preserves reader payloads at stream graph boundary",
          "[UT][wh/compose/node/"
          "passthrough.hpp][passthrough_node::compile][condition][branch][boundary]") {
  wh::compose::graph graph{wh::compose::graph_boundary{
      .input = wh::compose::node_contract::stream,
