...
static_assert(wh::compose::result_typed_sender<
              ...
              wh::core::result<wh::compose::graph_value>>);
static_assert(!wh::compose::result_typed_sender<
              decltype(stdexec::just(wh::core::result<int>{})),
              wh::core::result<wh::compose::graph_value>>);
...
static_assert(wh::compose::node_sync_run<decltype(
...
static_assert(!wh::compose::node_sync_run<decltype(
              [](wh::compose::graph_value &, wh::core::run_context &,
                 const wh::compose::node_runtime &) -> int {
                return 1;
              })>);
static_assert(wh::compose::node_async_run<decltype(
              [](wh::compose::graph_value &, wh::core::run_context &,
                 const wh::compose::node_runtime &) {
...
auto bound_nested_start(const void *state, const wh::compose::graph &,
                        wh::core::run_context &, wh::compose::graph_value &,
                        const wh::compose::graph_call_scope *,
                        const wh::compose::node_path *,
...
TEST_CASE("node runtime accessors and nested graph entry expose bound execution state",
  ...
  auto unbound = await_graph_sender(empty(dummy_graph, dummy_context, dummy_input,
  ...
  auto bound = await_graph_sender(entry(dummy_graph, dummy_context, dummy_input,
                                        nullptr, nullptr, nullptr, nullptr,
                                        nullptr));
...
TEST_CASE("execution sender helpers normalize bridge map and failure paths",
  ...
  auto ready_unit =
      await_graph_sender(wh::compose::detail::ready_graph_unit_sender());
  REQUIRE(ready_unit.has_value());
  REQUIRE(ready_unit.value().type() == typeid(std::monostate));

  auto failed = await_graph_sender(
      wh::compose::detail::failure_graph_sender(wh::core::errc::timeout));
  REQUIRE(failed.has_error());
  REQUIRE(failed.error() == wh::core::errc::timeout);

  auto normalized_ready = wh::compose::detail::normalize_graph_sender(
      wh::compose::detail::ready_graph_sender(
          wh::core::result<wh::compose::graph_value>{
...
TEST_CASE("execution factory binders preserve sync behavior and enforce async scheduler contract",
  ...
  auto async_factory = wh::compose::detail::bind_node_async_factory(
      [](wh::compose::graph_value &input, wh::core::run_context &,
        ...
        REQUIRE(typed != nullptr);
        return stdexec::just(
            wh::core::result<wh::compose::graph_value>{wh::compose::graph_value{
                *typed + 4}});
      });
  ...
  auto direct_sender_factory = wh::compose::detail::bind_node_async_factory(
      stdexec::just(
          wh::core::result<wh::compose::graph_value>{wh::compose::graph_value{11}}));
  auto direct_status =
      await_graph_sender(direct_sender_factory(async_input, context, runtime));
  REQUIRE(direct_status.has_value());
  REQUIRE(*wh::core::any_cast<int>(&direct_status.value()) == 11);

  auto direct_missing_scheduler =
      await_graph_sender(direct_sender_factory(async_input, context,
                                               wh::compose::node_runtime{}));
  REQUIRE(direct_missing_scheduler.has_error());
  REQUIRE(direct_missing_scheduler.error() ==
