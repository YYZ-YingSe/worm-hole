...
TEST_CASE("tools node builders expose metadata and dispatch sync value endpoints",
  ...
  registry.emplace("echo", wh::compose::tool_entry{
                               .invoke =
  ...
  runtime_options.missing = wh::compose::tool_entry{
      .invoke = [](const wh::compose::tool_call &, wh::tool::call_scope)
          -> wh::core::result<wh::compose::graph_value> {
        return wh::compose::graph_value{std::string{"fallback"}};
      }};

  auto node = wh::compose::make_tools_node("tools", registry,
  ...
  REQUIRE(*wh::core::any_cast<std::string>(
              &results.value().get().front().value) == "payload");
  ...
  REQUIRE(*wh::core::any_cast<std::string>(
              &fallback.value().get().front().value) == "fallback");
  ...
  REQUIRE(wrong_input.value().output_status.error() ==
          wh::core::errc::type_mismatch);
}
...
TEST_CASE("tools node builders dispatch sync stream endpoints into tool events",
  ...
  wh::compose::tool_registry registry{};
  registry.emplace("stream", wh::compose::tool_entry{
                                 .stream =
                                     [](const wh::compose::tool_call &call,
                                        wh::tool::call_scope)
                                         -> wh::core::result<
  ...
  REQUIRE(graph
              .add_tools(wh::compose::make_tools_node<
                         ...
                         wh::compose::node_contract::stream>("tools",
                                                             std::move(registry)))
              .has_value());
  REQUIRE(graph.add_entry_edge("tools").has_value());
  REQUIRE(graph.add_exit_edge("tools").has_value());
...
TEST_CASE("async stream tools node returns graph stream reader payload",
  ...
  REQUIRE(graph
              .add_tools(wh::compose::make_tools_node<
                         ...
                         wh::compose::node_exec_mode::async>("tools",
                                                             std::move(registry)))
              .has_value());
  REQUIRE(graph.add_entry_edge("tools").has_value());
  REQUIRE(graph.add_exit_edge("tools").has_value());
  ...
  auto waited = stdexec::sync_wait(wh::compose::run_compiled_async_node(
      compiled.value().get(), input, context, runtime));
...
TEST_CASE("tools node builders also compile async value endpoints and preserve labels",
  ...
  auto node = wh::compose::make_tools_node<
      wh::compose::node_contract::value,
      wh::compose::node_contract::value,
      wh::compose::node_exec_mode::async>("tools_async_value", registry);
  REQUIRE(node.options().type == "tools_node");
  REQUIRE(node.options().label == "tools_node");
  ...
  auto waited = stdexec::sync_wait(wh::compose::run_compiled_async_node(
      compiled.value().get(), input,
      context, runtime));
  REQUIRE(waited.has_value());
  auto status = std::get<0>(std::move(*waited));
  REQUIRE(status.has_value());

  auto results = read_any<std::vector<wh::compose::tool_result>>(
      std::move(status).value());
  REQUIRE(results.has_value());
  REQUIRE(results.value().size() == 1U);
  REQUIRE(*wh::core::any_cast<std::string>(&results.value().front().value) ==
          "async:payload");
}
