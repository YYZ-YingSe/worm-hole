...
TEST_CASE("field apply maps nested values and static values into target maps",
  ...
  auto mapped = wh::compose::apply_field_mappings(source, wh::compose::graph_value_map{},
                                                  std::vector{dynamic_rule, static_rule}, context);
  REQUIRE(mapped.has_value());

  auto request = wh::core::any_cast<wh::compose::graph_value_map>(&mapped.value().at("request"));
...
TEST_CASE("field apply honors missing policy skip and surfaces failures otherwise",
          "[UT][wh/compose/field/"
          "apply.hpp][apply_field_mappings_in_place][condition][branch][boundary]") {
  ...
  wh::core::run_context context{};
  auto skipped = wh::compose::apply_field_mappings(wh::compose::graph_value_map{}, target,
                                                   std::vector{skip_rule}, context);
  REQUIRE(skipped.has_value());
  REQUIRE(skipped.value().empty());

  auto failed = wh::compose::apply_field_mappings(wh::compose::graph_value_map{}, target,
                                                  std::vector{fail_rule}, context);
...
TEST_CASE("field apply uses extractor callbacks and rejects rules without any readable source",
  ...
  wh::core::run_context context{};
  auto extracted = wh::compose::apply_field_mappings(wh::compose::graph_value_map{},
                                                     wh::compose::graph_value_map{},
                                                     std::vector{extractor_rule}, context);
  REQUIRE(extracted.has_value());
  auto *out = wh::core::any_cast<wh::compose::graph_value_map>(&extracted->at("out"));
  REQUIRE(out != nullptr);
  REQUIRE(*wh::core::any_cast<int>(&out->at("answer")) == 9);

  auto invalid = wh::compose::apply_field_mappings(wh::compose::graph_value_map{},
                                                   wh::compose::graph_value_map{},
...
TEST_CASE("field apply in-place staging does not leak earlier writes when a later rule fails",
  ...
  first.from_path = wh::compose::field_path{.text = "input", .segments = {"input"}};
  first.to_path = wh::compose::field_path{.text = "out.ok", .segments = {"out", "ok"}};

  wh::compose::compiled_field_mapping_rule failing{};
  failing.from_path = wh::compose::field_path{.text = "missing", .segments = {"missing"}};
