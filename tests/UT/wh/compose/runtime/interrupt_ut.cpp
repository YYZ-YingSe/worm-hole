...
TEST_CASE(
    "compose runtime interrupt helpers create ids and explicit signals",
  ...
  REQUIRE(*wh::core::any_cast<std::string>(&explicit_signal.layer_payload) == "payload");
}

TEST_CASE("compose runtime interrupt helpers merge deduplicated sources",
          "[UT][wh/compose/runtime/"
          "interrupt.hpp][merge_interrupt_sources][condition][branch][boundary]") {
...
TEST_CASE(
    "compose runtime interrupt helpers cover auto-id projection tree conversion and reinterrupt "
    "mapping",
    "[UT][wh/compose/runtime/interrupt.hpp][to_reinterrupt_signal][condition][branch][boundary]") {
  auto auto_signal = wh::compose::make_interrupt_signal(wh::core::make_address({"graph", "leaf"}),
                                                        9, std::string{"layer"});
  ...
  const std::vector<wh::core::interrupt_signal> signals{
      wh::compose::make_interrupt_signal("root", wh::core::make_address({"root"}), 1),
      wh::compose::make_interrupt_signal("child", wh::core::make_address({"root", "child"}), 2),
  };
  const auto snapshot = wh::compose::flatten_interrupt_signals(std::span{signals});
  ...
  std::vector<wh::core::interrupt_signal> duplicates{
      wh::compose::make_interrupt_signal(copied_context.interrupt_id,
                                         wh::core::make_address({"dup"}), 3),
