...
TEST_CASE("workflow exposes stable input keys and step handles",
          ...
          "workflow.hpp][workflow_input_node_key][condition][branch][boundary]") {
...
TEST_CASE("workflow compiles invokes and maps entry/static values",
  ...
  REQUIRE(step.value().set_static_value("request.source", std::string{"ut"}).has_value());
  ...
  input.insert_or_assign("input", wh::compose::graph_value{wh::compose::graph_value_map{
  ...
  auto *request = wh::core::any_cast<wh::compose::graph_value_map>(
      &std::get<0>(*awaited).value().at("request"));
...
TEST_CASE(
    "workflow validates dependency staging compile freeze and data edges",
  ...
  auto invalid_first = invalid_workflow.add_step(wh::compose::make_passthrough_node("first"));
  auto invalid_second = invalid_workflow.add_step(wh::compose::make_passthrough_node("second"));
  REQUIRE(invalid_first.has_value());
  REQUIRE(invalid_second.has_value());
  REQUIRE(invalid_second.value().add_input_without_control(invalid_first.value()).has_error());
  ...
  auto data_first = data_workflow.add_step(wh::compose::make_passthrough_node("first"));
  auto data_second = data_workflow.add_step(wh::compose::make_passthrough_node("second"));
  ...
  REQUIRE(data_workflow.add_step(wh::compose::make_passthrough_node("late")).has_error());
  REQUIRE(data_workflow.step("first").has_error());
}

TEST_CASE("workflow rejects step handles borrowed from another workflow",
          "[UT][wh/compose/authored/"
          "workflow.hpp][workflow::step_ref::add_input][condition][branch][boundary]") {
  ...
  REQUIRE(right_target.value().add_input_without_control(left_source.value()).has_error());
}

TEST_CASE(
    "workflow step_ref accepts entry-only control dependencies and static value staging before "
    "compile",
    "[UT][wh/compose/authored/"
    "workflow.hpp][workflow::step_ref::from_entry_without_input][condition][branch][boundary]") {
  wh::compose::workflow workflow{};
  auto step = workflow.add_step(wh::compose::make_passthrough_node("target"));
  REQUIRE(step.has_value());
