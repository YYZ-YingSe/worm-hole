...
TEST_CASE("stream branch validates targets and selector configuration",
          ...
          "stream_branch.hpp][stream_branch::add_target][condition][branch][boundary]") {
...
TEST_CASE("stream branch lowers valid targets and rejects duplicates",
          "[UT][wh/compose/authored/"
          "stream_branch.hpp][stream_branch::apply][condition][branch][boundary]") {
  wh::compose::graph graph{};
  REQUIRE(graph
              .add_passthrough(
                  wh::compose::make_passthrough_node<wh::compose::node_contract::stream>("source"))
              .has_value());
  REQUIRE(graph
              .add_passthrough(
  ...
  REQUIRE(graph
              .add_passthrough(
                  wh::compose::make_passthrough_node<wh::compose::node_contract::stream>("right"))
              .has_value());
...
TEST_CASE("stream branch rejects empty selector objects and empty target sets on apply",
          "[UT][wh/compose/authored/"
          "stream_branch.hpp][stream_branch::set_selector][condition][branch][boundary]") {
  wh::compose::stream_branch branch{};
