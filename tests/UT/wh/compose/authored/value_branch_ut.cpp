...
TEST_CASE("value branch validates cases and lowers to graph edges",
          ...
          "value_branch.hpp][value_branch::add_case][condition][branch][boundary]") {
  ...
  REQUIRE(branch
              .add_case("right",
                        [](const wh::compose::graph_value &,
                           wh::core::run_context &) -> wh::core::result<bool> { return false; })
              .has_value());
  ...
  REQUIRE(graph.add_passthrough(wh::compose::make_passthrough_node("left")).has_value());
  REQUIRE(graph.add_passthrough(wh::compose::make_passthrough_node("right")).has_value());
  REQUIRE(branch.apply(graph, "source").has_value());
}

TEST_CASE("value branch rejects empty and duplicate targets during apply",
          "[UT][wh/compose/authored/"
          "value_branch.hpp][value_branch::apply][condition][branch][boundary]") {
  ...
  REQUIRE(duplicate
              .add_case("same",
                        [](const wh::compose::graph_value &,
                           wh::core::run_context &) -> wh::core::result<bool> { return false; })
              .has_value());
  REQUIRE(graph.add_passthrough(wh::compose::make_passthrough_node("same")).has_value());
...
TEST_CASE("value branch rejects cases without a target or predicate",
  ...
  auto missing_target = branch.add_case(
      "", [](const wh::compose::graph_value &, wh::core::run_context &) -> wh::core::result<bool> {
        return true;
      });
  REQUIRE(missing_target.has_error());
  REQUIRE(missing_target.error() == wh::core::errc::invalid_argument);

  auto missing_predicate = branch.add_case(wh::compose::value_branch_case{.to = "left"});
