...
TEST_CASE("parallel builder accepts authored node kinds and exposes stored nodes",
          "[UT][wh/compose/authored/parallel.hpp][parallel::add_component][condition][branch][boundary]") {
  ...
  REQUIRE(group.add_lambda(wh::compose::make_lambda_node(
                               "lambda",
  ...
  REQUIRE(group.add_tools(
              wh::compose::make_tools_node("tools", wh::compose::tool_registry{}))
              .has_value());

  REQUIRE(group.add_passthrough(wh::compose::make_passthrough_node("pass"))
              .has_value());
...
TEST_CASE("parallel apply requires at least two nodes and returns tail keys",
  ...
  REQUIRE(too_small.add_passthrough(wh::compose::make_passthrough_node("only"))
              .has_value());
  wh::compose::graph graph{};
  ...
  REQUIRE(valid.add_passthrough(wh::compose::make_passthrough_node("left"))
              .has_value());
  REQUIRE(valid.add_passthrough(wh::compose::make_passthrough_node("right"))
              .has_value());
...
TEST_CASE("parallel builder rejects authored nodes with empty keys",
          "[UT][wh/compose/authored/parallel.hpp][parallel::add_passthrough][condition][branch][boundary]") {
  wh::compose::parallel group{};
