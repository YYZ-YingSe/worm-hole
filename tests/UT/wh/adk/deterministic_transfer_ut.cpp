...
TEST_CASE(
    "deterministic transfer detail helpers classify and normalize transfer message forms",
    "[UT][wh/adk/"
    "deterministic_transfer.hpp][make_transfer_assistant_message][condition][branch][boundary]") {
  ...
  REQUIRE(wh::adk::detail::transfer::render_message_text(rendered) == "draft [tool:search]");
  ...
  REQUIRE_FALSE(wh::adk::detail::transfer::transfer_assistant_call_id(plain).has_value());
  ...
  REQUIRE(wh::adk::parse_transfer_tool_message(plain).error() == wh::core::errc::type_mismatch);
}

TEST_CASE(
    "deterministic transfer resolves authored targets records visible history and rewrites foreign "
    "messages",
    "[UT][wh/adk/"
    "deterministic_transfer.hpp][begin_deterministic_transfer][condition][branch][boundary]") {
  ...
  REQUIRE(
      wh::adk::record_parent_visible_event(
          visible, make_message_event(wh::core::address{{"root", "planner"}},
  ...
  REQUIRE(
      wh::adk::record_parent_visible_event(
          visible, make_message_event(wh::core::address{{"root", "worker"}},
                                      make_named_text_message(wh::schema::message_role::assistant,
                                                              "planner", "hidden")))
          .has_value());
  REQUIRE(visible.visible_history.size() == 1U);
  REQUIRE(wh::adk::record_parent_visible_event(
              visible, wh::adk::make_control_event(
  ...
  wh::adk::agent_message_stream_reader stream_reader{
      wh::schema::stream::make_values_stream_reader(std::vector<wh::schema::message>{
          make_named_text_message(wh::schema::message_role::assistant, "planner", "stream")})};
  auto unsupported_record = wh::adk::record_parent_visible_event(
      visible, wh::adk::make_message_event(std::move(stream_reader),
                                           wh::adk::event_metadata{
                                               .path = wh::core::address{{"root", "planner"}},
                                           }));
...
TEST_CASE(
    "deterministic transfer appends transfer pairs exactly once and skips non-normal completion",
    "[UT][wh/adk/"
    "deterministic_transfer.hpp][append_transfer_messages_once][condition][branch][boundary]") {
  std::vector<wh::schema::message> history{};
  wh::adk::deterministic_transfer_state state{};
  state.pending_target = std::string{"planner"};

  auto invalid = wh::adk::append_transfer_messages_once(history, state, "", "call-1",
  ...
  REQUIRE(wh::adk::append_transfer_messages_once(history, state, "planner", "call-2",
              ...
              .has_value());
  REQUIRE(wh::adk::append_transfer_messages_once(history, state, "planner", "call-3",
