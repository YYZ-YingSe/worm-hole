...
TEST_CASE("adk runner utility helpers normalize run paths events message collection and final-message projection",
          "[UT][wh/adk/runner.hpp][append_run_path_prefix][condition][branch][boundary]") {
  const auto combined = wh::adk::append_run_path_prefix(
      wh::adk::run_path{{"parent", "agent"}}, wh::adk::run_path{{"child"}});
  ...
  wh::adk::agent_message_stream_reader message_reader{
      wh::schema::stream::make_values_stream_reader(
          std::vector<wh::schema::message>{make_user_message("a"),
                                           make_assistant_message("b")})};
  auto messages =
      wh::adk::collect_agent_messages(std::move(message_reader));
  REQUIRE(messages.has_value());
  REQUIRE(messages.value().size() == 2U);

  std::vector<wh::adk::agent_event> collected_event_values{};
  collected_event_values.push_back(wh::adk::make_control_event(
      wh::adk::control_action{
          .kind = wh::adk::control_action_kind::exit,
      }));
  collected_event_values.push_back(
      wh::adk::make_message_event(make_assistant_message("final")));
  wh::adk::agent_event_stream_reader event_reader{
      wh::schema::stream::make_values_stream_reader(
          std::move(collected_event_values))};
  ...
  REQUIRE(std::get<wh::schema::text_part>(final_message->parts.front()).text ==
          "final");

  wh::adk::agent_message_stream_reader nested_reader{
      wh::schema::stream::make_values_stream_reader(
          std::vector<wh::schema::message>{make_assistant_message("nested")})};
  std::vector<wh::adk::agent_event> mixed_events{};
  mixed_events.push_back(
      wh::adk::make_message_event(make_assistant_message("first")));
  ...
  REQUIRE(std::get<wh::schema::text_part>(first_message->parts.front()).text ==
          "first");

  auto run_request =
      wh::adk::make_run_request(wh::adk::query_request{.text = "hello"});
  REQUIRE(run_request.messages.size() == 1U);
  REQUIRE(run_request.messages.front().role == wh::schema::message_role::user);
}

TEST_CASE("adk runner detail helpers lower requests and resolve resume targets from explicit and contextual locations",
  ...
  REQUIRE(context.resume_info->upsert("resume-1", wh::core::address{{"graph", "a"}},
                                      1)
              .has_value());
  ...
  auto from_resume =
      wh::adk::detail::resolve_resume_location_one(context, "resume-1");
  REQUIRE(from_resume.has_value());
  REQUIRE(from_resume.value() == wh::core::address({"graph", "a"}));

  auto from_interrupt =
      wh::adk::detail::resolve_resume_location_one(context, "interrupt-1");
  REQUIRE(from_interrupt.has_value());
  REQUIRE(from_interrupt.value() == wh::core::address({"graph", "b"}));

  auto missing =
      wh::adk::detail::resolve_resume_location_one(context, "missing");
  ...
  auto appended = wh::adk::detail::append_resume_targets(
      context,
      std::vector<wh::adk::resume_target>{
  ...
  auto invalid = wh::adk::detail::append_resume_targets(
      context, std::vector<wh::adk::resume_target>{
                   wh::adk::resume_target{
                       .interrupt_id = "",
...
TEST_CASE("adk runner facade lowers query run and resume requests onto the underlying implementation",
  ...
  auto query_status = wh::testing::helper::wait_value_on_test_thread(
  ...
  auto resume_status = wh::testing::helper::wait_value_on_test_thread(
      runner.resume(resume, context));
...
TEST_CASE("adk runner rejects missing checkpoint services when required and preserves moved run context for sender implementations",
  ...
  auto missing_checkpoint = wh::testing::helper::wait_value_on_test_thread(
      lowered_runner.resume(request, context));
  ...
  auto run_status = wh::testing::helper::wait_value_on_test_thread(
      sender_runner.run(std::move(run), context));
