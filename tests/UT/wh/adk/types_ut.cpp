...
TEST_CASE("adk types make_message_event preserves message payload and metadata",
  ...
  auto event = wh::adk::make_message_event(
      message, wh::adk::event_metadata{.agent_name = "planner"});
...
TEST_CASE("adk types make_message_event supports message-stream payloads and rejects them for checkpointing",
          "[UT][wh/adk/types.hpp][validate_agent_event_checkpoint_serializable][condition][branch][boundary]") {
  auto [writer, reader] = wh::schema::stream::make_pipe_stream<wh::schema::message>(1U);
  auto streamed = wh::adk::make_message_event(
  ...
  auto streamed_status =
      wh::adk::validate_agent_event_checkpoint_serializable(streamed, registry);
...
TEST_CASE("adk types build control custom and error events with owned payloads",
  ...
  auto custom =
      wh::adk::make_custom_event("metric", wh::core::any{9},
                                 {.tool_name = "search"});
  ...
  auto error = wh::adk::make_error_event(
      wh::core::errc::invalid_argument, "bad", wh::core::any{std::string{"detail"}});
...
TEST_CASE("adk types validate checkpoint-serializable payload registration for custom and error events",
          "[UT][wh/adk/types.hpp][validate_agent_event_checkpoint_serializable][condition][branch][boundary]") {
  wh::schema::message message{};
  ...
  REQUIRE(wh::adk::validate_agent_event_checkpoint_serializable(message_event,
                                                                registry)
              .has_value());
  REQUIRE(wh::adk::validate_agent_event_checkpoint_serializable(custom, registry)
              .has_error());
  REQUIRE(wh::adk::validate_agent_event_checkpoint_serializable(empty_custom,
                                                                registry)
              .has_value());
  REQUIRE(wh::adk::validate_agent_event_checkpoint_serializable(error, registry)
              .has_value());
}
