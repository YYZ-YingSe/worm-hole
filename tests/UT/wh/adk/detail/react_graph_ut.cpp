...
[[nodiscard]] auto invoke_react_graph(
...
TEST_CASE("react detail helpers normalize model messages tool actions tool batches and output projection",
          "[UT][wh/adk/detail/react_graph.hpp][react_detail::collect_tool_actions][condition][branch][boundary]") {
  ...
  auto messages =
      wh::adk::detail::react_detail::read_model_messages(std::move(stream).value());
  ...
  auto invalid_messages = wh::adk::detail::react_detail::read_model_messages(
      std::move(invalid_stream).value());
  REQUIRE(invalid_messages.has_error());
  REQUIRE(invalid_messages.error() == wh::core::errc::type_mismatch);

  auto instruction =
      wh::adk::detail::react_detail::make_instruction_message("role", "guide");
  REQUIRE(instruction.has_value());
  REQUIRE_FALSE(
      wh::adk::detail::react_detail::make_instruction_message("", "").has_value());
  ...
  auto actions = wh::adk::detail::react_detail::collect_tool_actions(
      assistant, authored.tools());
  ...
  auto payload_view = wh::adk::detail::read_history_request_payload_view(
      batch_with_history.calls.front().payload);
  ...
  auto tool_message =
      wh::adk::detail::react_detail::tool_result_to_message(string_result);
  ...
  auto typed_message =
      wh::adk::detail::react_detail::tool_result_to_message(typed_result);
  ...
  auto invalid_message =
      wh::adk::detail::react_detail::tool_result_to_message(invalid_result);
  ...
  wh::adk::detail::react_detail::write_output_value(
      state, "value", wh::agent::react_output_mode::value, assistant);
  wh::adk::detail::react_detail::write_output_value(
  ...
  auto output =
      wh::adk::detail::react_detail::make_agent_output(state, assistant);
  REQUIRE(output.history_messages.size() == 1U);
  REQUIRE(output.output_values.size() == 2U);
}

TEST_CASE("react state callbacks bootstrap request tool application direct emit and finalize branches",
          "[UT][wh/adk/detail/react_graph.hpp][react_detail::make_bootstrap_options][condition][branch][boundary]") {
  wh::compose::graph_process_state process_state{};
  ...
  auto prepare_request =
      wh::adk::detail::react_detail::make_prepare_request_options(
  ...
  auto prepare_tools =
      wh::adk::detail::react_detail::make_prepare_tools_options(
          wh::testing::helper::make_configured_react("tool-react", "assistant")
              .tools());
  wh::adk::detail::react_detail::prepared_tool_round prepared{
      .assistant_message = wh::testing::helper::make_text_message(
          wh::schema::message_role::assistant, "tool-call"),
  ...
  wh::adk::detail::react_detail::apply_tools_decision decision{
      .direct_return = true,
      .direct_message = wh::schema::message{
  ...
  REQUIRE(state->get().return_direct_call_id ==
          std::optional<std::string>{"call-1"});

  payload = decision.direct_message.value();
  auto emit_direct =
      wh::adk::detail::react_detail::make_emit_direct_options(
  ...
  REQUIRE(direct_output->output_values.find("direct") !=
          direct_output->output_values.end());
  ...
  REQUIRE(final_output->output_values.find("final") !=
          final_output->output_values.end());
...
TEST_CASE("react graph lowers authored shells into executable graphs and binders",
  ...
  auto output = invoke_react_graph(
      lowered.value(),
      {wh::testing::helper::make_text_message(wh::schema::message_role::user,
                                              "hello")});
