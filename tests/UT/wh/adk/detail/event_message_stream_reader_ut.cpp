...
TEST_CASE("event message stream detail helper consumes direct messages message streams and visitor failures",
          "[UT][wh/adk/detail/event_message_stream_reader.hpp][consume_message_event_messages][condition][branch][boundary]") {
  ...
  auto direct = wh::adk::detail::consume_message_event_messages(
      ...
      [&seen](wh::schema::message message) -> wh::core::result<void> {
        seen.push_back(
            std::get<wh::schema::text_part>(message.parts.front()).text);
        return {};
      });
  REQUIRE(direct.has_value());
  REQUIRE(seen == std::vector<std::string>{"one"});

  wh::adk::agent_message_stream_reader reader{
      wh::schema::stream::make_values_stream_reader(
          std::vector<wh::schema::message>{make_assistant_message("a"),
  ...
  wh::adk::agent_message_stream_reader failing_reader{
      wh::schema::stream::make_values_stream_reader(
...
TEST_CASE("event message stream reader flattens message events skips controls and maps errors and eof",
          "[UT][wh/adk/detail/event_message_stream_reader.hpp][event_message_stream_reader::read_impl][condition][branch][boundary]") {
  ...
  events.push_back(wh::adk::make_error_event(
      wh::core::make_error(wh::core::errc::unavailable), "boom"));

  wh::adk::detail::event_message_stream_reader flattened{
  ...
  REQUIRE(std::get<wh::schema::text_part>(second.value().value->parts.front()).text ==
          "nested-a");

  auto third = flattened.read();
  REQUIRE(third.has_value());
  REQUIRE(third.value().value.has_value());
  REQUIRE(std::get<wh::schema::text_part>(third.value().value->parts.front()).text ==
          "nested-b");
...
TEST_CASE("event message stream reader try_read async stop and close preserve nested reader state",
          "[UT][wh/adk/detail/event_message_stream_reader.hpp][event_message_stream_reader::read_async][condition][branch][boundary][concurrency]") {
