...
TEST_CASE("adk bind impl umbrella exposes concrete detail binders for all authored shell families",
  ...
  REQUIRE(
      wh::adk::detail::bind_plan_execute_agent(std::move(plan).value()).has_value());

  auto self_refine =
      wh::testing::helper::make_configured_self_refine("self-refine");
  REQUIRE(self_refine.has_value());
  REQUIRE(wh::adk::detail::bind_self_refine_agent(std::move(self_refine).value())
              .has_value());

  auto reviewer =
      wh::testing::helper::make_configured_reviewer_executor("reviewer");
  REQUIRE(reviewer.has_value());
  REQUIRE(wh::adk::detail::bind_reviewer_executor_agent(std::move(reviewer).value())
              .has_value());

  auto reflexion = wh::testing::helper::make_configured_reflexion("reflexion");
  REQUIRE(reflexion.has_value());
  REQUIRE(
      wh::adk::detail::bind_reflexion_agent(std::move(reflexion).value()).has_value());

  auto supervisor =
      wh::testing::helper::make_configured_supervisor("supervisor");
  REQUIRE(supervisor.has_value());
  REQUIRE(
      wh::adk::detail::bind_supervisor_agent(std::move(supervisor).value()).has_value());
  ...
  REQUIRE(
      wh::adk::detail::bind_research_agent(std::move(research).value()).has_value());
}
...
TEST_CASE("adk bind impl lowerings preserve authored names and descriptions after moves",
  ...
  auto react =
      wh::testing::helper::make_configured_react("react-move", "react-desc");
