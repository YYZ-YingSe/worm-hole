...
TEST_CASE("agent tool impl detail constants runtime access and metadata helpers stay stable",
          "[UT][wh/adk/detail/agent_tool_impl.hpp][agent_tool_access::make_runtime][condition][branch][boundary]") {
  ...
  REQUIRE(wh::adk::detail::agent_tool_interrupt_reason ==
  ...
  wh::adk::agent_tool unbound{"delegate", "delegate request",
  ...
  wh::adk::agent_tool tool{"delegate", "delegate request",
                           wh::agent::agent{"worker"}};
  REQUIRE(tool.bind_runner(
              [](const wh::adk::run_request &, wh::core::run_context &)
  ...
  auto default_metadata =
      wh::adk::detail::default_tool_metadata(runtime.value(), snapshot);
  REQUIRE(default_metadata.agent_name == "worker");
  REQUIRE(default_metadata.tool_name == "delegate");
  REQUIRE(default_metadata.run_path.to_string("/") ==
          "tool/delegate/call-1/agent/worker");

  wh::adk::event_metadata child_metadata{};
  child_metadata.run_path = wh::adk::run_path{{"agent", "leaf"}};
  auto normalized = wh::adk::detail::normalize_child_metadata(
      runtime.value(), snapshot, std::move(child_metadata));
  REQUIRE(normalized.run_path.to_string("/") ==
          "tool/delegate/call-1/agent/leaf");
  REQUIRE(normalized.agent_name == "worker");
  REQUIRE(normalized.tool_name == "delegate");
}

TEST_CASE("agent tool impl detail sanitizes checkpoints reifies events and preserves interrupt snapshots",
  ...
  wh::adk::detail::sanitize_checkpoint_any(move_only_payload);
  REQUIRE_FALSE(move_only_payload.has_value());

  wh::adk::event_metadata metadata{};
  metadata.attributes.emplace("copyable", wh::core::any{7});
  metadata.attributes.emplace("move-only", wh::core::any{std::make_unique<int>(9)});
  wh::adk::detail::sanitize_checkpoint_metadata(metadata);
  REQUIRE(metadata.attributes.contains("copyable"));
  REQUIRE_FALSE(metadata.attributes.contains("move-only"));

  auto message_event = wh::adk::detail::make_agent_tool_event(
      wh::adk::detail::agent_tool_event_record{
          .payload = wh::testing::helper::make_text_message(
              wh::schema::message_role::assistant, "chunk"),
          .metadata = {},
      });
  REQUIRE(std::holds_alternative<wh::adk::message_event>(message_event.payload));

  auto custom_event = wh::adk::detail::make_agent_tool_event(
      wh::adk::detail::agent_tool_event_record{
          .payload = wh::adk::detail::agent_tool_custom_event_record{
              .name = "custom",
  ...
  auto error_event = wh::adk::detail::make_agent_tool_event(
      wh::adk::detail::agent_tool_event_record{
          .payload = wh::adk::detail::agent_tool_error_event_record{
  ...
  auto checkpoint =
      wh::adk::detail::make_agent_tool_checkpoint_state(output);
  auto roundtrip =
      wh::adk::detail::make_agent_tool_output_summary(checkpoint);
...
TEST_CASE("agent tool impl detail parses requests builds run inputs and materializes terminal values",
          "[UT][wh/adk/detail/agent_tool_impl.hpp][materialize_agent_tool_value][condition][branch][boundary]") {
  ...
  wh::core::run_context context{};
  auto request = wh::adk::detail::build_agent_tool_request(
      request_runtime, request_call, context);
  ...
  auto custom_request = wh::adk::detail::build_agent_tool_request(
      custom_runtime, request_call, context);
  REQUIRE(custom_request.has_value());
  REQUIRE(custom_request->messages.size() == 1U);

  wh::model::chat_request history_request{};
  history_request.messages.push_back(wh::testing::helper::make_text_message(
      wh::schema::message_role::user, "history"));
  ...
  auto history = wh::adk::detail::build_agent_tool_request(
      history_runtime, history_call, context);
  REQUIRE(history.has_value());
  REQUIRE(history->messages.size() == 1U);

  REQUIRE(wh::adk::detail::default_message_history_schema().find("messages") !=
          std::string::npos);

  auto value = wh::adk::detail::materialize_agent_tool_value(
      wh::adk::agent_tool_result{
          .output_text = "joined",
      });
  ...
  auto failed = wh::adk::detail::materialize_agent_tool_value(
      wh::adk::agent_tool_result{
  ...
  auto empty = wh::adk::detail::materialize_agent_tool_value(
      wh::adk::agent_tool_result{});
