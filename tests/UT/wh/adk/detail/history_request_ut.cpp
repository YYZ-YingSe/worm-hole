...
TEST_CASE("history request helpers detect trailing tool calls rewrite react history and read payload views",
  ...
  REQUIRE(
      wh::adk::detail::history_request_has_tool_call_part(assistant_with_call));
  ...
  state.messages.push_back(
      wh::testing::helper::make_text_message(wh::schema::message_role::system,
                                             "system"));
  state.messages.push_back(
      wh::testing::helper::make_text_message(wh::schema::message_role::user,
                                             "question"));
  state.messages.push_back(
      wh::testing::helper::make_text_message(wh::schema::message_role::tool,
                                             "tool result"));
  ...
  REQUIRE(history_request.value().messages.front().role ==
          wh::schema::message_role::user);
  REQUIRE(std::get<wh::schema::text_part>(
              history_request.value().messages.front().parts.front())
              .text == "question");
  REQUIRE(std::get<wh::schema::text_part>(
              history_request.value().messages.back().parts.front())
              .text.find("For context: tool returned result: tool result.") !=
          std::string::npos);
  ...
  auto view_from_request =
      wh::adk::detail::read_history_request_payload_view(request_any);
  ...
  auto view_from_payload =
      wh::adk::detail::read_history_request_payload_view(payload_any);
  ...
  REQUIRE(*wh::core::any_cast<std::string>(view_from_payload.value().state_payload) ==
          "state");
  ...
  REQUIRE(*wh::core::any_cast<std::string>(&copied_payload.value().state_payload) ==
          "state");

  auto missing =
      wh::adk::detail::read_history_request_payload_view(wh::core::any{});
  REQUIRE(missing.has_error());
  REQUIRE(missing.error() == wh::core::errc::not_found);
  auto wrong_type =
      wh::adk::detail::read_history_request_payload_view(wh::core::any{3});
...
TEST_CASE("history request keeps trailing assistant text when the tail has no tool call",
          "[UT][wh/adk/detail/history_request.hpp][history_request_has_tool_call_part][condition][branch][boundary]") {
  wh::agent::react_state state{};
  state.messages.push_back(
      wh::testing::helper::make_text_message(wh::schema::message_role::system,
                                             "system"));
  ...
  REQUIRE(request.value().messages.back().role ==
          wh::schema::message_role::user);
  REQUIRE(std::get<wh::schema::text_part>(
              request.value().messages.back().parts.front())
