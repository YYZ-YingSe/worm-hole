...
TEST_CASE("revision detail helpers expose runtime context payload readers and shared-state lookup",
          "[UT][wh/adk/detail/"
          "revision_graph.hpp][revision_detail::make_context][condition][branch][boundary]") {
  ...
  auto messages = wh::adk::detail::revision_detail::read_messages_payload(messages_payload);
  REQUIRE(messages.has_value());
  REQUIRE(messages->size() == 1U);

  wh::compose::graph_value invalid_messages_payload = 7;
  auto invalid_messages =
      wh::adk::detail::revision_detail::read_messages_payload(invalid_messages_payload);
  ...
  auto invalid_output = wh::adk::detail::revision_detail::move_agent_output(invalid_output_payload);
...
TEST_CASE(
    "revision callbacks bootstrap request capture review parse and emit final draft branches",
    "[UT][wh/adk/detail/"
    "revision_graph.hpp][revision_detail::make_bootstrap_options][condition][branch][boundary]") {
  using runtime_state = wh::adk::detail::revision_detail::runtime_state;
  ...
  auto request = wh::adk::detail::revision_detail::make_request_options(
  ...
  old_draft.final_message =
      wh::testing::helper::make_text_message(wh::schema::message_role::assistant, "old draft");
  state->get().current_draft = old_draft;

  wh::agent::agent_output new_draft{};
  new_draft.final_message =
      wh::testing::helper::make_text_message(wh::schema::message_role::assistant, "new draft");
  payload = new_draft;
  ...
  old_review.final_message =
      wh::testing::helper::make_text_message(wh::schema::message_role::assistant, "old review");
  ...
  auto parse_review = wh::adk::detail::revision_detail::make_parse_review_options(
      wh::testing::helper::make_review_decision_reader(wh::agent::review_decision_kind::accept));
...
TEST_CASE(
    "revision graph lowerer and binder surfaces support self refine reviewer executor and "
    "reflexion shells",
    "[UT][wh/adk/detail/revision_graph.hpp][bind_reflexion_agent][condition][branch][boundary]") {
  auto draft_graph = wh::testing::helper::make_passthrough_graph("draft_node");
  auto review_graph = wh::testing::helper::make_passthrough_graph("review_node");
  ...
  auto with_memory_draft = wh::testing::helper::make_passthrough_graph("memory_node");
  ...
  auto bound_self_refine = wh::adk::detail::bind_self_refine_agent(std::move(self_refine).value());
  ...
  auto bound_reviewer = wh::adk::detail::bind_reviewer_executor_agent(std::move(reviewer).value());
  ...
  auto bound_reflexion = wh::adk::detail::bind_reflexion_agent(std::move(reflexion).value());
