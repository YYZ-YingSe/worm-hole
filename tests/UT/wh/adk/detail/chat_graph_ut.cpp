...
TEST_CASE("chat detail helpers normalize message streams instructions and output slots",
  ...
  auto instruction = wh::adk::detail::chat_detail::make_instruction_message("role", "prompt");
  REQUIRE(instruction.has_value());
  REQUIRE(instruction->role == wh::schema::message_role::system);
  REQUIRE(std::get<wh::schema::text_part>(instruction->parts.front()).text == "role\nprompt");
  REQUIRE_FALSE(wh::adk::detail::chat_detail::make_instruction_message("", "").has_value());
  ...
  REQUIRE(wh::adk::detail::chat_detail::render_message_text(rendered) == "alphabeta");

  wh::agent::agent_output output{};
  wh::adk::detail::chat_detail::write_output_value(output, "", wh::agent::chat_output_mode::value,
                                                   rendered);
  REQUIRE(output.output_values.empty());

  wh::adk::detail::chat_detail::write_output_value(output, "message",
                                                   wh::agent::chat_output_mode::value, rendered);
  ...
  wh::adk::detail::chat_detail::write_output_value(output, "text",
                                                   wh::agent::chat_output_mode::text, rendered);
...
TEST_CASE("chat graph lowers authored chat shells into executable compose graphs and binders",
  ...
  auto bound_authored = wh::testing::helper::make_configured_chat("bound-chat", "assistant");
