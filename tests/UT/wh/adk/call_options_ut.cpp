...
static_assert(std::same_as<wh::adk::option_string_hash, wh::core::transparent_string_hash>);
static_assert(std::same_as<wh::adk::option_string_equal, wh::core::transparent_string_equal>);
...
TEST_CASE("adk call options set_option and option_value_copy cover hit miss and type mismatch",
  ...
  REQUIRE(wh::adk::option_value_copy<int>(bag, "missing").error() == wh::core::errc::not_found);
...
TEST_CASE("adk call options named scope helpers upsert find and merge deterministic overlays",
  ...
  REQUIRE(wh::adk::set_option(wh::adk::detail::find_named_scope(next_scopes, "planner")->values,
                              "mode", std::string{"override"})
              .has_value());
  REQUIRE(wh::adk::set_option(wh::adk::detail::find_named_scope(next_scopes, "worker")->values,
                              "stage", std::string{"execute"})
              .has_value());
  ...
  REQUIRE(wh::adk::option_value_copy<std::string>(merged_planner->values, "mode").value() ==
          "override");
  REQUIRE(scopes.size() == 2U);
  REQUIRE(wh::adk::option_value_copy<std::string>(scopes[1].values, "stage").value() == "execute");
}
...
TEST_CASE("adk call options overlay helpers update only provided budget trim and impl fields",
  ...
  wh::adk::detail::overlay_budget(budget, {.max_concurrency = 2U, .fail_fast = true});
  ...
  wh::adk::detail::overlay_transfer_trim(trim, {.trim_assistant_transfer_message = true});
  wh::adk::detail::overlay_transfer_trim(trim, {.trim_tool_transfer_pair = true});
  ...
  REQUIRE(wh::adk::option_value_copy<std::string>(target["openai"], "model").value() == "gpt-5");
  REQUIRE(wh::adk::option_value_copy<std::string>(target["anthropic"], "model").value() ==
          "claude");
}
...
TEST_CASE("adk call options resolve_call_options overlays defaults workflow adk and call override",
  ...
  const auto merged = wh::adk::resolve_call_options(&defaults, &workflow, &adk, &call_override);

  REQUIRE(wh::adk::option_value_copy<double>(merged.global, "temperature").value() == 0.2);
...
TEST_CASE("adk call options materialize agent and tool scopes with correct visibility",
  ...
  REQUIRE(wh::adk::set_tool_option(merged, "search", "timeout", std::chrono::milliseconds{250})
              .has_value());
  ...
  REQUIRE(wh::adk::option_value_copy<double>(planner_scope.values, "temperature").value() == 0.2);
  REQUIRE(wh::adk::option_value_copy<std::string>(planner_scope.values, "mode").value() == "plan");
  REQUIRE(wh::adk::option_value_copy<std::chrono::milliseconds>(planner_scope.values, "timeout")
              .error() == wh::core::errc::not_found);
  REQUIRE(wh::adk::option_value_copy<std::string>(planner_scope.checkpoint_fields, "resume-id")
              .value() == "resume-1");
  ...
  REQUIRE(wh::adk::option_value_copy<std::chrono::milliseconds>(search_scope.values, "timeout")
              .value() == std::chrono::milliseconds{250});
  REQUIRE(wh::adk::option_value_copy<std::string>(search_scope.values, "temperature").error() ==
          wh::core::errc::type_mismatch);
  REQUIRE(wh::adk::option_value_copy<std::string>(search_scope.values, "mode").error() ==
