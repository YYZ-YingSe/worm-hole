...
TEST_CASE("clear tool result detail helpers estimate tokens and classify reducible tool messages",
          "[UT][wh/agent/middlewares/reduction/clear_tool_result.hpp][count_part_tokens][condition][branch][boundary]") {
  wh::schema::message_part text_part{wh::schema::text_part{"abcd"}};
  REQUIRE(wh::agent::middlewares::reduction::detail::count_part_tokens(
              text_part) == 1U);
  ...
  wh::schema::message user_message =
      make_text_message(wh::schema::message_role::user, "payload");
  wh::agent::middlewares::reduction::clear_tool_result_options options{};
  options.excluded_tool_names.insert("keep");
  REQUIRE(wh::agent::middlewares::reduction::detail::default_estimate_tokens(
              tool_message) == 2U);
  REQUIRE(wh::agent::middlewares::reduction::detail::should_reduce_message(
      tool_message, options));
  REQUIRE_FALSE(wh::agent::middlewares::reduction::detail::should_reduce_message(
      make_text_message(wh::schema::message_role::tool, "payload", "keep"),
      options));
  REQUIRE_FALSE(wh::agent::middlewares::reduction::detail::should_reduce_message(
      user_message, options));
}

TEST_CASE("clear tool result middleware trims only old tool messages outside protected suffix and honors custom estimator",
          "[UT][wh/agent/middlewares/reduction/clear_tool_result.hpp][make_clear_tool_result_middleware][condition][branch][boundary]") {
  ...
  request.messages.push_back(
      make_text_message(wh::schema::message_role::tool, "old tool payload",
                        "search"));
  request.messages.push_back(
      make_text_message(wh::schema::message_role::tool, "excluded payload",
                        "keep"));
  request.messages.push_back(
      make_text_message(wh::schema::message_role::user, "recent user text"));
  request.messages.push_back(
      make_text_message(wh::schema::message_role::tool, "recent tool payload",
                        "search"));
  ...
  REQUIRE(std::get<wh::schema::text_part>(unchanged.messages.front().parts.front())
              .text == "short");
          ...
          wh::agent::middlewares::reduction::clear_tool_result_options{
              ...
              .estimate_tokens =
                  wh::agent::middlewares::reduction::token_estimator{
                      [](const wh::schema::message &) -> std::size_t {
                        return 10U;
                      }},
          });
  REQUIRE(custom(fallback_placeholder).has_value());
  REQUIRE(std::get<wh::schema::text_part>(
              fallback_placeholder.messages.front().parts.front())
              .text == "[tool result omitted]");
}
