...
TEST_CASE("agent toolset validates raw entry schemas and handler presence",
  ...
  auto invalid_entry = tools.add_entry(
      {.name = "noimpl", .description = "missing handlers"}, bad_entry);
...
TEST_CASE("agent toolset make_tool_entry bridges sync invoke tools into compose values",
  ...
  auto invoked =
      entry.invoke({.tool_name = "sync", .arguments = "payload"},
                   {.run = context,
                    .component = "tools",
                    .implementation = "tool",
                    .tool_name = "sync",
...
TEST_CASE("agent toolset make_tool_entry bridges async stream tools into graph stream readers",
  ...
  auto stream_status = stdexec::sync_wait(entry.async_stream(
      {.tool_name = "stream", .arguments = "chunk"},
      {.run = context,
       .component = "tools",
       .implementation = "tool",
       .tool_name = "stream",
...
TEST_CASE("agent toolset preserves middleware and node authoring options",
  ...
  REQUIRE(tools.set_node_options(
