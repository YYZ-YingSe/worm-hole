...
TEST_CASE("self refine shell binds worker optional reviewer and lowers into executable agent",
  ...
  REQUIRE(authored.set_worker_request_builder(
...
TEST_CASE("self refine shell rejects missing builders non executable worker and late mutation",
          "[UT][wh/agent/self_refine.hpp][self_refine::set_worker_request_builder][condition][branch][boundary]") {
  ...
  REQUIRE(non_executable.set_worker_request_builder(
              wh::testing::helper::make_revision_request_builder())
              .has_value());
  REQUIRE(non_executable.set_reviewer_request_builder(
              wh::testing::helper::make_revision_request_builder())
              .has_value());
  REQUIRE(non_executable.set_review_decision_reader(
              wh::testing::helper::make_review_decision_reader(
                  wh::agent::review_decision_kind::accept))
              .has_value());
  ...
  auto configured =
      wh::testing::helper::make_configured_self_refine("configured");
  REQUIRE(configured.has_value());
  REQUIRE(configured->freeze().has_value());
  REQUIRE(configured->set_max_iterations(3U).has_error());
  REQUIRE(configured->set_worker_request_builder(
              wh::testing::helper::make_revision_request_builder())
              .has_error());
