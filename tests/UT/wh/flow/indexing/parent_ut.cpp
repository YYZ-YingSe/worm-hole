...
TEST_CASE("indexing parent flow derives parent ids and writes transformed chunks",
  ...
  wh::flow::indexing::parent flow{
      ...
      [](const wh::schema::document &document,
        ...
        return std::vector<wh::schema::document>{wh::schema::document{document.content() + "/0"},
  ...
  parent_document.set_metadata(std::string{wh::flow::indexing::parent_id_metadata_key}, "pid");
  REQUIRE(wh::flow::indexing::detail::indexing::effective_parent_id(parent_document) == "pid");
...
TEST_CASE("indexing parent flow rejects empty transforms and id count mismatch",
  ...
  wh::flow::indexing::parent empty_transform{
      indexer,
      [](const wh::schema::document &,
         wh::core::run_context &) -> wh::core::result<std::vector<wh::schema::document>> {
        return std::vector<wh::schema::document>{};
      },
      [](const wh::schema::document &, const std::size_t, wh::core::run_context &)
          -> wh::core::result<std::vector<std::string>> { return std::vector<std::string>{}; }};
...
TEST_CASE("indexing parent helper falls back to document content when parent id metadata is absent",
  ...
  explicit_parent.set_metadata(std::string{wh::flow::indexing::parent_id_metadata_key}, "parent-a");
  REQUIRE(wh::flow::indexing::detail::indexing::effective_parent_id(explicit_parent) == "parent-a");

  wh::schema::document fallback_parent{"content-b"};
  REQUIRE(wh::flow::indexing::detail::indexing::effective_parent_id(fallback_parent) ==
          "content-b");
}
