...
struct query_retriever_impl {
  ...
  [[nodiscard]] auto retrieve(const wh::retriever::retriever_request &request) const
    ...
    return wh::retriever::retriever_response{
        wh::schema::document{"doc:" + request.query}};
  }
};
...
TEST_CASE("multi query helpers expose stable keys and rewrite helpers",
  ...
  REQUIRE(wh::flow::retrieval::detail::multi_query::render_message_text(message) ==
          "alphabeta");
  ...
  REQUIRE(rewritten.value() ==
          std::vector<std::string>{"alpha", "alpha", "beta"});

  REQUIRE(wh::flow::retrieval::detail::multi_query::request_node_key() ==
          "multi_query_request");
  REQUIRE(wh::flow::retrieval::detail::multi_query::rewrite_node_key() ==
          "multi_query_rewrite");
  REQUIRE(wh::flow::retrieval::detail::multi_query::batch_node_key() ==
          "multi_query_batch");
  REQUIRE(wh::flow::retrieval::detail::multi_query::tools_node_key() ==
          "multi_query_tools");
  REQUIRE(wh::flow::retrieval::detail::multi_query::fusion_node_key() ==
          "multi_query_fusion");
  REQUIRE(wh::flow::retrieval::detail::multi_query::tool_name() ==
...
TEST_CASE("multi query flow rewrites clips and fuses retrieved documents",
  ...
  wh::flow::retrieval::multi_query mq{
      wh::retriever::retriever{query_retriever_impl{}}, rewriter};
  ...
  REQUIRE(std::find_if(documents.begin(), documents.end(),
...
TEST_CASE("multi query flow propagates rewrite and fusion failures",
  ...
  auto failing_rewriter = [](const wh::retriever::retriever_request &,
                             wh::core::run_context &)
      -> wh::core::result<std::vector<std::string>> {
...
TEST_CASE("multi query flow clips expanded queries and rejects empty rewrite output",
          "[UT][wh/flow/retrieval/multi_query.hpp][multi_query::set_max_queries][condition][boundary]") {
  auto repeated_rewriter = [](const wh::retriever::retriever_request &,
                              wh::core::run_context &)
      -> wh::core::result<std::vector<std::string>> {
