...
TEST_CASE("timer helper exposes timed scheduler operations and timeout senders",
  ...
  auto after = stdexec::sync_wait(
      wh::core::schedule_after(scheduler, std::chrono::milliseconds{1}));
  REQUIRE(after.has_value());

  auto at = stdexec::sync_wait(
      wh::core::schedule_at(scheduler, now + std::chrono::milliseconds{1}));
  REQUIRE(at.has_value());

  auto timeout_sender = wh::core::timeout<wh::core::result<int>>(
      scheduler, stdexec::just(wh::core::result<int>{7}),
...
TEST_CASE("timer helper timeout helpers surface timeout branch for slow senders",
  ...
  auto slow_sender =
      wh::core::schedule_after(scheduler, std::chrono::milliseconds{10}) |
      stdexec::then([]() noexcept { return wh::core::result<int>{1}; });

  auto timed_out = wh::core::timeout<wh::core::result<int>>(
      scheduler, std::move(slow_sender), std::chrono::milliseconds{1});
