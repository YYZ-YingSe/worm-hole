...
struct async_indexer_impl {
  [[nodiscard]] auto write_sender(const wh::indexer::indexer_request &request) const {
    ...
    return stdexec::just(
        wh::core::result<wh::indexer::indexer_response>{std::move(response)});
  }
};
...
TEST_CASE("indexer wrapper normalizes async sender outputs",
  ...
  REQUIRE(std::get<0>(*awaited).value().document_ids ==
          std::vector<std::string>{"a-async"});
}
...
TEST_CASE(
  ...
  combined_request.options.set_base(wh::indexer::indexer_common_options{
      .failure_policy = wh::indexer::write_failure_policy::stop,
      .embedding_model = "",
      .combine_with_embedding = true});

  auto invalid = wh::indexer::detail::run_sync_indexer_impl(
      failing_single_indexer_impl{}, combined_request);
  ...
  retry_request.options.set_base(wh::indexer::indexer_common_options{
      ...
      .max_retries = 1U});
  auto exhausted = wh::indexer::detail::run_sync_indexer_impl(
      failing_single_indexer_impl{}, retry_request);
  ...
  auto skipped = wh::indexer::detail::run_sync_indexer_impl(
      failing_single_indexer_impl{}, skip_request);
