...
TEST_CASE(
    "simple chat template renders messages and records failures under strict mode",
    "[UT][wh/prompt/simple_chat_template.hpp][simple_chat_template::render][branch][boundary]") {
  wh::prompt::simple_chat_template prompt{};
  prompt.add_template(
      {.role = wh::schema::message_role::system, .text = "Hello {{ name }}", .name = "welcome"});
  ...
  ok_request.options.set_base(wh::prompt::prompt_common_options{.template_name = "chat"});
  ...
  REQUIRE(std::get<wh::schema::text_part>(rendered.value()[0].parts.front()).text == "Hello Ada");
  ...
  missing_request.options.set_base(
      wh::prompt::prompt_common_options{.strict_missing_variables = true, .template_name = "chat"});
...
TEST_CASE("simple chat template keeps raw text when strict missing variables is disabled",
  ...
  request.options.set_base(wh::prompt::prompt_common_options{.strict_missing_variables = false,
...
TEST_CASE(
    "simple chat template falls back to template-name when unnamed entry fails",
    "[UT][wh/prompt/"
    "simple_chat_template.hpp][simple_chat_template::add_template][condition][branch][boundary]") {
  wh::prompt::simple_chat_template prompt{};
  prompt.add_template({.role = wh::schema::message_role::user, .text = "{{ missing }}"});

  wh::prompt::prompt_render_request request{};
  request.options.set_base(wh::prompt::prompt_common_options{.strict_missing_variables = true,
