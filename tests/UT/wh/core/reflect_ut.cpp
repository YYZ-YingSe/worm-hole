...
TEST_CASE("reflect field binding and validation cover name and key constraints",
  ...
  constexpr wh::core::field_binding<test_profile, int> empty_name{"", 1U, &test_profile::id};
  constexpr wh::core::field_binding<test_profile, int> duplicate_name{"id", 2U, &test_profile::id};
  constexpr wh::core::field_binding<test_profile, std::string> duplicate_key{"name", id_field.key,
                                                                             &test_profile::name};

  STATIC_REQUIRE(wh::core::validate_field_map<test_profile>(id_field, name_field));
  STATIC_REQUIRE_FALSE(wh::core::validate_field_map<test_profile>(empty_name, name_field));
  STATIC_REQUIRE_FALSE(wh::core::validate_field_map<test_profile>(id_field, duplicate_name));
  STATIC_REQUIRE_FALSE(wh::core::validate_field_map<test_profile>(id_field, duplicate_key));
}
...
TEST_CASE("reflect field map exposes names keys and member references",
  ...
  constexpr auto id_field = wh::core::field("id", &test_profile::id);
  constexpr auto name_field = wh::core::field("name", &test_profile::name);
  constexpr auto enabled_field = wh::core::field("enabled", &test_profile::enabled);
  constexpr auto field_map =
      wh::core::make_field_map<test_profile>(id_field, name_field, enabled_field);

  STATIC_REQUIRE(std::same_as<decltype(field_map)::owner_type, test_profile>);
  REQUIRE(field_map.size() == 3U);
  REQUIRE(field_map.names() == std::array<std::string_view, 3U>{"id", "name", "enabled"});
...
TEST_CASE("reflect field iteration and lookup cover hit miss and key branches",
  ...
  wh::core::for_each_field(field_map, [&](const auto &binding) { names.push_back(binding.name); });
  REQUIRE(names == std::vector<std::string_view>{"id", "name", "enabled"});

  test_profile profile{.id = 1, .name = "carol", .enabled = false};
  bool id_branch_called = false;
  bool name_branch_called = false;

  const bool found_id = wh::core::visit_field(field_map, "id", [&](const auto &binding) {
  ...
  const bool found_missing = wh::core::visit_field(
      field_map, "missing", [&](const auto &) { FAIL("missing field must not invoke callback"); });
  REQUIRE_FALSE(found_missing);

  bool found_name_by_key = false;
  const auto name_key = wh::internal::stable_name_hash("name");
  const bool found_key =
      wh::core::visit_field_by_key(field_map, name_key, [&](const auto &binding) {
        using binding_t = std::remove_cvref_t<decltype(binding)>;
...
TEST_CASE("reflect type key helpers keep alias lookup stable",
  ...
  STATIC_REQUIRE(std::same_as<std::remove_cvref_t<decltype(alpha_key)>, wh::core::type_key>);
  REQUIRE(alpha_key.value != 0U);
  REQUIRE(beta_key.value != 0U);
  REQUIRE(alpha_key != beta_key);

  const auto found_alpha = wh::core::find_type_key<registry_alpha, registry_beta>("registry_alpha");
  const auto found_beta = wh::core::find_type_key<registry_alpha, registry_beta>("registry_beta");
  const auto missing = wh::core::find_type_key<registry_alpha, registry_beta>("missing");
  ...
  REQUIRE(wh::core::find_type_alias<registry_alpha, registry_beta>(alpha_key) == "registry_alpha");
  REQUIRE(wh::core::find_type_alias<registry_alpha, registry_beta>(beta_key) == "registry_beta");
  REQUIRE(wh::core::find_type_alias<registry_alpha, registry_beta>(wh::core::type_key{0U}).empty());
}
