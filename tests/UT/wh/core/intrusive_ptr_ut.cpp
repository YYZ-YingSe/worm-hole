...
struct construction_probe
    : wh::core::detail::intrusive_enable_from_this<construction_probe> {
  static inline const construction_probe *observed_self{nullptr};

  construction_probe() {
    observed_self = this->intrusive_from_this().get();
  }
};

struct destructor_probe
    : wh::core::detail::intrusive_enable_from_this<destructor_probe> {
  static inline std::size_t destroyed{0U};

  ~destructor_probe() { ++destroyed; }
};

struct reserved_bit_probe
    : wh::core::detail::intrusive_enable_from_this<reserved_bit_probe, 1U> {
  [[nodiscard]] auto marked() const noexcept -> bool {
    return this->template is_set<0U>();
  }

  auto mark() noexcept -> void { static_cast<void>(this->template set_bit<0U>()); }

  auto clear() noexcept -> void {
    static_cast<void>(this->template clear_bit<0U>());
  }
};
