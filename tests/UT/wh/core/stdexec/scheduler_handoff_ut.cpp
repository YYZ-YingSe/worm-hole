...
TEST_CASE(
    "scheduler handoff same_scheduler query and sender-like concepts cover both sender families",
  ...
  REQUIRE_FALSE(wh::core::detail::scheduler_handoff::same_scheduler(different_scheduler));
  REQUIRE_FALSE(wh::core::detail::scheduler_handoff::same_scheduler(stdexec::inline_scheduler{}));

  using schedule_sender_t =
      decltype(wh::core::detail::scheduler_handoff::make_schedule_handoff_sender(same_scheduler));
  using try_sender_t =
      decltype(wh::core::detail::scheduler_handoff::make_try_schedule_handoff_sender(
          try_scheduler_t{}));

  STATIC_REQUIRE(
      wh::core::detail::scheduler_handoff::schedule_handoff_sender_like<schedule_sender_t>);
  STATIC_REQUIRE(
      wh::core::detail::scheduler_handoff::try_schedule_handoff_sender_like<try_sender_t>);
}
...
TEST_CASE("scheduler handoff schedule sender uses target scheduler and env projection",
  ...
  auto sender = wh::core::detail::scheduler_handoff::make_schedule_handoff_sender(scheduler);
  ...
  auto operation = stdexec::connect(std::move(sender),
...
TEST_CASE("scheduler handoff try-schedule sender covers success inline and error branches",
          "[UT][wh/core/stdexec/"
          "scheduler_handoff.hpp][make_try_schedule_handoff_sender][condition][branch][boundary]") {
  wh::testing::helper::manual_scheduler_state state{};
  const auto scheduler = try_scheduler_t{&state};
  auto sender = wh::core::detail::scheduler_handoff::make_try_schedule_handoff_sender(scheduler);

  REQUIRE(sender.target_scheduler().state == &state);

  wh::testing::helper::sender_capture<void> value_capture{};
  auto value_operation =
      stdexec::connect(sender, wh::testing::helper::sender_capture_receiver<void>{&value_capture});
  ...
  auto inline_operation =
      stdexec::connect(sender, wh::testing::helper::sender_capture_receiver<void>{&inline_capture});
  stdexec::start(inline_operation);
  REQUIRE(inline_capture.ready.try_acquire());
  REQUIRE(inline_capture.terminal == wh::testing::helper::sender_terminal_kind::value);
  ...
  auto error_operation =
      stdexec::connect(sender, wh::testing::helper::sender_capture_receiver<void>{&error_capture});
