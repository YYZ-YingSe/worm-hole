...
struct stop_observer {
  ...
  auto operator()() const noexcept -> void { fired->store(true, std::memory_order_release); }
};
...
TEST_CASE("receiver_stop_bridge forwards outer stop requests into the inner stop env",
  ...
  stdexec::stop_callback_for_t<stdexec::inplace_stop_token, stop_observer> callback{
      inner_token, stop_observer{&inner_stopped}};
...
TEST_CASE("receiver_stop_bridge completes at most once and respects pre-stopped outer tokens",
          "[UT][wh/core/stdexec/detail/"
          "receiver_stop_bridge.hpp][receiver_stop_bridge::set_value][boundary][branch]") {
  {
    wh::testing::helper::sender_capture<int> capture{};
    using receiver_t = wh::testing::helper::sender_capture_receiver<int, tagged_env>;
...
TEST_CASE(
    "receiver_stop_bridge construction propagates stop callback setup failures before publication",
    "[UT][wh/core/stdexec/detail/"
