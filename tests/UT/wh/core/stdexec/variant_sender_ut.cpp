...
TEST_CASE("variant sender default constructor assignment and emplace select branches",
  ...
  constexpr auto append_done =
      +[](std::string value) { return value + "-done"; };

  using sender_t = wh::core::detail::variant_sender<
      decltype(stdexec::just(std::string{"branch-a"})),
  ...
  REQUIRE(sender.index() == 0U);
  REQUIRE(wh::testing::helper::wait_value_on_test_thread(sender) ==
          "reassigned-a");

  sender.emplace<1U>(stdexec::just(std::string{"branch-b"}) |
                     stdexec::then(append_done));
  REQUIRE(sender.index() == 1U);
  REQUIRE(wh::testing::helper::wait_value_on_test_thread(std::move(sender)) ==
          "branch-b-done");
}
...
TEST_CASE("variant sender chooses only the active runtime branch",
  ...
  using sender_t = wh::core::detail::variant_sender<
      decltype(stdexec::just(1)),
      decltype(stdexec::just(2) | stdexec::then(plus_one))>;

  const auto first_status =
      wh::testing::helper::wait_value_on_test_thread([&]() -> sender_t {
        if (true) {
          first_called = true;
          return sender_t{std::in_place_index<0U>, stdexec::just(1)};
        }
        second_called = true;
        return sender_t{std::in_place_index<1U>,
                        stdexec::just(2) | stdexec::then(plus_one)};
      }());
