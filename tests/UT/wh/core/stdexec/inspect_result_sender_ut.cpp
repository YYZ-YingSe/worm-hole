...
TEST_CASE("inspect result sender observes successful status and forwards it unchanged",
  ...
  auto sender = wh::core::detail::inspect_result_sender(
      stdexec::just(wh::core::result<int>{5}),
...
TEST_CASE("inspect result sender observes error status and forwards it unchanged",
  ...
  auto sender = wh::core::detail::inspect_result_sender(
      stdexec::just(wh::core::result<std::string>::failure(
...
TEST_CASE("inspect result sender also forwards result<void> without altering readiness",
          "[UT][wh/core/stdexec/inspect_result_sender.hpp][inspect_result_sender][condition][branch][boundary]") {
  bool inspector_called = false;

  auto sender = wh::core::detail::inspect_result_sender(
      stdexec::just(wh::core::result<void>{}),
