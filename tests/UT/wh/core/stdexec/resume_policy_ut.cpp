...
TEST_CASE("resume policy resume_if covers unchanged passthrough and restore scheduling",
  ...
  REQUIRE(wh::testing::helper::wait_value_on_test_thread(std::move(unchanged_sender)) ==
          3);

  wh::testing::helper::manual_scheduler_state state{};
  auto restored_sender = wh::core::detail::resume_if<resume_mode::restore>(
      stdexec::just(5), scheduler_t{&state});

  wh::testing::helper::sender_capture<int> capture{};
  auto operation = stdexec::connect(
      std::move(restored_sender),
...
TEST_CASE("resume policy defer_resume_sender reads scheduler only in restore mode",
  ...
  auto unchanged_sender =
      wh::core::detail::defer_resume_sender<resume_mode::unchanged>(
          [&saw_passthrough](auto scheduler_or_passthrough) {
            using param_t = std::remove_cvref_t<decltype(scheduler_or_passthrough)>;
            STATIC_REQUIRE(
                std::same_as<param_t, wh::core::detail::resume_passthrough_t>);
  ...
  auto wrapped_sender = wh::core::detail::write_sender_scheduler(
      std::move(restore_sender), scheduler_ref);
...
TEST_CASE("resume policy unchanged mode ignores provided schedulers completely",
  ...
  wh::testing::helper::manual_scheduler_state state{};
  auto sender = wh::core::detail::resume_if<wh::core::resume_mode::unchanged>(
      stdexec::just(17), scheduler_t{&state});
