...
TEST_CASE(
    "map result helpers cover value void nested-result and error branches",
  ...
  const auto mapped_value = wh::core::detail::map_result_value<int_result>(7);
  REQUIRE(mapped_value.has_value());
  REQUIRE(mapped_value.value() == 7);

  const auto mapped_nested = wh::core::detail::map_result_value<int_result>(int_result{8});
  ...
  const auto nested_status = wh::core::detail::map_result_status<string_result>(
      [](int value) { return string_result{std::to_string(value + 1)}; }, int_result{9});
  ...
  const auto propagated_error = wh::core::detail::map_result_status<string_result>(
      [](int) { return std::string{"unused"}; }, int_result::failure(wh::core::errc::not_found));
...
TEST_CASE("map result sender covers sender pipeline conversions",
  ...
  auto mapped_string = wh::core::detail::map_result_sender<wh::core::result<std::string>>(
      stdexec::just(wh::core::result<int>{4}), [](int value) { return std::to_string(value * 2); });
  ...
  auto mapped_void = wh::core::detail::map_result_sender<wh::core::result<void>>(
      stdexec::just(wh::core::result<int>{3}), [](int value) { REQUIRE(value == 3); });
  const auto void_status = wh::testing::helper::wait_value_on_test_thread(std::move(mapped_void));
  REQUIRE(void_status.has_value());
}
