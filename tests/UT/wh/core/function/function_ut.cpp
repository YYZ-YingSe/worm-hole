...
TEST_CASE("standard_function supports callable construction swap nullptr "
  ...
  wh::core::standard_function<int(int)> add_one{
...
TEST_CASE(
    "function_ref observes external callable state without taking ownership",
...
TEST_CASE(
    "function facade aligns copyability and const-callable construction rules",
    "[UT][wh/core/function/"
  ...
  STATIC_REQUIRE(
      std::is_constructible_v<const_callback_t, decltype((copyable))>);

  const const_callback_t callback{copyable};
  REQUIRE(callback(3) == 7);

  auto lower = [payload = std::make_unique<int>(7)]() mutable {
    return *payload;
  };
  STATIC_REQUIRE(std::is_constructible_v<wh::core::move_only_function<int()>,
                                         decltype(lower)>);
  STATIC_REQUIRE(!std::is_constructible_v<wh::core::callback_function<int()>,
                                          decltype(lower)>);
