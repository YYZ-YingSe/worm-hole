...
struct noexcept_callable {
  [[nodiscard]] auto operator()(int value) const noexcept -> int { return value + 3; }
};
...
template <typename signature_t> struct standard_probe : wh::core::fn::standard_accept<signature_t> {
  template <typename fun_t> [[nodiscard]] static consteval auto eligible() -> bool {
    return standard_probe::template is_eligible<fun_t>;
  }
};

template <typename signature_t> struct move_probe : wh::core::fn::non_copyable_accept<signature_t> {
  template <typename fun_t> [[nodiscard]] static consteval auto eligible() -> bool {
    return move_probe::template is_eligible<fun_t>;
  }
};

template <typename signature_t> struct ptr_probe : wh::core::fn::ptr_accept<signature_t> {
  template <typename fun_t> [[nodiscard]] static consteval auto eligible() -> bool {
...
static_assert(!standard_probe<int(int)>::template eligible<std::unique_ptr<copyable_callable>>());
static_assert(ptr_probe<int(int)>::template eligible<std::unique_ptr<copyable_callable>>());
static_assert(move_ptr_probe<int(int)>::template eligible<std::unique_ptr<move_only_callable>>());
static_assert(!standard_probe<int(int) noexcept>::template eligible<copyable_callable>());
static_assert(standard_probe<int(int) noexcept>::template eligible<noexcept_callable>());
...
static_assert(standard_probe<int(int) const &>::template eligible<const_lvalue_callable>());

} // namespace

TEST_CASE(
    "standard_accept respects copyability and cv-ref qualified signatures",
  ...
  REQUIRE_FALSE(standard_probe<int(int)>::template eligible<move_only_callable>());
  REQUIRE(standard_probe<int(int) &>::template eligible<lvalue_only_callable>());
  REQUIRE_FALSE(standard_probe<int(int) &&>::template eligible<lvalue_only_callable>());
  REQUIRE(standard_probe<int(int) &&>::template eligible<rvalue_only_callable>());
  REQUIRE(standard_probe<int(int) const &>::template eligible<const_lvalue_callable>());
}

TEST_CASE(
    "non_copyable acceptance policies admit move-only callables when requested",
  ...
  REQUIRE(move_ptr_probe<int(int)>::template eligible<std::unique_ptr<move_only_callable>>());
...
TEST_CASE("pointer and noexcept acceptance policies distinguish pointee invocation requirements",
          ...
          "acceptance_policy.hpp][ptr_accept][condition][branch][boundary]") {
  REQUIRE_FALSE(standard_probe<int(int)>::template eligible<std::unique_ptr<copyable_callable>>());
  REQUIRE(ptr_probe<int(int)>::template eligible<std::unique_ptr<copyable_callable>>());
  REQUIRE_FALSE(standard_probe<int(int) noexcept>::template eligible<copyable_callable>());
  REQUIRE(standard_probe<int(int) noexcept>::template eligible<noexcept_callable>());
}
