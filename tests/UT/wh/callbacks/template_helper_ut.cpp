...
using with_stage_callback_t =
    decltype([](const wh::callbacks::stage, const int &, const wh::callbacks::run_info &) {});
using without_stage_callback_t = decltype([](const int &, const wh::callbacks::run_info &) {});
using invalid_typed_callback_t = decltype([](double) {});

static_assert(wh::callbacks::TypedStageCallbackWithStage<with_stage_callback_t, int>);
static_assert(wh::callbacks::TypedStageCallbackWithoutStage<without_stage_callback_t, int>);
static_assert(!wh::callbacks::TypedStageCallbackWithStage<invalid_typed_callback_t, int>);
static_assert(!wh::callbacks::TypedStageCallbackWithoutStage<invalid_typed_callback_t, int>);

auto start_only_config() -> wh::callbacks::callback_config {
  return wh::callbacks::make_callback_config([](const wh::callbacks::stage stage) noexcept {
...
TEST_CASE("template helper typed stage callback ignores mismatched payload types",
          "[UT][wh/callbacks/"
          "template_helper.hpp][make_typed_stage_callback][condition][branch][boundary]") {
  ...
  callback(wh::callbacks::stage::start, wh::callbacks::make_event_view(observed), info);
...
TEST_CASE(
    "template helper make_typed_stage_callbacks installs the same typed callback for every stage",
    "[UT][wh/callbacks/"
    "template_helper.hpp][make_typed_stage_callbacks][condition][branch][boundary]") {
  std::vector<wh::callbacks::stage> stages{};
  auto callbacks = wh::callbacks::make_typed_stage_callbacks<int>(
      [&stages](const wh::callbacks::stage stage, const int &, const wh::callbacks::run_info &) {
...
TEST_CASE(
    "template helper register_typed_local_callbacks covers moved and copied run-context inputs",
    "[UT][wh/callbacks/"
    "template_helper.hpp][register_typed_local_callbacks][condition][branch][boundary]") {
  ...
  wh::core::inject_callback_event(moved.value(), wh::core::callback_stage::start, 3,
                                  wh::core::callback_run_info{});
  wh::core::inject_callback_event(moved.value(), wh::core::callback_stage::end, 5,
                                  wh::core::callback_run_info{});
  ...
  wh::core::inject_callback_event(copied.value(), wh::core::callback_stage::start, 4,
                                  wh::core::callback_run_info{});
