...
using stage_callback_like_t = decltype([](wh::callbacks::stage, wh::callbacks::event_view,
                                          const wh::callbacks::run_info &) {});
using not_stage_callback_like_t = decltype([](int) {});

static_assert(wh::callbacks::detail::StageCallbackLike<stage_callback_like_t>);
static_assert(!wh::callbacks::detail::StageCallbackLike<not_stage_callback_like_t>);
...
TEST_CASE("stage callback builder reports empty before any stage callback is set",
          "[UT][wh/callbacks/"
          "callback_builder.hpp][stage_callback_builder::empty][condition][branch][boundary]") {
...
TEST_CASE(
    "stage callback builder rejects build when no stage callback exists",
    "[UT][wh/callbacks/"
    "callback_builder.hpp][stage_callback_builder::build_callbacks][condition][branch][boundary]") {
  wh::callbacks::stage_callback_builder builder{};
...
TEST_CASE("stage callback builder installs every stage hook and reset clears them",
          "[UT][wh/callbacks/"
          "callback_builder.hpp][stage_callback_builder::reset][condition][branch][boundary]") {
  wh::callbacks::stage_callback_builder builder{};
  std::vector<wh::callbacks::stage> seen{};

  builder.on_start([&seen](const wh::callbacks::stage stage, const wh::callbacks::event_view,
                           const wh::callbacks::run_info &) { seen.push_back(stage); });
  REQUIRE_FALSE(builder.empty());

  auto built =
      builder
          .on_end([&seen](const wh::callbacks::stage stage, const wh::callbacks::event_view,
                          const wh::callbacks::run_info &) { seen.push_back(stage); })
          .on_error([&seen](const wh::callbacks::stage stage, const wh::callbacks::event_view,
                            const wh::callbacks::run_info &) { seen.push_back(stage); })
          .on_stream_start([&seen](const wh::callbacks::stage stage,
                                   const wh::callbacks::event_view,
                                   const wh::callbacks::run_info &) { seen.push_back(stage); })
          .on_stream_end([&seen](const wh::callbacks::stage stage, const wh::callbacks::event_view,
                                 const wh::callbacks::run_info &) { seen.push_back(stage); })
          .build_callbacks();
