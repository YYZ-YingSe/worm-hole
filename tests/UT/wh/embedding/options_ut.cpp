...
TEST_CASE("embedding options resolve base and override layers",
  ...
  REQUIRE(base_view.failure_policy ==
          wh::embedding::batch_failure_policy::partial_success);
...
TEST_CASE("embedding options keep component specific payload",
  ...
  const auto *probe =
      options.component_options().impl_specific_if<embedding_options_probe>();
