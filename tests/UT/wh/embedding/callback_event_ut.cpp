...
static_assert(std::is_default_constructible_v<wh::embedding::embedding_callback_event>);
...
TEST_CASE("embedding callback event preserves aggregate token usage through copy and move",
          "[UT][wh/embedding/"
          "callback_event.hpp][embedding_callback_event][condition][branch][boundary]") {
  const wh::embedding::embedding_callback_event source{
      .model_id = "embed-v1",
      .usage =
          wh::schema::token_usage{.prompt_tokens = 10, .completion_tokens = 0, .total_tokens = 10},
      .batch_size = 3U};
