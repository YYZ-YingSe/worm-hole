...
TEST_CASE("document parser supports extension routing fallback and explicit missing parser",
          "[core][document][functional]") {
  wh::document::parser::ext_parser parser{};
  auto parsed = parser.parse(wh::document::parser::parse_request{
      "content",
      wh::document::parser::parse_options{"test.unknown", {{"source", "unit-test"}}, {}}});
  REQUIRE(parsed.has_value());
  REQUIRE(parsed.value().size() == 1U);
  REQUIRE(parsed.value().front().metadata_or<std::string>("_source", "") == "test.unknown");

  wh::document::parser::ext_parser no_fallback{};
  no_fallback.clear_fallback();
  auto missing = no_fallback.parse(wh::document::parser::parse_request{
      "content", wh::document::parser::parse_options{"test.unknown", {}, {}}});
...
TEST_CASE("document parser rvalue paths keep stable uri for routing and metadata",
  ...
  public:
    ...
    [[nodiscard]] auto descriptor() const -> wh::core::component_descriptor {
      return wh::core::component_descriptor{"TaggedParser", wh::core::component_kind::document};
    }
  ...
  auto parsed = component.process(
      wh::document::document_request{.source_kind = wh::document::document_source_kind::uri,
                                     .source = "in-memory://payload"},
      callback_context);
  REQUIRE(parsed.has_value());
  REQUIRE(parsed.value().size() == 1U);
  REQUIRE(parsed.value().front().metadata_or<std::string>("_source", "") == "in-memory://payload");
}
...
TEST_CASE("document parser handle keeps value semantics for small and large impls",
  ...
  public:
    ...
    [[nodiscard]] auto descriptor() const -> wh::core::component_descriptor {
      return wh::core::component_descriptor{"SmallParser", wh::core::component_kind::document};
    }
  ...
  public:
    explicit large_parser(std::string tag) : tag_(std::move(tag)) { padding_.fill(0x2A); }

    [[nodiscard]] auto descriptor() const -> wh::core::component_descriptor {
      return wh::core::component_descriptor{"LargeParser", wh::core::component_kind::document};
    }
    ...
    [[nodiscard]] auto parse(const wh::document::parser::parse_request &request) const
      ...
      wh::schema::document doc{request.content};
      doc.set_metadata("tag", tag_);
      doc.set_metadata("padding", std::to_string(static_cast<int>(padding_.front())));
      return wh::document::document_batch{std::move(doc)};
    }
  ...
  auto parse_tag = [](const wh::document::parser::parser &parser_handle,
    ...
    auto parsed = parser_handle.parse(wh::document::parser::parse_request{"body", {}});
    REQUIRE(parsed.has_value());
    REQUIRE(parsed.value().size() == 1U);
    REQUIRE(parsed.value().front().metadata_or<std::string>("tag", "") == expected_tag);
  };
...
TEST_CASE("document processor parser callbacks and metadata injection stay consistent",
  ...
  public:
    [[nodiscard]] auto descriptor() const -> wh::core::component_descriptor {
      return wh::core::component_descriptor{"PassthroughParser",
  ...
  processor.set_loader(
      [](const std::string &,
         const wh::document::loader_options &) -> wh::core::result<std::string> { return "body"; });
  ...
  REQUIRE(parsed.value()[0].metadata_or<std::string>("_source", "") == "override://uri");
...
TEST_CASE("document async path only exists for true async implementations",
  ...
  static_assert(
      !document_async_available<wh::document::document<wh::document::document_processor>>);
...
TEST_CASE("document loader callbacks report loaded bytes for uri sources",
  ...
  processor.set_loader(
      [](const std::string &uri,
         const wh::document::loader_options &) -> wh::core::result<std::string> { return uri; });
  wh::document::document component{std::move(processor)};
  auto parsed = component.process(
      wh::document::document_request{.source_kind = wh::document::document_source_kind::uri,
                                     .source = "doc-content",
