...
TEST_CASE("async mutex public facade transfers the lock to queued waiters in FIFO order",
  ...
  auto holder_operation = stdexec::connect(
      mutex.lock(), wh::testing::helper::sender_capture_receiver<lock_guard_t, scheduler_env_t>{
  ...
  REQUIRE(holder_capture.terminal == wh::testing::helper::sender_terminal_kind::value);
  ...
  REQUIRE(first_waiter_capture.terminal == wh::testing::helper::sender_terminal_kind::value);
  REQUIRE_FALSE(second_waiter_capture.ready.try_acquire());
  ...
  REQUIRE(second_waiter_capture.terminal == wh::testing::helper::sender_terminal_kind::value);
...
TEST_CASE("async mutex public facade stops one pending waiter and keeps the mutex reusable",
  ...
  auto stopped_operation = stdexec::connect(
      mutex.lock(), wh::testing::helper::sender_capture_receiver<lock_guard_t, scheduler_env_t>{
                        &stopped_capture, {{&stop_scheduler}, stop_source.get_token()}});
