...
struct int_identity_component {
  auto invoke(int value, wh::core::run_context &) const -> wh::core::result<int> { return value; }
};

struct string_identity_component {
  auto invoke(std::string value, wh::core::run_context &) const -> wh::core::result<std::string> {
    return value;
  }
};
...
TEST_CASE("compose subgraph node preserves explicit stream output boundary",
  ...
  auto subgraph = wh::compose::make_subgraph_node("child", std::move(child_graph).value());
  auto lowered_subgraph = build_single_node_graph(subgraph);
  REQUIRE(lowered_subgraph.has_value());
  REQUIRE(lowered_subgraph->node->meta.input_contract == wh::compose::node_contract::value);
  REQUIRE(lowered_subgraph->node->meta.output_contract == wh::compose::node_contract::stream);
}
...
TEST_CASE("compose subgraph node executes real child graph through nested sender path",
  ...
  auto output = execute_single_compiled_node(subgraph, wh::core::any(5), context,
...
TEST_CASE("compose subgraph node executes child chain through shared nested invoke protocol",
  ...
  wh::core::run_context context{};
  auto output = execute_single_compiled_node(subgraph, wh::core::any(4), context,
                                             make_test_node_runtime(std::addressof(call_scope)));
  REQUIRE(output.has_value());
  auto typed = read_graph_value<int>(std::move(output).value());
  REQUIRE(typed.has_value());
...
TEST_CASE("compose subgraph graph_view wrapper bypasses public invoke path",
  ...
  struct graph_view_wrapper {
    ...
    [[nodiscard]] auto graph_view() const noexcept -> const wh::compose::graph & { return *child; }

    [[nodiscard]] auto invoke(wh::core::run_context &, const wh::compose::graph_value &) const
        -> wh::compose::graph_sender {
      public_invokes->fetch_add(1, std::memory_order_acq_rel);
      return wh::compose::graph_sender{stdexec::just(
          wh::core::result<wh::compose::graph_value>::failure(wh::core::errc::contract_violation))};
    }
  };
  ...
  SECTION("child exact input gate rejects mismatched parent source") {
    ...
    REQUIRE(parent
                .add_component(wh::compose::make_component_node<
                               ...
                               wh::compose::node_contract::value, std::string, std::string>(
                    "source", string_identity_component{}))
                .has_value());
    ...
    REQUIRE(parent.diagnostics().back().message.find("source -> child") != std::string::npos);
  }
  ...
  SECTION("child exact output gate rejects mismatched parent consumer") {
    ...
    REQUIRE(parent
                .add_component(wh::compose::make_component_node<
                               wh::core::component_kind::custom, wh::compose::node_contract::value,
                               wh::compose::node_contract::value, std::string, std::string>(
                    "consumer", string_identity_component{}))
                .has_value());
    ...
    REQUIRE(compiled.has_error());
    REQUIRE(compiled.error() == wh::core::errc::contract_violation);
    REQUIRE(parent.diagnostics().back().message.find("child -> consumer") != std::string::npos);
  }
}
