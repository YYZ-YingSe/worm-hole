...
TEST_CASE("compose lambda node writes metadata and supports transform path",
  ...
  auto transform_node = wh::compose::make_lambda_node(
      ...
      [](wh::compose::graph_value_map &input, wh::core::run_context &,
         const wh::compose::graph_call_scope &)
          -> wh::core::result<wh::compose::graph_value_map> { return std::move(input); });
...
TEST_CASE("compose lambda node supports function and sender invoke adapters",
  ...
  auto sender_node = wh::compose::make_lambda_node<
      ...
      [](const wh::compose::graph_value &input, wh::core::run_context &,
        ...
        return stdexec::just(wh::core::result<wh::compose::graph_value>{
            wh::core::any(typed.value() + 2)});
      });

  wh::core::run_context context{};
  auto function_output =
      execute_single_compiled_node(function_node, wh::core::any(5), context);
  ...
  auto sender_output =
      execute_single_compiled_node(sender_node, wh::core::any(5), context);
...
TEST_CASE("compose lambda node supports function and sender transform adapters",
  ...
  wh::core::run_context context{};
  auto function_output =
      execute_single_compiled_node(function_node, make_payload(), context);
  REQUIRE(function_output.has_value());
  auto function_typed =
      read_graph_value<wh::compose::graph_value_map>(function_output.value());
  REQUIRE(function_typed.has_value());
  REQUIRE(function_typed.value().contains("base"));
  REQUIRE(function_typed.value().contains("task"));

  auto sender_output =
      execute_single_compiled_node(sender_node, make_payload(), context);
  REQUIRE(sender_output.has_value());
  auto sender_typed =
      read_graph_value<wh::compose::graph_value_map>(sender_output.value());
  REQUIRE(sender_typed.has_value());
...
TEST_CASE("compose lambda node supports explicit boundary matrix",
  ...
  auto value_to_stream = wh::compose::make_lambda_node<
  ...
  REQUIRE(lowered_value_to_stream->node->meta.input_contract ==
          wh::compose::node_contract::value);
  REQUIRE(lowered_value_to_stream->node->meta.output_contract ==
          wh::compose::node_contract::stream);

  auto stream_to_value = wh::compose::make_lambda_node<
  ...
  REQUIRE(lowered_stream_to_value->node->meta.input_contract ==
          wh::compose::node_contract::stream);
  REQUIRE(lowered_stream_to_value->node->meta.output_contract ==
          wh::compose::node_contract::value);

  auto stream_to_stream = wh::compose::make_lambda_node<
  ...
  auto value_to_stream_reader =
      read_graph_value<wh::compose::graph_stream_reader>(
          std::move(value_to_stream_output).value());
  ...
  auto [stream_to_value_writer, stream_to_value_reader] =
  ...
  auto [stream_to_stream_writer, stream_to_stream_reader] =
      wh::compose::make_graph_stream();
  ...
  auto stream_to_stream_output = execute_single_compiled_node(
      stream_to_stream, wh::core::any(std::move(stream_to_stream_reader)),
      context);
  REQUIRE(stream_to_stream_output.has_value());
  auto stream_to_stream_result =
      read_graph_value<wh::compose::graph_stream_reader>(
          std::move(stream_to_stream_output).value());
  REQUIRE(stream_to_stream_result.has_value());
  auto stream_to_stream_chunks = wh::compose::collect_graph_stream_reader(
      std::move(stream_to_stream_result).value());
...
TEST_CASE("compose async value-to-stream lambda should preserve graph stream reader payload",
  ...
  auto async_value_to_stream = wh::compose::make_lambda_node<
      wh::compose::node_contract::value, wh::compose::node_contract::stream,
      wh::compose::node_exec_mode::async>(
      "async-value-to-stream",
      [](wh::compose::graph_value &input, wh::core::run_context &,
         const wh::compose::graph_call_scope &) {
        auto typed = read_graph_value<int>(input);
        if (typed.has_error()) {
        ...
        auto reader =
            wh::compose::make_single_value_stream_reader(typed.value() + 1);
        if (reader.has_error()) {
          return stdexec::just(
              wh::core::result<wh::compose::graph_stream_reader>::failure(
                  reader.error()));
