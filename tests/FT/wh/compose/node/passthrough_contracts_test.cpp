...
TEST_CASE("compose passthrough node preserves explicit stream boundary",
  ...
  auto passthrough = wh::compose::make_passthrough_node<
      wh::compose::node_contract::stream>("passthrough");
  auto lowered_passthrough = build_single_node_graph(passthrough);
  REQUIRE(lowered_passthrough.has_value());
  REQUIRE(lowered_passthrough->node->meta.input_contract ==
          wh::compose::node_contract::stream);
  REQUIRE(lowered_passthrough->node->meta.output_contract ==
          wh::compose::node_contract::stream);
  ...
  auto passthrough_result =
      read_graph_value<wh::compose::graph_stream_reader>(
          std::move(passthrough_output).value());
  REQUIRE(passthrough_result.has_value());
  auto passthrough_chunks = wh::compose::collect_graph_stream_reader(
      std::move(passthrough_result).value());
