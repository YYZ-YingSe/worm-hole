...
[[nodiscard]] auto make_review_decision_reader(const wh::agent::review_decision_kind kind)
...
[[nodiscard]] auto make_plan_request_builder() -> wh::agent::plan_execute_request_builder {
  return [](const wh::agent::plan_execute_context &,
            wh::core::run_context &) -> wh::core::result<std::vector<wh::schema::message>> {
    return std::vector<wh::schema::message>{};
  };
}
...
[[nodiscard]] auto make_step_reader() -> wh::agent::plan_execute_step_reader {
  return
      [](const wh::agent::agent_output &,
         wh::core::run_context &) -> wh::core::result<std::string> { return std::string{"done"}; };
}

[[nodiscard]] auto make_plan_execute_decision_reader() -> wh::agent::plan_execute_decision_reader {
  return [](const wh::agent::agent_output &,
...
TEST_CASE("agent authoring freezes topology and transfer rules", "[core][agent][condition]") {
  ...
  REQUIRE(root.add_child(wh::agent::agent{"late"}).error() == wh::core::errc::contract_violation);
  REQUIRE(root.allow_transfer_to_child("late").has_error());
  REQUIRE(root.allow_transfer_to_child("late").error() == wh::core::errc::contract_violation);
}
...
TEST_CASE("agent authoring rejects empty child names", "[core][agent][boundary]") {
  wh::agent::agent root{"root"};
...
TEST_CASE("agent tool schema hook emits stable authored schema and freeze requires bound runner",
  ...
  wh::adk::agent_tool custom_tool{"delegate_custom", "delegate custom", wh::agent::agent{"worker"}};
  REQUIRE(custom_tool.set_input_mode(wh::adk::agent_tool_input_mode::custom_schema).has_value());
...
TEST_CASE("agent tool build hook rejects missing custom schema", "[core][agent][boundary]") {
  wh::adk::agent_tool custom_tool{"delegate_custom", "delegate custom", wh::agent::agent{"worker"}};
  REQUIRE(custom_tool.set_input_mode(wh::adk::agent_tool_input_mode::custom_schema).has_value());
...
TEST_CASE("plan execute build hook freezes planner executor and effective replanner",
  ...
  REQUIRE(authored.set_replanner_decision_reader(make_plan_execute_decision_reader()).has_value());
...
TEST_CASE("plan execute build hook rejects missing or conflicting roles",
  ...
  REQUIRE(conflicting.set_planner_request_builder(make_plan_request_builder()).has_value());
  REQUIRE(conflicting.set_executor_request_builder(make_plan_request_builder()).has_value());
  REQUIRE(conflicting.set_replanner_request_builder(make_plan_request_builder()).has_value());
  ...
  REQUIRE(
      conflicting.set_replanner_decision_reader(make_plan_execute_decision_reader()).has_value());
  ...
  REQUIRE(non_executable.set_executor(wh::agent::agent{"executor"}).has_value());
  REQUIRE(non_executable.set_planner_request_builder(make_plan_request_builder()).has_value());
  REQUIRE(non_executable.set_executor_request_builder(make_plan_request_builder()).has_value());
  REQUIRE(non_executable.set_replanner_request_builder(make_plan_request_builder()).has_value());
  ...
  REQUIRE(non_executable.set_replanner_decision_reader(make_plan_execute_decision_reader())
              .has_value());
  REQUIRE(non_executable.freeze().has_error());
  REQUIRE(non_executable.freeze().error() == wh::core::errc::contract_violation);
}
...
TEST_CASE("reflexion build hook keeps optional memory writer explicit",
  ...
  REQUIRE(missing_memory_builder
              .set_review_decision_reader(
                  make_review_decision_reader(wh::agent::review_decision_kind::revise))
              .has_value());
...
TEST_CASE("supervisor build hook requires at least one worker", "[core][agent][boundary]") {
  wh::agent::supervisor authored{"supervisor"};
