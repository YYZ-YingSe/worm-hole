...
TEST_CASE("deterministic transfer resolves authored child and parent targets with loop guard",
  ...
  auto child = wh::adk::begin_deterministic_transfer(
      root, root_state,
  ...
  auto repeated = wh::adk::begin_deterministic_transfer(
      root, root_state,
      wh::adk::transfer_target{
          .kind = wh::adk::transfer_target_kind::child,
          .agent_name = "planner",
  ...
  auto parent = wh::adk::begin_deterministic_transfer(
      planner_ref.value().get(), planner_state,
      wh::adk::transfer_target{
...
TEST_CASE("deterministic transfer resolves raw target names against authored topology",
  ...
  REQUIRE(planner_ref.has_value());
  auto to_parent =
      wh::adk::resolve_transfer_target(planner_ref.value().get(), "root");
...
TEST_CASE("deterministic transfer enforces whitelist before resolving targets",
  ...
  auto blocked = wh::adk::begin_deterministic_transfer(
      root, state,
      wh::adk::transfer_target{
          .kind = wh::adk::transfer_target_kind::child,
          .agent_name = "planner",
...
TEST_CASE("deterministic transfer appends transfer messages exactly once and skips non-normal completion",
          "[core][agent][condition]") {
  std::vector<wh::schema::message> history{};
  wh::adk::deterministic_transfer_state state{};
  ...
  const auto *tool_call =
      std::get_if<wh::schema::tool_call_part>(&assistant.parts.front());
