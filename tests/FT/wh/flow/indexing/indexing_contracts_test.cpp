...
[[nodiscard]] auto make_indexer(std::shared_ptr<indexer_probe_state> probe_state = nullptr) {
  return wh::indexer::indexer{sync_indexer_batch_impl{
      [probe_state = std::move(probe_state)](const wh::indexer::indexer_request &request)
        ...
        for (std::size_t index = 0U; index < request.documents.size(); ++index) {
          ...
          response.document_ids.push_back(sub_id == nullptr ? "doc-" + std::to_string(index)
                                                            : *sub_id);
        }
        return response;
      }}};
...
TEST_CASE("flow indexing assembler freezes under flow namespace", "[core][flow][indexing]") {
...
TEST_CASE("flow indexing parent can be used as one async indexer component node",
  ...
  REQUIRE(
      graph
          .add_component<wh::compose::component_kind::indexer, wh::compose::node_contract::value,
...
TEST_CASE("flow indexing parent executes direct write path and annotates child documents",
  ...
  parent_document.set_metadata(std::string{wh::document::parent_id_metadata_key}, "parent-id");
  ...
  REQUIRE(status.value().document_ids == std::vector<std::string>{"sub-0", "sub-1"});
  ...
  const auto *first_parent_id = probe_state->seen_documents[0].metadata_ptr<std::string>(
  ...
  const auto *first_sub_id =
      probe_state->seen_documents[0].metadata_ptr<std::string>(wh::document::sub_id_metadata_key);
  REQUIRE(first_sub_id != nullptr);
  REQUIRE(*first_sub_id == "sub-0");

  const auto *second_sub_id =
      probe_state->seen_documents[1].metadata_ptr<std::string>(wh::document::sub_id_metadata_key);
