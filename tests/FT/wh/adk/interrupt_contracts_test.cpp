...
TEST_CASE("adk interrupt run path projection keeps only agent and tool segments",
  ...
  const auto projected = wh::adk::project_interrupt_run_path(
      wh::core::address{"graph", "root", "agent", "planner", "node", "inner", "tool", "search",
                        "call-7", "compose", "lane"});
...
TEST_CASE("adk interrupt info distinguishes exact and descendant resume targets",
  ...
  REQUIRE(exact.value().run_path.to_string("/") == "agent/worker/tool/search/call-1");
  ...
  REQUIRE(descendant.value().target_kind == wh::adk::interrupt_target_kind::descendant);
}
...
TEST_CASE("adk interrupt patch lowers approve edit and reject onto compose resume state",
  ...
  REQUIRE(wh::adk::apply_interrupt_patch(approved, context,
  ...
  REQUIRE(approved_patch.value().get().decision == wh::compose::interrupt_decision_kind::approve);
  REQUIRE(*wh::core::any_cast<std::string>(&approved_patch.value().get().data) == "original");
  REQUIRE(approved_patch.value().get().audit.audit_id == "audit-1");

  wh::core::resume_state edited{};
  REQUIRE(wh::adk::apply_interrupt_patch(edited, context,
                                         wh::adk::interrupt_patch{
  ...
  REQUIRE(edited_patch.value().get().decision == wh::compose::interrupt_decision_kind::edit);
  REQUIRE(*wh::core::any_cast<std::string>(&edited_patch.value().get().data) == "edited");

  wh::core::resume_state rejected{};
  auto rejected_status =
      wh::adk::apply_interrupt_patch(rejected, context,
                                     wh::adk::interrupt_patch{
...
TEST_CASE("adk interrupt patch batch keeps last write for same interrupt id",
  ...
  REQUIRE(patch.value().get().decision == wh::compose::interrupt_decision_kind::edit);
  REQUIRE(*wh::core::any_cast<std::string>(&patch.value().get().data) == "override");
}
