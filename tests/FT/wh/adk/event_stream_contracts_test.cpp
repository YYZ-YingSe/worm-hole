...
TEST_CASE("adk event stream transports events and closes to eof",
          "[core][adk][condition]") {
...
TEST_CASE("adk event stream helpers normalize empty endpoints",
          "[core][adk][boundary]") {
  ...
  REQUIRE(wh::adk::send_agent_event(empty_writer, make_message_event("unused"))
              .has_error());
  REQUIRE(wh::adk::send_agent_event(empty_writer, make_message_event("unused"))
              .error() == wh::core::errc::channel_closed);
...
TEST_CASE("adk event-message stream async read forwards stop into nested "
  ...
  REQUIRE(message_writer.try_write(make_assistant_message("resumed message"))
              .has_value());
  ...
  REQUIRE(std::get<wh::schema::text_part>(resumed.value().value->parts.front())
