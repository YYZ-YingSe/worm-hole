...
TEST_CASE("adk custom event transport preserves payload and metadata", "[core][adk][condition]") {
  auto [writer, reader] = wh::adk::make_agent_event_stream();
  auto event = wh::adk::make_custom_event("business.custom", wh::core::any{9},
  ...
  const auto *custom = std::get_if<wh::adk::custom_event>(&next.value().value->payload);
  ...
  REQUIRE(next.value().value->metadata.path == wh::adk::run_path{{"root", "agent"}});
}
