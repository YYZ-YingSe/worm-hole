...
[[nodiscard]] inline auto make_runtime_stream_identity_graph(
    const wh::compose::graph_runtime_mode mode,
  ...
  wh::compose::graph graph{std::move(options)};
  auto added =
      graph.add_passthrough<wh::compose::node_contract::stream>("worker");
  if (added.has_error()) {
    return wh::core::result<wh::compose::graph>::failure(added.error());
  }
  auto start = graph.add_entry_edge("worker");
...
[[nodiscard]] inline auto capture_exact_checkpoint(
  ...
  if (snapshot.nodes.empty()) {
    return wh::core::result<wh::compose::checkpoint_state>::failure(
      ...
      [target_node_key, graph_name = graph.options().name](
        ...
        return std::optional<wh::core::interrupt_signal>{
            wh::compose::make_interrupt_signal(
                ...
                wh::core::make_address({graph_name, target_node_key}),
                std::monostate{})};
      }};

  auto invoked = invoke_graph_sync(graph, std::move(input), context, controls,
