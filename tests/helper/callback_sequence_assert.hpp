...
public:
  auto record(const wh::core::callback_stage stage) -> void { stages_.push_back(stage); }

  [[nodiscard]] auto stages() const noexcept -> std::span<const wh::core::callback_stage> {
    return {stages_.data(), stages_.size()};
  }

  [[nodiscard]] auto expect(const std::initializer_list<wh::core::callback_stage> expected) const
      -> wh::core::result<void> {
