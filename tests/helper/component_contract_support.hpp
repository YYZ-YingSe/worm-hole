...
template <typename text_t>
  requires std::constructible_from<std::string, text_t &&>
[[nodiscard]] auto make_assistant_message(text_t &&text) -> wh::schema::message {
  wh::schema::message message{};
...
[[nodiscard]] auto take_try_chunk(reader_t &reader) ->
  ...
  if (std::holds_alternative<wh::schema::stream::stream_signal>(next)) {
    return wh::core::result<typename std::remove_cvref_t<reader_t>::chunk_type>::failure(
...
concept document_async_available =
    requires(document_t &component, wh::document::document_request document_request_value,
...
concept tool_async_invoke_available = requires(
    tool_t &tool, wh::tool::tool_request tool_request_value, wh::core::run_context &run_context) {
...
public:
  explicit tool_binding_probe_model_impl(const std::shared_ptr<tool_binding_probe_state> &state,
                                         const std::size_t bound_tool_count = 0U)
      : state_(state), bound_tool_count_(bound_tool_count) {}

  explicit tool_binding_probe_model_impl(std::shared_ptr<tool_binding_probe_state> &&state,
                                         const std::size_t bound_tool_count = 0U)
      : state_(std::move(state)), bound_tool_count_(bound_tool_count) {}

  [[nodiscard]] auto descriptor() const -> wh::core::component_descriptor {
    return wh::core::component_descriptor{"ToolBindingProbeModel", wh::core::component_kind::model};
  }
...
using tool_binding_probe_model = wh::model::chat_model<tool_binding_probe_model_impl>;
...
sync_tool_impl(invoke_impl_t, stream_impl_t) -> sync_tool_impl<invoke_impl_t, stream_impl_t>;
...
template <typename fn_t> sender_tool_invoke_impl(fn_t) -> sender_tool_invoke_impl<fn_t>;
...
template <typename fn_t> sender_tool_stream_impl(fn_t) -> sender_tool_stream_impl<fn_t>;
...
template <typename invoke_impl_t>
sender_tool_impl(invoke_impl_t) -> sender_tool_impl<invoke_impl_t, missing_tool_path>;

template <typename invoke_impl_t, typename stream_impl_t>
sender_tool_impl(invoke_impl_t, stream_impl_t) -> sender_tool_impl<invoke_impl_t, stream_impl_t>;
...
template <typename fn_t> sync_indexer_single_impl(fn_t) -> sync_indexer_single_impl<fn_t>;
